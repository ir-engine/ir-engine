...
import { createMessageHandler, JavelinMessage, JavelinMessageType } from "@javelin/net"
...
import NetworkedPosition3D from "./components/NetworkedPosition3D"
import { app, framerate, updateBytesTransferred } from "../jav/src/client/graphics"
import { interpolate, render } from "../jav/src/client/systems"
...
world.registerComponentFactory(NetworkedPosition3D)
...
function createPositionBuffer(entity: number) {
  ...
  world.insert(local, NetworkedPosition3D.create())
}
...
function updatePositionBuffer(component: ComponentOf<typeof Position>) {
  ...
  const buffer = world.storage.findComponent(entity, NetworkedPosition3D)
