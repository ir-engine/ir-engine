import { createQuery, mut, World } from "@javelin/ecs"
import { NetworkedPosition } from "../components/NetworkedPosition3D"

const buffers = createQuery(mut(NetworkedPosition))
...
export function interpolate(dt: number, world: World) {
  ...
  for (const [buffer] of world.query(buffers)) {
    ...
    if (buffer.updates.length >= 2 && buffer.updates[0][0] <= renderTime && renderTime <= buffer.updates[1][0]) {
