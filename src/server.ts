...
async function main() {
  ...
  for (let i = 0; i < numCPUs; i++) {
    ...
    const { worker, router, roomState } = await startMediasoup()
    workers[i] = worker
    routers[i] = router
  ...
  if (process.env.CERT) {
    ...
    server = createHttpsServer(tls, expressApp as any)
    server.on("error", e => {
    ...
    await new Promise(resolve => {
      server.listen(3001, () => {
        console.log(`server is running and listening on ` + `https://localhost:3001`)
        resolve()
      })
    })
  } else {
    server = createHttpServer({}, expressApp as any)
    server.listen(3001, () => {
  ...
  setInterval(() => {
    ...
    for (let i = 0; i < roomStates.length; i++) {
      Object.entries(roomStates[i].peers).forEach(([id, p]) => {
        if (now - (p as any).lastSeenTs > config.httpPeerStale) {
  ...
  io.on("connection", socket => {
    ...
    socket.on("sync", async (data, callback) => {
      ...
      try {
        ...
        const peerLoc = peerLocations[peerId.toString()]
    ...
    socket.on("join-as-new-peer", async (data, callback) => {
      try {
        ...
        // assign random room:
        const peerLoc = getRandomInt(0, numCPUs - 1)
    ...
    socket.on("create-transport", async (data, callback) => {
      try {
        ...
        transport = await createWebRtcTransport({ peerId, direction })
    ...
    socket.on("send-track", async (data, callback) => {
      ...
      for (let i = 0; i < numCPUs; i++) {
        if (i == peerLoc) {
          ...
          const { pipeProducer } = await routers[peerLoc].pipeToRouter({
    ...
    socket.on("recv-track", async (data, callback) => {
      try {
        ...
        const producer = roomStates[peerLoc].producers.find(p => p.appData.mediaTag === mediaTag && p.appData.peerId === mediaPeerId)
        ...
        const transport = Object.values(roomStates[peerLoc].transports).find(
...
function closePeer(peerId) {
  ...
  const peerLoc = peerLocations[peerId.toString()]
  for (const [, transport] of Object.entries(roomStates[peerLoc].transports)) {
    if ((transport as any).appData.peerId === peerId) {
