...
class EntityPool extends ObjectPool<Entity> {
  ...
  expand(count) {
    for (let n = 0; n < count; n++) {
      const clone = new this.T(this.entityManager)
      clone._pool = this
...
export class EntityManager {
  ...
  entityAddComponent(entity, Component, values) {
    ...
    const componentPool = this.world.componentsManager.getComponentsPool(Component)
  ...
  entityRemoveComponent(entity, Component, immediately) {
    const index = entity._ComponentTypes.indexOf(Component)
    if (!~index) return
  ...
  stats() {
    ...
    for (const ecsyComponentId in this.componentsManager._componentPool) {
      const pool = this.componentsManager._componentPool[ecsyComponentId]
