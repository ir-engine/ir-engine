...
export const handleTouch: Behavior = (
      entity: Entity,
      args: { event: TouchEvent; value: Binary }): void => {
  if (args.value === BinaryValue.ON) {
    let s: string = 'Touch start.';
...
export const handleTouchMove: Behavior = (
      entity: Entity,
      args: { event: TouchEvent }): void => {
  let s: string = 'Touch move.';
  if (args.event.targetTouches.length)
    s += (' x: ' + Math.trunc(args.event.targetTouches[0].clientX) +
        ', y: ' + Math.trunc(args.event.targetTouches[0].clientY));
  console.log(s);
