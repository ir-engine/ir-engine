...
export default class InputActionSystem extends System {
  ...
  public execute(): void {
    ...
    if (this._userInputActionQueue.buffer.getBufferLength() < 1) return
    this.queries.actionReceivers.results.forEach(receiver => {
      if (receiver.getComponent(InputActionHandler).buffer.getBufferLength() > 0) {
        receiver.getMutableComponent(InputActionHandler).buffer.clear()
      }
      this.applyInputToListener(this._userInputActionQueue, receiver)
    })
    this._userInputActionQueue.buffer.clear()
  }

  private validateActions(actionHandler: Entity): void {
    const actionQueueArray = actionHandler.getComponent(InputActionHandler).buffer.toArray()
    for (let i = 0; i < actionQueueArray.length; i++) {
      for (let k = 0; k < actionQueueArray.length; k++) {
        ...
        if (this.actionsOpposeEachOther(actionQueueArray, i, k)) {
          actionHandler.getMutableComponent(InputActionHandler).buffer.remove(i)
          actionHandler.getMutableComponent(InputActionHandler).buffer.remove(k)
        }
  ...
  private applyInputToListener(userInputActionQueue: InputActionHandler, listenerActionQueue: Entity) {
    ...
    userInputActionQueue.buffer.toArray().forEach(userInput => {
      this._skip = false
      listenerActionQueue
        ...
        .forEach((listenerAction, listenerIndex) => {
          ...
          if (userInput.action === listenerAction.action && userInput.value === listenerAction.value) {
            ...
            listenerActionQueue.getMutableComponent(InputActionHandler).buffer.remove(listenerIndex)
            listenerActionQueue.getMutableComponent(InputActionHandler).buffer.add(userInput)
            this._skip = true
          }
        })
      if (!this._skip) {
