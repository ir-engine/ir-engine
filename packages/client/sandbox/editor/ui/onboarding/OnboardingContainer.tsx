...
const StyledOnboardingContainer = (styled as any).div`
  position: absolute;
  display: flex;
  left: 0;
  top: 0;
...
export default class OnboardingContainer extends Component {
  ...
  nextStep = () => {
    const { steps, curStepIdx } = this.state as any;
  ...
  prevStep = () => {
    const { curStepIdx } = this.state as any;
  ...
  setStep = (index, skip?) => {
    ...
    if (index === -1) {
      if (skip) {
        (this.props as any).onSkip(curStepIdx);
      } else {
        (this.props as any).onFinish();
      }
    }
  };
