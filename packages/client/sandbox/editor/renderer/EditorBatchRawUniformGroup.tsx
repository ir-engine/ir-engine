...
export default class EditorBatchRawUniformGroup extends BatchRawUniformGroup {
  meshAtlases = new Map();
  addMesh(mesh, atlas) {
  ...
  update(_time) {
    for (let instanceId = 0; instanceId < this.meshes.length; instanceId++) {
      ...
      this.setInstanceTransform(
        instanceId,
        getVisibility(mesh) ? mesh.matrixWorld : HIDE_MATRIX
      );
