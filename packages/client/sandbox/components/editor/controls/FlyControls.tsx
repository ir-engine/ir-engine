...
export default class FlyControls {
  ...
  constructor(camera, inputManager) {
    ...
    this.maxXRotation = MathUtils.degToRad(80);
  ...
  update(dt) {
    ...
    candidateWorldQuat.multiplyQuaternions(
      quat.setFromAxisAngle(
        ...
        input.get(Fly.lookY) * this.lookSensitivity
      ),
      worldQuat
    );
    ...
    const newCamForwardY = vec.set(0, 0, -1).applyQuaternion(candidateWorldQuat)
      .y;
    const extrema = Math.sin(this.maxXRotation);
    const allowRotationInX =
      newCamUpY > 0 &&
      ((newCamForwardY < extrema && newCamForwardY > -extrema) ||
        newCamUpY > camUpY);
    if (allowRotationInX) {
      ...
      parentInverse.getInverse(
        this.camera.parent ? this.camera.parent.matrixWorld : IDENTITY
      );
      this.camera.matrix.multiplyMatrices(
        parentInverse,
    ...
    this.camera.position.y +=
      input.get(Fly.moveY) * dt * this.moveSpeed * boostSpeed;
  }
}
