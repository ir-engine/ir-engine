...
const ChartContainer = (styled as any).div`
  margin: 4px 0;
  padding: 4px;
...
const Thumbnail = (styled as any).img`
  width: 64px;
  height: 64px;
...
const ImageItemContainer = (styled as any).li`
  display: flex;
  margin: 4px 0;
  padding: 4px;
  background-color: ${props => props.theme.panel2};
...
const MeshItemContainer = (styled as any).li`
  display: flex;
...
const StatsContainer = (styled as any).ul`
  margin: 4px 0;
  padding: 8px;
  background-color: ${props => props.theme.panel2};
  border-radius: 4px;
...
const ValidationInfoContainer = (styled as any).div`
  margin: 4px 0;
  padding: 8px;
  background-color: ${props => props.theme.panel2};
  border-radius: 4px;
...
const IssueItemContainer = (styled as any).li`
  margin: 4px 0;
  padding: 8px;
  background-color: ${props => props.theme.panel2};
  border-radius: 4px;
...
const NoIssues = (styled as any).div`
  width: 100%;
  text-align: center;
...
export function GLTFValidation({ node }) {
  ...
  const onValidate = useCallback(() => {
    const validate = async () => {
      ...
      try {
        const validation = await validateString(JSON.stringify(node.gltfJson), null) as any;
        setValidation(validation);
      } catch (error) {
        console.log(error);
