...
const messages = {
  ...
  "waypoint.label-willMaintainInitialOrientation":
    "Maintain initial orientation",
  "waypoint.description-canBeSpawnPoint":
    "Avatars may be teleported to this waypoint when entering the scene",
  ...
  "waypoint.description-willDisableMotion":
    "Avatars will not be able to move after using this waypoint",
  "waypoint.description-willDisableTeleport":
    "Avatars will not be able to teleport after using this waypoint",
...
> {
  constructor(props) {
    ...
    const createPropSetter = propName => value => {
      return (this.props.editor as any).setPropertySelected(propName, value);
    };
      ...
      <NodeEditor description={WayPointNodeEditor.description} {...this.props}>
        {propertyNames.map(name => (
          ...
          >
            <BooleanInput
              value={node[name]}
              onChange={this.setters.get(name)}
            />
          </InputGroup>
        ))}
      </NodeEditor>
