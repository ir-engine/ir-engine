...
class GLTFLoader {
  ...
  usesExtension(extensionName) {
    return (
      Array.isArray(this.json.extensionsUsed) &&
      this.json.extensionsUsed.indexOf(extensionName) !== -1
    );
  }
  /**
  ...
  async loadGLTF() {
    try {
      ...
      const sceneAnimations = await this.getDependency(
        "sceneAnimations",
        sceneIndex
      );
      ...
      for (const key in stats.bufferInfo) {
        if (!Object.prototype.hasOwnProperty.call(stats.bufferInfo, key))
          continue;
        const item = stats.bufferInfo[key];
        stats.totalSize += item.size || 0;
      }
      for (const key in stats.textureInfo) {
        if (!Object.prototype.hasOwnProperty.call(stats.textureInfo, key))
          continue;
  ...
  async loadScene(sceneIndex) {
    ...
    if (sceneDef.extensions)
      this.addUnknownExtensionsToUserData(scene, sceneDef);
  ...
  loadAnimationsForNodes(json, nodeIds) {
    ...
    for (
      ...
      animationIndex < animationDefs.length;
      animationIndex++
    ) {
  ...
  async loadMesh(meshIndex, options) {
    ...
    for (let i = 0, il = geometries.length; i < il; i++) {
      ...
      let mesh = await this.runFirstHook(
        "createPrimitive",
        meshDef,
        primitive,
        geometry,
  ...
  async addPrimitiveAttributes(geometry, primitiveDef) {
    ...
    const assignAttributeAccessor = (accessorIndex, attributeName) => {
      return this.getDependency("accessor", accessorIndex).then(function(
        accessor
      ) {
    ...
    for (const gltfAttributeName in attributes) {
      if (!Object.prototype.hasOwnProperty.call(attributes, gltfAttributeName))
        continue;
    ...
    if (primitiveDef.indices !== undefined && !geometry.index) {
      const accessor = this.getDependency(
        "accessor",
    ...
    return primitiveDef.targets !== undefined
      ? this.addMorphTargets(geometry, primitiveDef.targets)
      : geometry;
  }
  /**
  ...
  async loadMaterial(materialIndex) {
    ...
    if (materialDef.extensions)
      this.addUnknownExtensionsToUserData(material, materialDef);
    return material;
  }
  ...
  async loadAccessor(accessorIndex) {
    ...
    if (byteStride && byteStride !== itemBytes) {
      ...
      for (
        let elementIndex = 0;
        elementIndex < accessorDef.count;
        elementIndex++
      ) {
        for (let itemIndex = 0; itemIndex < itemSize; itemIndex++) {
          array[elementIndex * itemSize + itemIndex] =
            typedBufferView[elementIndex * stride + itemIndex + offset];
        }
      }
    } else {
      if (bufferView === null) {
  ...
  addUnknownExtensionsToUserData(object, objectDef) {
    for (const name in objectDef.extensions) {
      if (
        name == "componentData" ||
        (this.options.addUnknownExtensionsToUserData &&
          !this.knownExtensions.has(name))
      ) {
