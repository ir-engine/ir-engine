...
import { getComponent } from "../gltf/ComponentData";
...
export default class KitPackager {
  ...
  async package(kitName, url, onProgress) {
    ...
    const loader = new GLTFLoader(url, undefined, {
    ...
    scene.traverse(object => {
      ...
      if (altMaterialsComponent) {
        ...
        altMaterialsComponent.altMaterials = altMaterialsComponent.altMaterials.map(
          primitiveAltMaterials =>
    ...
    for (let i = 0; i < pieces.length; i++) {
      ...
      onProgress(
        `Generating thumbnail for "${component.name}" ${i} out of ${
          pieces.length
        }`
      );
    ...
    const zipBlob = await zip.generateAsync(
      ...
      ({ percent, currentFile }) => {
        if (currentFile) {
          onProgress(
            `Zipping "${currentFile}" ${percent.toFixed(2)}% complete`
          );
        }
      }
    );
