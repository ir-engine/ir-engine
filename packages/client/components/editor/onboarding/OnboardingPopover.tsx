...
const Popover = (styled as any).div`
  position: absolute;
  top: 0;
  left: 0;
  pointer-events: all;
...
const PopoverContent = (styled as any).div`
  display: flex;
...
const PopoverNav = (styled as any).div`
  display: flex;
  height: 50px;
  flex-direction: row;
...
export default class OnboardingPopover extends Component {
  ...
  componentDidMount() {
    const targetEl = document.querySelector((this.props as any).target);
    this.updatePosition(targetEl);
    window.addEventListener("resize", this.onResize);
  }

  componentDidUpdate(prevProps) {
    if ((this.props as any).target !== prevProps.target) {
      const targetEl = document.querySelector((this.props as any).target);
      this.updatePosition(targetEl);
    }
  }
  ...
  updatePosition(targetEl) {
    ...
    const { position, padding } = this.props as any;
    const popoverRect = (this.popoverRef.current as any).getBoundingClientRect();
  ...
  render() {
    const { transform, transformOrigin } = this.state as any;
    const {
      position,
      children,
      steps,
