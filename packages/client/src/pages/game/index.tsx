...
import icon from './ui/assets/ico/icon.svg'
...
function Component() {
  ...
  window.themeCSSLink = loadCSS('./src/pages/game/ui/assets/css/' + getTheme() + '.css')
  ...
  if (window.enableLogging) {
    ...
    var TriggerPrompt = function (articleUrl, contentId) {
      ...
      var url =
        'https://survey.g.doubleclick.net/survey?site=_5lgefwumzxr6qxsbcz46dpx624' +
        '&url=' +
  ...
  ;(function () {
    ...
    var url = 'https://quantcast.mgr.consensu.org'.concat('/choice/', 'M1Q1fpfqa7Vk4', '/', host, '/choice.js')
    var uspTries = 0
    var uspTriesLimit = 3
    element.async = true
  ...
  function withGoodBrowser() {
    ...
    console.log('Checking browser... done!')
    console.log('Loading UI...')
    console.log('Loading backend...')

    ReactDOM.render(
      <div id="content" className="h-100 d-flex flex-column">
        <UIIndex />
      </div>,
  ...
  goodIDB(function (idbResult) {
    if (idbResult !== 'good' || !checkBrowser()) {
      ...
      if (idbResult === 'open-failed') {
        errorMsg =
          '<p><b>Error!</b> Cannot store data.</p><p>If you have disabled cookies in your browser, you must enable them to play Basketball GM.</p><p>If you\'re using Firefox\'s Private Browsing mode, there is currently a bug in Firefox that prevents Basketball GM from running. Either load Basketball GM outside of Private Browsing mode, or try using <a href="https://www.google.com/chrome/">Google Chrome</a> instead.</p>'
      } else {
        errorMsg =
          '<p><b>Error!</b> Your browser is not modern enough to run Basketball GM. The latest versions of <a href="https://www.google.com/chrome/">Google Chrome</a> and <a href="https://www.mozilla.org/en-US/firefox/">Mozilla Firefox</a> work best.</p>'
      }
      ...
      if (/(iPad|iPhone|iPod)/.test(navigator.userAgent)) {
        errorMsg +=
          "<p>If you're on an iPhone/iPad, upgrade to iOS 10.3 or higher to play Basketball GM. If you can't do that, then please come back on a desktop/laptop or a non-Apple mobile device!</p>"
      }
    ...
    <div id="content" className="h-100 d-flex flex-column">
      <div style={{ margin: '0 15px' }}>
        <div style={{ maxWidth: '360px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', margin: '48px 0 100px 0' }}>
            <img
              id="loading-icon"
              src={icon}
              width="48"
              height="48"
              className="spin"
              style={{ animationPlayState: 'runnung' }}
              alt=""
            />
          </div>
          ...
          <noscript>
            <p>
              <b>Error!</b> JavaScript is disabled in your browser. Please enable JavaScript to play Basketball GM.
            </p>
          </noscript>
          <div id="startup-ui"></div>
          <div id="startup-worker"></div>

          <div className="alert alert-danger mt-3" id="startup-error" style={{ display: 'none' }}></div>

          <div className="alert alert-danger mt-3" id="startup-error-unknown" style={{ display: 'none' }}>
            <p>This should only take a few seconds on a fast connection.</p>
            <p>
              If it gets stuck for a while, read{' '}
              <a href="https://basketball-gm.com/manual/debugging/">the debugging instructions</a> and{' '}
              <a href="https://basketball-gm.com/contact/">ask for help</a> if it still isn't working.
            </p>
          </div>
        </div>
      </div>
