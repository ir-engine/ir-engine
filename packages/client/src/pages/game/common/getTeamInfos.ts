...
const getTeamInfos = (teams: { tid: number; cid: number; did: number; abbrev: string }[]) => {
  return teams.map((t) => {
    ...
    const info: {
      ...
      imgURL: `/src/pages/game/ui/assets/src/ui/assets/img/logos-primary/${actualAbbrev}.svg`
    }

    if (!noSmallLogo.includes(actualAbbrev)) {
      info.imgURLSmall = `/src/pages/game/ui/assets/src/ui/assets/img/logos-secondary/${actualAbbrev}.svg`
    }
