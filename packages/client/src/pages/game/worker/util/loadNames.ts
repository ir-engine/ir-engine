import { DefaultNames, getFrequencies, mergeCountries } from '../../common/names'
...
const toCumSumArray = <T extends string>(obj: Record<T, number>): [T, number][] => {
  let cumsum = 0
...
export const initDefaults = async (force?: boolean) => {
  if (!defaultNamesCountries || !defaultNamesGroups || force) {
    if (process.env.NODE_ENV === 'test') {
      ...
      const response = await fetch('/src/pages/game/ui/assets/files/names.json')
...
const loadNames = async (): Promise<PlayerBioInfoProcessed> => {
  ...
  const mergedCountries = mergeCountries(gPlayerBioInfo, defaultNamesCountries, defaultNamesGroups, groups)

  const countries: PlayerBioInfoProcessed['countries'] = {}
  for (const [country, { first, last, colleges, fractionSkipCollege, races }] of Object.entries(mergedCountries)) {
    if (first && last) {
      countries[country] = {
