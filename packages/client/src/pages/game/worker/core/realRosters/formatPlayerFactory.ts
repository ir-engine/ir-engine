...
import type { GetLeagueOptions, PlayerContract, PlayerInjury } from '../../../common/types'
...
const formatPlayerFactory = async (
  ...
  const getTidNormal = (abbrev?: string): number | undefined => {
    ...
    const t = teams.find((t) => t.srID !== undefined && oldAbbrevTo2020BBGMAbbrev(t.srID) === abbrev)
  ...
  return (
    ...
    const allRatings = Array.isArray(ratingsInput) ? ratingsInput : [ratingsInput]
    ...
    if (draftProspect || legends) {
      draft = {
        ...
        round: 0,
        pick: 0,
        year: legends ? season - 1 : draftRatingsAlreadySet ? ratings.season : ratings.season - 1
      }
    } else {
      let draftTid
      const draftTeam = teams.find((t) => t.srID !== undefined && oldAbbrevTo2020BBGMAbbrev(t.srID) === bio.draftAbbrev)
      if (draftTeam) {
    ...
    if (draftProspect) {
      ...
      if (options.type === 'real' && options.phase >= PHASE.PLAYOFFS) {
        ...
        statsRow = basketball.teams.find((row) => row.slug === slug && row.season === ratings.season)
      }
      ...
      if (legends) {
        const team = teams.find((t) => {
          if (hasQueens && abbrev === 'NOL' && ratings.season < 2003) {
            return t.srID !== undefined && oldAbbrevTo2020BBGMAbbrev(t.srID) === 'CHA'
          }

          return t.srID !== undefined && oldAbbrevTo2020BBGMAbbrev(t.srID) === abbrev
        })
    ...
    if (tid >= PLAYER.FREE_AGENT && !draftProspect) {
      ...
      if (
        draft.year > season ||
        (draft.year === season && (options.type === 'legends' || options.phase <= PHASE.DRAFT))
      ) {
        draft = {
          tid: -1,
          originalTid: -1,
    ...
    if (legends) {
      ...
      const awardsCutoffSeason = options.type === 'real' && options.phase > PHASE.PLAYOFFS ? season + 1 : season
      awards =
    ...
    const addDummyRookieRatings =
      !draftProspect &&
      options.type === 'real' &&
      (options.realStats === 'all' || options.realStats === 'allActive' || options.realStats === 'allActiveHOF') &&
    ...
    type StatsRow = Omit<BasketballStats[number], 'slug' | 'abbrev' | 'playoffs'> & {
      playoffs: boolean
      tid: number
      minAvailable: number
      ewa: number
    ...
    if (options.type === 'real' && basketballStats) {
      ...
      const statsSeason = options.phase > PHASE.REGULAR_SEASON ? options.season : options.season - 1
      const includePlayoffs = options.phase !== PHASE.PLAYOFFS

      if (options.realStats === 'lastSeason') {
        statsTemp = basketballStats.filter(
          (row) => row.slug === slug && row.season === statsSeason && (includePlayoffs || !row.playoffs)
        )
      } else if (
    ...
    const hof = !!awards && awards.some((award) => award.type === 'Inducted into the Hall of Fame')
    ...
    return {
      ...
      hgt: bio.height,
      tid,
      imgURL: '/src/pages/game/ui/assets/img/blank-face.png',
      real: true,
      draft,
