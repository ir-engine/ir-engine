...
export type Basketball = {
  ...
  draftPicks: Record<
    number,
    {
      ...
      range?: [[number, number], [number, number]] | [[number, number], null] | [null, [number, number]]
    }[]
  >
  freeAgents: any[]
  teamSeasons: Record<
...
const loadData = async () => {
  ...
  const response = await fetch('/src/pages/game/ui/assets/files/real-player-data.json')
