 ...
 * @return {Promise}
 */
const genPlayers = async (draftYear: number, scoutingRank: number | undefined | null = null, forceScrubs?: boolean) => {
  // If scoutingRank is not supplied, have to hit the DB to get it
  if (scoutingRank === undefined || scoutingRank === null) {
    const teamSeasons = await idb.cache.teamSeasons.indexGetAll('teamSeasonsByTidSeason', [
  ...
  const allDraftProspects = await idb.cache.players.indexGetAll('playersByTid', PLAYER.UNDRAFTED)

  const existingPlayers = allDraftProspects.filter((p) => p.draft.year === draftYear)
  ...
  if (g.get('randomDebutsForever') !== undefined) {
    ...
    const nextRealPlayers = allDraftProspects.filter((p) => p.real && p.draft.year === draftYear + 1).length
    if (currentRealPlayers === 0 || nextRealPlayers === 0) {
      await realRosters.updateRandomDebutsForever(draftYear, currentRealPlayers)
      return
    }
  }

  const players = await genPlayersWithoutSaving(draftYear, scoutingRank, existingPlayers, forceScrubs)
  ...
  if (!forceScrubs) {
    if (Math.random() < 1 / 100000) {
      const p = player.generate(PLAYER.UNDRAFTED, 19, draftYear, false, scoutingRank)
      p.born.year = draftYear - 48
      p.born.loc = 'Los Angeles, CA'
      p.college = 'Washington State University'
      p.firstName = 'LaVar'
      p.hgt = 78
      p.imgURL = '/src/pages/game/ui/assets/img/lavar.jpg'
      p.lastName = 'Ball'
      p.weight = 250
      ...
      if (typeof pid === 'number') {
        await logEvent({
          type: 'playerFeat',
          text: `<a href="${helpers.leagueUrl(['player', pid])}">${p.firstName} ${
            p.lastName
      ...
      p.firstName = 'Barack'
      p.hgt = 73
      p.imgURL = '/src/pages/game/ui/assets/img/obama.jpg'
      p.lastName = 'Obama'
      p.weight = 175
      Object.assign(p.ratings[0], {
        hgt: 20,
      ...
      p.firstName = 'Donald'
      p.hgt = 75
      p.imgURL = '/src/pages/game/ui/assets/img/trump.jpg'
      p.lastName = 'Trump'
      p.weight = 240
      Object.assign(p.ratings[0], {
        hgt: 40,
