...
import { DataTable, DraftAbbrev, SkillsBlock, PlayerNameLabels, MoreLinks } from '../components'
  ...
  return (
    <button
      ...
      onClick={async () => {
        ...
        const { filename, json } = await toWorker('main', 'exportDraftClass', season)
...
const DraftHistory = ({ challengeNoRatings, draftType, players, season, stats, userTid }: View<'draftHistory'>) => {
    ...
    return {
      ...
      data: [
        ...
        p.pos,
        {
          ...
          value: <DraftAbbrev originalTid={p.draft.originalTid} tid={p.draft.tid} season={season} />
        },
        p.draft.age,
        ...
        <a href={helpers.leagueUrl(['roster', `${p.currentAbbrev}_${p.currentTid}`])}>{p.currentAbbrev}</a>,
        p.currentAge,
  ...
  return (
    <>
      <MoreLinks type="draft" page="draft_history" draftType={draftType} season={season} />

      <p>
        Players drafted by your team are <span className="text-info">highlighted in blue</span>. Players in the Hall of
        Fame are <span className="text-danger">highlighted in red</span>.
      </p>

      <ExportButton season={season} />

      <DataTable cols={cols} defaultSort={[0, 'asc']} name="DraftHistory" rows={rows} superCols={superCols} />
    </>
  )
}
