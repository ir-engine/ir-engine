...
import type { DraftLotteryResultArray, View, DraftType } from '../../common/types'
...
const draftTypeDescriptions: Record<DraftType | 'dummy', string> = {
  ...
  randomLotteryFirst3: 'Random lottery for the top 3 picks, like the NBA from 1987-1989',
  randomLottery: 'Non-playoff teams draft in random order, like the NBA from 1985-1986',
  coinFlip: 'Coin flip to determine the top 2 picks, like the NBA from 1966-1984',
  noLottery: 'No lottery, teams draft in order of their record, from worst to best with non-playoff teams coming first',
  noLotteryReverse:
  ...
  freeAgents: 'There is no draft and all, rookies simply become free agents who can be signed by any team',
  dummy: 'From historical data'
}

const getProbs = (
  result: DraftLotteryResultArray,
  draftType: Exclude<DraftType, 'random' | 'noLottery' | 'noLotteryReverse' | 'freeAgents'>
): (number | undefined)[][] => {
  const probs: number[][] = []
  const topNCombos = new Map()
  const totalChances = result.reduce((total, { chances }) => total + chances, 0)
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    for (let k = 0; k < result.length; k++) {
      if (k !== i) {
        probs[i][1] += ((result[k].chances / totalChances) * result[i].chances) / (totalChances - result[k].chances)
          ...
          if (l !== i && l !== k) {
            ...
            if (draftType === 'nba2019') {
              ...
              for (let m = 0; m < result.length; m++) {
                if (m !== i && m !== k && m !== l) {
                  const combosTemp2 =
                    ((result[k].chances / totalChances) *
                      (result[l].chances / (totalChances - result[k].chances)) *
                      (result[m].chances / (totalChances - result[k].chances - result[l].chances)) *
                      result[i].chances) /
                    (totalChances - result[k].chances - result[l].chances - result[m].chances)
                  ...
                  if (!topNCombos.has(topFourKey)) {
                    ...
                    topNCombos.set(topFourKey, topNCombos.get(topFourKey) + combosTemp2)
                  }
                }
              }
            } else {
              ...
              if (!topNCombos.has(topThreeKey)) {
                ...
                topNCombos.set(topThreeKey, topNCombos.get(topThreeKey) + combosTemp)
              }
            }
          }
        }
...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case 'revealOne': {
      ...
      const revealState = indRevealed >= state.toReveal.length - 1 ? 'done' : state.revealState
      return {
        ...state,
        indRevealed,
        revealState
...
const Rigged = ({ numToPick, result, rigged, type }: Pick<Props, 'numToPick' | 'result' | 'rigged' | 'type'>) => {
            ...
            {result.map(({ dpid, tid, originalTid }) => {
              ...
              return (
                <option key={dpid} value={dpid}>
                  {abbrev === originalAbbrev ? abbrev : `${abbrev} (from ${originalAbbrev})`}
                </option>
              )
            })}
          </select>
...
const DraftLotteryTable = (props: Props) => {
  ...
  const revealPickAuto = () => {
    if (!isMounted.current || numLeftToReveal.current <= 0 || revealState.current !== 'running') {
      return
    }
  ...
  const { numToPick, rigged, season, type, userTid } = props
  ...
  const showStartButton = type === 'readyToRun' && state.revealState === 'init' && result && result.length > 0
  ...
  if (props.notEnoughTeams) {
    return (
      <div className="alert alert-danger d-inline-block">
        <p>
          <b>Error:</b> Not enough teams for your selected lottery type, so there will be no draft lottery.
        </p>
        {props.challengeWarning ? (
          <p>
            This is probably because you're using the "no draft picks" challenge mode and controlling many teams in
            multi team mode, so not enough teams are eligible for the draft lottery.
          </p>
        ) : null}
      </div>
    )
        ...
        <ResponsiveTableWrapper nonfluid>
          <table className="table table-striped table-bordered table-sm table-hover">
            <thead>
              ...
              <Rigged numToPick={numToPick} rigged={rigged} result={props.result} type={props.type} />
            </thead>
            <tbody>
              {result.map((t, i) => (
                <Row
  ...
  return (
    <>
      ...
      {showStartButton ? (
        <button className="btn btn-large btn-success" onClick={() => startLottery()}>
          Start Lottery
        </button>
      ) : null}
      {type === 'readyToRun' && (state.revealState === 'running' || state.revealState === 'paused') ? (
        <div className="btn-group">
          {state.revealState === 'paused' ? (
            <button className="btn btn-light-bordered" onClick={handleResume} title="Resume Lottery">
              <span className="glyphicon glyphicon-play" />
            </button>
          ) : (
            <button className="btn btn-light-bordered" onClick={handlePause} title="Pause Lottery">
...
const DraftLottery = (props: Props) => {
  ...
  return (
    <>
      <MoreLinks type="draft" page="draft_lottery" draftType="nba1994" season={props.season} />

      {props.type === 'projected' ? (
        <p>This is what the draft lottery probabilities would be if the lottery was held right now.</p>
      ) : null}

      {props.showExpansionTeamMessage ? (
        <p>New expansion teams are treated as if they won half their games but missed the playoffs.</p>
      ) : null}
