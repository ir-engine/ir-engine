...
const PlayerTeam = ({
  ...
  return <a href={helpers.leagueUrl(['roster', `${p.abbrev}_${p.tid}`, season])}>{p.abbrev}</a>
}
...
const AllStarHistory = ({ allAllStars, userTid }: View<'allStarHistory'>) => {
  ...
  const cols = getCols(['Season', 'Result', 'Captain 1', 'Team', 'Captain 2', 'Team', 'MVP', 'Team'])

  const rows = allAllStars.map((row) => {
    const classNamesCaptain1 = row.captain1 && row.captain1.tid === userTid ? 'table-info' : ''
    const classNamesCaptain2 = row.captain2 && row.captain2.tid === userTid ? 'table-info' : ''
    const classNamesMVP = row.mvp && row.mvp.tid === userTid ? 'table-info' : ''
    ...
    return {
      ...
      data: [
        ...
        },
        {
          ...
          value: (
            // @ts-ignore
            <PlayerName p={row.captain1}>{row.captain1 ? row.captain1.name : '???'}</PlayerName>
          )
        },
        {
          classNames: classNamesCaptain1,
        ...
        },
        {
          ...
          value: (
            // @ts-ignore
            <PlayerName p={row.captain2}>{row.captain2 ? row.captain2.name : '???'}</PlayerName>
          )
        },
        {
          classNames: classNamesCaptain2,
        ...
        },
        {
          ...
          value: (
            // @ts-ignore
            <PlayerName p={row.mvp}>{row.mvp ? row.mvp.name : '???'}</PlayerName>
          )
        },
        {
          classNames: classNamesMVP,
  ...
  return (
    <>
      ...
      <DataTable cols={cols} defaultSort={[0, 'desc']} name="AllStarHistory" pagination={pagination} rows={rows} />
    </>
  )
}
