...
const OwnerMoodsChart = ({ ownerMoods, year }: { ownerMoods: OwnerMood[]; year: number }) => {
  ...
  useEffect(() => {
    if (node) {
      ...
      const yScale = scaleLinear().domain(yDomain).range([height, 0]) as (y: number) => number
      ...
      const drawReferenceLine = (y: number, color: string, text?: string, position?: string) => {
      ...
      const drawLine = (attr: Exclude<keyof Data, 'year'>, color: string, strokeWidth = 1) => {
      ...
      svg.append('g').attr('class', 'chart-axis').attr('transform', `translate(0,${height})`).call(axisBottom(xScale))
    }
  ...
  return (
    <div className="position-relative mt-n1">
      ...
      >
        <p>This plot shows what the owner thinks of you and how that's changed over time.</p>
        <p>
          If your <b>Total</b> line drops below the <span className="text-danger">You're fired!</span> cutoff, then
          you're fired!
        </p>
        <p>
          The other lines (regular season, playoffs, finances) cannot individually get you fired. You only get fired
          based on the combination, which is the <b>Total</b> line.
        </p>
