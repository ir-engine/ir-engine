...
const Charts = ({
  ...
  teams,
  usePts
}: Pick<View<'tradeSummary'>, 'phase' | 'season' | 'seasonsToPlot' | 'stat' | 'teams' | 'usePts'>) => {
  ...
  useEffect(() => {
    if (node && node2) {
      ...
      const drawHorizontal = (svg: any, yScale: (y: number) => number, y: number, color: string) => {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const filtered = seasonsToPlot.filter((row) => row.teams[i][valueKey] !== undefined)
      ...
      for (let i = 0; i < 2; i++) {
        ...
        const filtered = seasonsToPlot.filter((row) => row.teams[i].stat !== undefined)
  ...
  return (
    <>
      ...
      >
        <div className="text-center">Team {usePts ? 'point' : 'winning'} percentages before and after the trade</div>
        <div ref={getNode} />
        <div
