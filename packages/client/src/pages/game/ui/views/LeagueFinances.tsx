...
const LeagueFinances = ({
  ...
  const rows = teams.map((t) => {
    ...
    const payroll = season === currentSeason ? t.seasonAttrs.payroll : t.seasonAttrs.salaryPaid

    const data: ReactNode[] = [
      <a href={helpers.leagueUrl(['team_finances', `${t.seasonAttrs.abbrev}_${t.seasonAttrs.tid}`])}>
      ...
      ...(showTicketPrice ? [helpers.formatCurrency(t.budget.ticketPrice.amount, '', 2)] : []),
      ...(budget
        ? [
  ...
  return (
    <>
      <p>
        Salary cap: <b>{helpers.formatCurrency(salaryCap, 'M')}</b> (teams over this amount cannot sign{' '}
        {hardCap ? 'players' : 'free agents'} for more than the minimum contract)
        <br />
        Minimum payroll limit: <b>{helpers.formatCurrency(minPayroll, 'M')}</b> (teams with payrolls below this limit
        will be assessed a fine equal to the difference at the end of the season)
        {!hardCap ? (
          <>
            <br />
            Luxury tax limit: <b>{helpers.formatCurrency(luxuryPayroll, 'M')}</b> (teams with payrolls above this limit
            will be assessed a fine equal to {luxuryTax} times the difference at the end of the season)
          </>
        ) : null}
      </p>

      <DataTable cols={cols} defaultSort={[5, 'desc']} name="LeagueFinances" nonfluid rows={rows} />
    </>
  )
}
