...
const Seasons = ({ history }: Pick<View<'teamHistory'>, 'history'>) => {
  const numTeamNames = new Set(history.map((h) => h.name).filter((name) => name !== undefined)).size
  ...
  const historySeasons = history.map((h, i) => {
    const recordAndPlayoffs = (
      <RecordAndPlayoffs
        ...
        style={h.playoffRoundsWon === h.numPlayoffRounds ? { fontWeight: 'bold' } : undefined}
        tid={h.tid}
        tied={h.tied}
        won={h.won}
      />
    ...
    return (
      <div key={h.season} className={gap && !newName ? 'mt-2' : undefined}>
        {newName ? <h4 className={i > 0 ? 'mt-2' : undefined}>{newName}</h4> : null}
        {recordAndPlayoffs}
        <br />
      </div>
    )
