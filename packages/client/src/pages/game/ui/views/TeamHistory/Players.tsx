...
const Players = ({
  ...
  Partial<Pick<View<'teamHistory'>, 'season' | 'userTid'>> & {
    gmHistory?: boolean
  }) => {
  const includeRetireJerseyButton = tid === userTid && !gmHistory
  ...
  const retireJerseyNumber = async (p: any) => {
    ...
    const seasonTeamInfo =
      p.retirableJerseyNumbers[number] && p.retirableJerseyNumbers[number].length > 0
        ? p.retirableJerseyNumbers[number][p.retirableJerseyNumbers[number].length - 1]
        : season
  ...
  const cols = getCols(['Name', 'Pos', ...stats.map((stat) => `stat:${stat}`), 'Last Season', 'Actions'])
  ...
  const rows = players.map((p) => {
    const canRetireJerseyNumber =
      !!p.retirableJerseyNumbers && Object.keys(p.retirableJerseyNumbers).length > 0 && p.tid !== tid

    return {
      key: p.pid,
      data: [
        <PlayerNameLabels injury={p.injury} jerseyNumber={p.jerseyNumber} pid={p.pid} watch={p.watch}>
          {p.name}
        </PlayerNameLabels>,
        p.pos,
  ...
  return (
    <>
      ...
      <p>
        ...
        <span className="text-success">highlighted in green</span>. Other active players are{' '}
        <span className="text-info">highlighted in blue</span>. Players in the Hall of Fame are{' '}
        <span className="text-danger">highlighted in red</span>.
      </p>
      <DataTable cols={cols} defaultSort={[2, 'desc']} name="TeamHistory" rows={rows} pagination />
    </>
  )
}
