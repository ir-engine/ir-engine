...
const PlayerStatDists = ({ numGames, season, statType, statsAll }: View<'playerStatDists'>) => {
  useTitleBar({
  ...
  return (
    <>
      ...
      <p>
        These <a href="http://en.wikipedia.org/wiki/Box_plot">box plots</a> show the league-wide distributions of player
        stats for all active players in the selected season. Blue plots are for this league and green plots are from the
        2018-2020 NBA seasons, for comparison. NBA data was generously provided by{' '}
        <a href="https://www.basketball-reference.com/">basketball-reference.com</a>. The five vertical lines in each
        plot represent the minimum of the scale, the minimum, the first{' '}
            ...
            .map((stat) => {
              ...
              const bbgmPlot = (
                <tr key={`${stat}-bbgm`}>
                  ...
                  <td style={width100}>
                    <BoxPlot color="var(--blue)" data={statsAll[stat]} scale={(scale as any)[stat]} />
                  </td>
                </tr>
              )
