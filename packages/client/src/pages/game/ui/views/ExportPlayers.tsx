...
const ExportPlayers = ({ challengeNoRatings, multipleSeasons, players, season }: View<'exportPlayers'>) => {
  ...
  const commonRows = (p: typeof players[number]) => {
    ...
    return [
      ...
      p.ratings.pos,
      p.age,
      <a href={helpers.leagueUrl(['roster', `${p.stats.abbrev}_${p.stats.tid}`, season])}>{p.stats.abbrev}</a>,
  ...
  return (
    <>
      ...
      <p>Here you can export any number of players to a JSON file which can be imported into another league.</p>
      {multipleSeasons ? <p>Players can be selected from any season using the menu above.</p> : null}
      <p>
        When you export a player, it includes all of his seasons. Then when you import, you will be able to select
        whichever season you want, including the ability to select multiple seasons from the same player.
      </p>

      <div className="row">
        <div className="col-12 col-lg-6">
          <div className="clearfix">
            <DataTable cols={cols} defaultSort={[0, 'asc']} name="ExportPlayers" pagination rows={rows} />
          </div>
        ...
        <div className="col-12 col-lg-6">
          {rows2.length === 0 ? (
            ...
            <>
              <div className="clearfix">
                <DataTable cols={cols2} defaultSort={[0, 'asc']} name="ExportPlayers2" pagination rows={rows2} />
              </div>
              ...
              {errorMessage ? (
                <div>
                  <div className="alert alert-danger d-inline-block mb-0">Error exporting players: {errorMessage}</div>
                </div>
              ) : null}
            </>
          )}
