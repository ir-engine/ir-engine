...
const PlayerInfo = ({
  ...
  return (
    <>
      ...
      {!challengeNoRatings || asset.retiredYear !== Infinity ? `${asset.ovr}/${asset.pot}, ` : null}
      {asset.age} yo)
    </>
  )
}
...
const FrivolitiesTrades = ({
  ...
  const cols = getCols(['#', 'Season', 'Team', 'Received', 'stat:ws', 'Team', 'Received', 'stat:ws', 'Links'])
  for (const i of [4, 7]) {
    if (cols[i].desc) {
  ...
  const rows = trades.map((trade) => {
    const teamCols = trade.teams.map((t) => {
      const searchValue = t.assets
        .map((asset) => {
          ...
          if (asset.type === 'realizedPick') {
            return `${asset.pos} ${asset.name} ${helpers.ordinal(asset.round)} round pick}`
          }
      ...
      return [
        <a href={helpers.leagueUrl(['roster', `${t.abbrev}_${t.tid}`, trade.season])}>
          {t.region} {t.name}
        </a>,
        {
          value: (
            <ul className="list-unstyled mb-0">
              {t.assets.map((asset, i) => {
                if (asset.type === 'player') {
                  return (
                    <li key={i}>
                      <PlayerInfo asset={asset} challengeNoRatings={challengeNoRatings} />
                    </li>
                  )
                }
    ...
    return {
      ...
      classNames: {
        'table-info': trade.teams[0].tid === userTid || trade.teams[1].tid === userTid
      }
    }
  })
