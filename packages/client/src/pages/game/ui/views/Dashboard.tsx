...
import { DIFFICULTY, SPORT_HAS_LEGENDS, SPORT_HAS_REAL_PLAYERS, WEBSITE_PLAY } from '../../common'
...
const randomOtherSport = 'basketball'
...
const DifficultyText = ({ children: difficulty }: { children: number | undefined }) => {
  if (difficulty === undefined) {
    return null
  }
...
const PlayButton = ({
  ...
  if (!disabled && !throbbing) {
    return (
      <a className="btn btn-lg btn-success" href={`/l/${lid}`} onClick={onClick}>
        Play
      </a>
    )
  }
  ...
  if (throbbing) {
    return <button className="btn btn-lg btn-success dashboard-play-loading">Play</button>
  }
...
const Dashboard = ({ leagues }: View<'dashboard'>) => {
  ...
  const cols = getCols(['', 'League', 'Team', 'Phase', '# Seasons', 'Difficulty', 'Created', 'Last Played', ''], {
    '': {
      width: '1%'
    }
  })
  ...
  const rows = leagues.map((league) => {
    const disabled = deletingLID !== undefined || loadingLID !== undefined || cloningLID !== undefined
    ...
    return {
      ...
      data: [
        ...
        </LeagueName>,
        {
          value: (
            <div className="d-flex align-items-center">
              <TeamLogoInline imgURL={league.imgURL} size={48} className="mr-2" />
              <div>
        ...
        },
        {
          ...
          sortValue: league.created && league.created.getTime ? league.created.getTime() : 0,
          value: <Ago date={league.created} />
        },
        {
          searchValue: league.lastPlayed ? ago(league.lastPlayed) : '',
          sortValue: league.lastPlayed && league.lastPlayed.getTime ? league.lastPlayed.getTime() : 0,
          value: <Ago date={league.lastPlayed} />
        },
        <Dropdown style={dropdownStyle} className={window.mobile ? 'dropdown-mobile' : undefined}>
          <Dropdown.Toggle
            as="span"
            bsPrefix="no-caret"
            ...
            title="Actions"
          >
            <span className="glyphicon glyphicon-option-vertical text-muted p-2" data-no-row-highlight="true" />
          </Dropdown.Toggle>
          {!disabled ? (
            <Dropdown.Menu>
              <Dropdown.Item href={`/new_league/${league.lid}`}>Import</Dropdown.Item>
              <Dropdown.Item
                onClick={async () => {
              ...
              <Dropdown.Item
                onClick={async () => {
                  try {
                    ...
                    const name = await toWorker('main', 'cloneLeague', league.lid)
  ...
  const otherSportsText = 'basketball'

  return (
    <>
      {location.host.indexOf('beta') === 0 ? (
        <p className="alert alert-warning d-inline-block" style={{ maxWidth: 840 }}>
          You are on the beta site. Sometimes new features are tested on the beta site, but most of the time it gets
          updated less frequently than <a href={`https://${WEBSITE_PLAY}/`}>the main site</a>. So unless you're testing
          some specific thing, you probably should be playing on <a href={`https://${WEBSITE_PLAY}/`}>the main site</a>.
        </p>
      ) : null}
      <div className={SPORT_HAS_REAL_PLAYERS ? 'mt-2 dashboard-top-wrapper' : 'mt-2'}>
        {SPORT_HAS_REAL_PLAYERS ? (
          <>
            <a href="/new_league/real" className="btn btn-primary dashboard-top-link dashboard-top-link-new mr-3 mb-3">
              New league
              <br />
            ...
            <a href="/new_league" className="btn btn-primary dashboard-top-link dashboard-top-link-new mr-sm-3 mb-3">
              New league
              <br />
          ...
          <a href="/new_league" className="btn btn-primary dashboard-top-link dashboard-top-link-new mr-3 mb-3">
            Create new
            <br />
            league
          </a>
