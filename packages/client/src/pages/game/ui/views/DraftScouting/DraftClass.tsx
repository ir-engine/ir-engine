...
import { DataTable, LeagueFileUpload, PlayerNameLabels } from '../../components'
  ...
  const rows = players.map((p) => {
    return {
      ...
      data: [
        ...
        {
          value: (
            <PlayerNameLabels pid={p.pid} season={season} skills={p.skills} watch={p.watch}>
              {p.nameAbbrev}
            </PlayerNameLabels>
          ),
          sortValue: p.name,
  ...
  return (
    <>
      ...
      <div className="btn-group mb-3">
        <button className="btn btn-light-bordered btn-xs" onClick={() => setShowImportForm((val) => !val)}>
          Import
        </button>
        <button
        ...
        <div>
          ...
          <LeagueFileUpload
            ...
            onDone={async (err, leagueFile) => {
              ...
              await toWorker('main', 'handleUploadedDraftClass', leagueFile, season)
      ...
      <DataTable cols={cols} defaultSort={[0, 'asc']} name={`DraftScouting:${offset}`} rows={rows} />
    </>
  )
}
