...
const HeadToHeadAll = ({ infoByTidByTid, season, teams, type, userTid }: View<'headToHeadAll'>) => {
  useTitleBar({
    title: 'Head-to-Head',
  ...
  const teamsFiltered = !hasInactiveTeams || showInactive ? teams : teams.filter((t) => !t.disabled)

  const cols = [
    ...getCols(['Team']),
    ...teamsFiltered.map((t): Col => {
      return {
        classNames: classNames('text-center', userTid === t.tid ? 'table-info' : undefined),
  ...
  const rows = teamsFiltered.map((t) => {
    ...
    return {
      ...
      data: [
        {
          classNames: classNames('align-middle', userTid === t.tid ? 'table-info' : undefined),
          value: <a href={helpers.leagueUrl(['head2head', `${t.abbrev}_${t.tid}`, season, type])}>{t.abbrev}</a>
        },
