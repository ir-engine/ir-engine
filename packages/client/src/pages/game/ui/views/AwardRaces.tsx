...
import { PlayerNameLabels, DataTable, RatingWithChange, StatWithChange, MoreLinks } from '../components'
import type { View } from '../../common/types'
import { PLAYER } from '../../common'

const AwardRaces = ({ awardCandidates, challengeNoRatings, season, teams, userTid }: View<'awardRaces'>) => {
  useTitleBar({
    title: 'Award Races',
    jumpTo: true,
    jumpToSeason: season,
  ...
  const globalCols = getCols(['#', 'Name', 'Pos', 'Age', 'Team', 'Record', 'Ovr'])
    ...
    <>
      ...
      <div className="row" style={{ marginTop: -14 }}>
        {awardCandidates.map(({ name, players, stats }) => {
          ...
          const cols = [...globalCols, ...getCols(stats.map((stat) => `stat:${stat}`))]
          ...
          const rows = players.map((p, j) => {
            ...
            const data = [
              ...
              p.age,
              <>
                <a href={helpers.leagueUrl(['roster', `${abbrev}_${tid}`, season])}>{abbrev}</a>
              </>,
              record
            ]
            ...
            if (mip) {
              data.push(pr && showRatings ? <RatingWithChange change={pr.dovr}>{pr.ovr}</RatingWithChange> : undefined)
              ...
              const statsRow = stats.map((stat) => (ps ? helpers.roundStat(ps[stat], stat) : undefined))
              data.push(...statsRow)
            }
          ...
          return (
            <div key={name} className={mip ? 'col-12 col-lg-9' : 'col-12 col-lg-6'} style={{ marginTop: 14 }}>
              <h2>{name}</h2>
              {rows.length > 0 ? (
                <DataTable
                  cols={cols}
