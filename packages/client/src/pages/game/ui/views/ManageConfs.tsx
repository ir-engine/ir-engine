...
const ManageTeams = ({ confs, divs, phase }: View<'manageConfs'>) => {
  ...
  const updateConfName = (cid: number) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const updateDivName = (did: number) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  if (phase < 0) {
    return <p>Wait until after your fantasy or expansion draft to edit conferences and divisions.</p>
  }

  return (
    <>
      <p>If you delete a conference, all divisions in that conference will be deleted too.</p>
      <p>
        If you delete a division, all teams belonging to that division will be assigned to some other division. You can
        reassign them on the <a href={helpers.leagueUrl(['manage_teams'])}>manage teams page</a>.
      </p>

      {phase >= PHASE.PLAYOFFS ? <p className="alert alert-warning d-inline-block">{nextSeasonWarning}</p> : null}
      ...
      <form onSubmit={handleSubmit}>
        <div className="row">
                  ...
                  <div className="card-body">
                    <div className="d-flex align-items-end mb-3">
                      <div className="form-group mb-0 flex-grow-1">
                        <label htmlFor={`conf-name-${conf.cid}`}>Conference Name</label>
                        <input
                      ...
                      <button className="btn btn-danger ml-3" onClick={deleteConf(conf.cid)} type="button">
                        Delete
                      </button>
                    </div>
                    {liveDivs
                      ...
                      .map((div) => (
                        <div key={div.did} className="d-flex align-items-end mb-3 ml-4">
                          <div className="form-group mb-0 flex-grow-1">
                            <label htmlFor={`div-name-${div.did}`}>Division Name</label>
                            <input
                              className="form-control"
                              id={`div-name-${div.did}`}
                              value={div.name}
                          ...
                          <button className="btn btn-danger ml-3" onClick={deleteDiv(div.did)} type="button">
                            Delete
                          </button>
                        </div>
                      ))}
                    <button className="btn btn-secondary ml-4" onClick={addDiv(conf.cid)} type="button">
                      Add Division
                    </button>
                  </div>
                </div>
          ...
          <div className="col-xl-3 col-lg-4 col-md-6 mb-3">
            <div className="card">
              <div className="card-body">
                <button className="btn btn-secondary" onClick={addConf} type="button">
                  Add Conference
                </button>
              </div>
            </div>
