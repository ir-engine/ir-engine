...
const FantasyDraft = ({ phase, teams, userTids }: View<'fantasyDraft'>) => {
  ...
  return (
    <>
      <p>
        In a "fantasy draft", all non-retired players are put into one big pool and teams take turns drafting players,
        similar to a fantasy basketball draft. At the beginning of the draft, the order of picks is randomized. During
        the draft, the order of picks snakes (reverses every other round). For example, the team that picks first in the
        first round picks last in the second round.
      </p>

      <p>
        To make things as fair as possible, all traded draft picks will be returned to their original owners after the
        fantasy draft.
      </p>

      <button className="btn btn-light-bordered mb-3" disabled={starting} onClick={randomize}>
        Randomize order
      </button>
      ...
      <SortableTable
        ...
        onChange={({ oldIndex, newIndex }) => {
          const newSortedTids = arrayMoveImmutable(sortedTids, oldIndex, newIndex)
          setSortedTids(newSortedTids)
        }}
        ...
        row={({ index, value }) => (
          <>
            <td>{index + 1}</td>
            <td>
              <a href={helpers.leagueUrl(['roster', `${value.abbrev}_${value.tid}`])}>
      ...
      <p>
        <button className="btn btn-large btn-success" disabled={starting} onClick={startDraft}>
          Start Fantasy Draft
        </button>
      </p>
