...
import { ResponsiveTableWrapper, MovOrDiff, TeamLogoInline } from '../components'
import useTitleBar from '../hooks/useTitleBar'
import { getCols, helpers } from '../util'
import useClickable from '../hooks/useClickable'
import type { TeamSeason, View } from '../../common/types'
import { TIEBREAKERS } from '../../common'

type StandingsTeam = View<'standings'>['rankingGroups']['league'][number][number]

const record = (seasonAttrs: StandingsTeam['seasonAttrs'], type: 'Home' | 'Away' | 'Div' | 'Conf') => {
...
export const TeamColumn = ({
  ...
  t: {
    tid: number
    seasonAttrs: Pick<TeamSeason, 'abbrev' | 'clinchedPlayoffs' | 'imgURL' | 'imgURLSmall' | 'region'> & {
  ...
  return (
    <td className="py-1">
      <div className="d-flex align-items-center">
        ...
        <TeamLogoInline imgURL={t.seasonAttrs.imgURL} imgURLSmall={t.seasonAttrs.imgURLSmall} className="mx-1" />
        <div>
          <a
          ...
          {t.seasonAttrs.clinchedPlayoffs ? ` ${t.seasonAttrs.clinchedPlayoffs}` : null}
        </div>
      </div>
    </td>
  )
...
const GroupStandingsRow = ({
  ...
  usePts,
  userTid
}: Pick<View<'standings'>, 'season' | 'showTiebreakers' | 'ties' | 'otl' | 'type' | 'usePts' | 'userTid'> & {
  separator: boolean
  t: StandingsTeam
}) => {
  ...
  return (
    ...
    >
      <TeamColumn rank={t.rank.playoffs > 0 ? t.rank.playoffs : null} season={season} t={t} includeName />
      ...
      <td>{showTiebreakers && t.tiebreaker ? TIEBREAKERS[t.tiebreaker] : null}</td>
    </tr>
  )
}
...
export const ColPtsOrGB = ({
  ...
  const style: CSSProperties | undefined = alignRight ? { textAlign: 'right' } : undefined
...
const GroupStandings = ({
  ...
  View<'standings'>,
  'pointsFormula' | 'season' | 'showTiebreakers' | 'ties' | 'otl' | 'usePts' | 'type' | 'userTid'
> & {
  name?: string
...
const SmallStandingsRow = ({
  ...
  return (
    <tr
      ...
      onClick={toggleClicked}
    >
      <TeamColumn rank={playoffsByConf ? t.rank.conf : t.rank.league} rankWidth={15} season={season} t={t} />
      <td className="text-right">
        {usePts ? Math.round(t.seasonAttrs.pts) : playoffsByConf ? t.gb.conf : t.gb.league}
      </td>
    </tr>
  )
}
...
const SmallStandings = ({
  ...
  userTid,
  usePts
}: Pick<View<'standings'>, 'maxPlayoffSeed' | 'playoffsByConf' | 'pointsFormula' | 'season' | 'usePts' | 'userTid'> & {
  teams: StandingsTeam[]
}) => {
  return (
    <table className="table table-striped table-bordered table-sm align-middle-all">
      <thead>
        <tr>
          ...
          <ColPtsOrGB alignRight pointsFormula={pointsFormula} usePts={usePts} />
        </tr>
      </thead>
      <tbody>
        {teams.map((t, i) => (
...
const Standings = ({
  ...
  const confHasMultipleDivs = confs.some((conf) => divs.filter((div) => div.cid === conf.cid).length > 1)
  const showSmallPlayoffStandings =
    type === 'div' && ((playoffsByConf && confHasMultipleDivs) || (!playoffsByConf && divs.length > 1))
  ...
  const footer = (
    <>
      ...
      <div className="float-md-right mt-3 mt-md-0" style={{ maxWidth: 400 }}>
        ...
        <p>
          The value shown in the "Tiebreaker" column above means "reason this team is ahead of all the tied teams below
          it, for this level of standings". You can switch between division/conference/league standings to view the
          tiebreaker results at different levels.
        </p>
      </div>
    </>
  )
