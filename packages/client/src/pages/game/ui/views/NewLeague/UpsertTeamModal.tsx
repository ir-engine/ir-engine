...
const UpsertTeamModal = ({
  ...
  const save = () => {
    ...
    const edited = {
      ...
      imgURLSmall: controlledTeam.imgURLSmall === '' ? undefined : controlledTeam.imgURLSmall
    }
  ...
  return (
    <Modal size="lg" show={t !== undefined} onHide={onCancel}>
      <Modal.Header closeButton>{t !== undefined && t.tid >= 0 ? 'Edit' : 'Add'} Team</Modal.Header>
      <Modal.Body>
        {controlledTeam ? (
          <form
            id="foo"
              ...
              <TeamForm
                ...
                handleInputChange={(field, event) => {
                  if (field.startsWith('colors')) {
                    ...
                    if (ind >= 0 && ind <= 2) {
                      const colors = [...controlledTeam.colors] as [string, string, string]
