...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case 'addConf': {
      const maxCID = state.confs.length > 0 ? Math.max(...state.confs.map((conf) => conf.cid)) : -1
      return {
        ...state,
        confs: [
          ...state.confs,
    ...
    case 'addDiv': {
      const maxDID = state.divs.length > 0 ? Math.max(...state.divs.map((div) => div.did)) : -1
      return {
        ...state,
        divs: [
          ...state.divs,
    ...
    case 'moveDiv': {
      ...
      if (
        ...
        } else if (confIndex < state.confs.length - 1 && action.direction === 1) {
          newCID = state.confs[confIndex + 1].cid
        }
      }
    ...
    case 'deleteTeam':
      return {
        ...state,
        teams: makeTIDsSequential(state.teams.filter((t) => t.tid !== action.tid))
      }
...
const EditButton = ({ onClick }: { onClick: () => void }) => {
  return (
    <button className="ml-2 btn btn-link p-0 border-0 text-reset" onClick={onClick} title="Edit" type="button">
...
const DeleteButton = ({ onClick }: { onClick: () => void }) => {
  return (
    <button className="ml-2 btn btn-link text-danger p-0 border-0" onClick={onClick} title="Delete" type="button">
...
const CardHeader = ({
  ...
  return (
    <div className={classNames('card-header', renaming ? 'p-1' : undefined)} style={{ height: 44 }}>
      {renaming ? (
        <form
          className="d-flex"
          onSubmit={(event) => {
...
const Division = ({
  ...
  return (
    <div className="card mt-3">
          ...
          if (teams.length === 0) {
            ...
            const siblings = divs
              ...
              .map((div2) => {
                ...
                return {
                  key: div2.did,
                  text: `Move teams to "${div2.name}" division (${conf ? conf.name : 'unknown conference'})`
                }
              })
        ...
        {teams.map((t) => (
          <li key={t.tid} className="list-group-item d-flex">
            <div className="mr-auto">
              {t.region} {t.name}{' '}
              <span className={abbrevsUsedMultipleTimes.includes(t.abbrev) ? 'text-danger' : undefined}>
                ({t.abbrev})
              </span>
            </div>
            <EditButton
      ...
      <AddTeam addTeam={addTeam} did={div.did} availableBuiltInTeams={availableBuiltInTeams} />
    </div>
  )
}
...
const CustomizeTeams = ({
  ...
  const randomize = (weightByPopulation: boolean) => async () => {
    ...
    try {
      ...
      const numTeamsPerDiv = myDivs.map((div) => myTeams.filter((t) => t.did === div.did).length)

      const response = await toWorker('main', 'getRandomTeams', myDivs, numTeamsPerDiv, weightByPopulation)
  ...
  return (
    <>
      ...
      <StickyBottomButtons>
        <Dropdown>
          <Dropdown.Toggle variant="danger" id="customize-teams-reset" disabled={randomizing}>
            {randomizing ? processingSpinner : 'Reset'}
          </Dropdown.Toggle>
          <Dropdown.Menu>
            <Dropdown.Item onClick={resetDefault}>Default</Dropdown.Item>
            <Dropdown.Item onClick={randomize(false)}>Random built-in teams</Dropdown.Item>
            <Dropdown.Item onClick={randomize(true)}>Random built-in teams (population weighted)</Dropdown.Item>
          </Dropdown.Menu>
        </Dropdown>
        <div className="ml-2 pt-2">
          <HelpPopover title="Reset">
            <p>
              <b>Default</b>: Resets conferences, divisions, and teams to their default values.
            </p>
            <p>
              <b>Random built-in teams</b>: This replaces any teams you currently have with random built-in teams. Those
              teams are grouped into divisions based on their geographic location. Then, if your division names are the
              same as the default division names and each division has the same number of teams, it tries to assign each
              group to a division name that makes sense.
            </p>
            <p>
              <b>Random built-in teams (population weighted)</b>: Same as above, except larger cities are more likely to
              be selected, so the set of teams may feel a bit more realistic.
            </p>
          </HelpPopover>
        </div>
        <form
        ...
        >
          <button className="btn btn-secondary" type="button" onClick={onCancel} disabled={randomizing}>
            Cancel
          </button>
          <button className="btn btn-primary mr-2" type="submit" disabled={randomizing}>
            Save Teams
          </button>
        </form>
      </StickyBottomButtons>
