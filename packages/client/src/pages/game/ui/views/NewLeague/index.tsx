...
import { ActionButton, LeagueFileUpload, NextPrevButtons, PopText } from '../../components'
import useTitleBar from '../../hooks/useTitleBar'
import { confirm, helpers, logEvent, realtimeUpdate, toWorker, safeLocalStorage } from '../../util'
import type { View, RealTeamInfo, GetLeagueOptions, Div, Conf } from '../../../common/types'
...
const teamsDefault: NewLeagueTeam[] = helpers.addPopRank(helpers.getTeamsDefault())
...
const initKeptKeys = ({
  ...
  if (newAllKeys) {
    ...
    allKeys = leagueFile ? Object.keys(leagueFile).filter((key) => key !== 'version') : []
  }
...
const getNewTid = (prevTeamRegionName: string, newTeams: NewLeagueTeam[]) => {
  ...
  const closestNewTeam = newTeamsSorted.find((t) => prevTeamRegionName <= `${t.region} ${t.name}`)
...
const getSettingsFromGameAttributes = (
  ...
  if (gameAttributes) {
    for (const key of helpers.keys(newSettings)) {
      if (key === 'noStartingInjuries' || key === 'randomization' || key === 'realStats') {
        continue
      }
...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case 'newLeagueFile': {
      ...
      const newSettings = getSettingsFromGameAttributes(action.leagueFile.gameAttributes, action.defaultSettings)

      if (action.leagueFile.gameAttributes) {
        if (action.leagueFile.gameAttributes.confs) {
          confs = unwrapGameAttribute(action.leagueFile.gameAttributes, 'confs')
        }
    ...
    case 'newLeagueInfo': {
      ...
      const newSettings = getSettingsFromGameAttributes(action.gameAttributes, action.defaultSettings)
...
const NewLeague = (props: View<'newLeague'>) => {
  ...
  const [startingSeason, setStartingSeason] = useState(String(new Date().getFullYear()))
  const [currentScreen, setCurrentScreen] = useState<'default' | 'teams' | 'settings'>('default')

  const [state, dispatch] = useReducer(reducer, props, (props: View<'newLeague'>): State => {
  ...
  const createLeague = async (settingsOverride?: State['settings']) => {
    ...
    const actualShuffleRosters = state.keptKeys.includes('players') ? settings.randomization === 'shuffle' : false

    const actualStartingSeason = state.customize === 'default' ? startingSeason : undefined
    ...
    try {
      ...
      if (state.customize === 'real') {
        getLeagueOptions = {
          ...
          season: state.season,
          phase: state.phase,
          randomDebuts: settings.randomization === 'debuts' || settings.randomization === 'debutsForever',
    ...
    (err, newLeagueFile) => {
      ...
      if (newLeagueFile.gameAttributes) {
        if (Array.isArray(newLeagueFile.gameAttributes)) {
          newLeagueFile.gameAttributes = gameAttributesArrayToObject(newLeagueFile.gameAttributes)
        }
      }
      ...
      dispatch({
        ...
        teams: applyRealTeamInfos(newTeams, props.realTeamInfo, newLeagueFile.startingSeason),
  ...
  const handleNewLeagueInfo = (leagueInfo: LeagueInfo) => {
    ...
    dispatch({
      ...
      teams: applyRealTeamInfos(newTeams, props.realTeamInfo, leagueInfo.startingSeason),
  ...
  const displayedTeams = !keptKeysIsAvailable || state.keptKeys.includes('teams') ? state.teams : teamsDefault

  const createLeagueText = props.lid !== undefined ? 'Import League' : 'Create League'
  ...
  const disableWhileLoadingLeagueFile =
    ((state.customize === 'custom-rosters' || state.customize === 'custom-url') &&
      (state.leagueFile === null || state.loadingLeagueFile)) ||
    ((state.customize === 'real' || state.customize === 'legends') && state.pendingInitialLeagueInfo)
  ...
  if (state.phase > PHASE.PLAYOFFS && bannedExpansionSeasons.includes(state.season)) {
  ...
  if (state.season === 2022 && state.phase > PHASE.PRESEASON) {
    invalidSeasonPhaseMessage = "Sorry, I'm not allowed to share the results of the 2022 season yet."
  }
      ...
      {subPage ? (
        ...
        >
          {props.lid !== undefined ? (
            <>
              <p>
                Here you can create a new league that overwrites one of your existing leagues. This is no different than
                deleting the existing league and creating a new one, it's just a little more convenient for people who
                do that a lot.
              </p>
              <p>
                If you just want to create a new league, <a href="/new_league">click here</a>.
              </p>
            </>
          ) : null}
          <div className="row">
            <div className="col-sm-6">
                ...
                <>
                  <div className="form-group">
                    <LeagueMenu
                      ...
                      quickValues={['1956', '1968', '1984', '1996', '2003', '2022']}
                      value2={state.phase}
                      values2={phases}
                      onNewValue2={(phase) => {
                        dispatch({
                    ...
                    />
                    {invalidSeasonPhaseMessage ? (
                      <div className="text-danger mt-1">{invalidSeasonPhaseMessage}</div>
                    ) : (
                      <div className="text-muted mt-1">
                        {state.season} in BBGM is the {state.season - 1}-{String(state.season).slice(2)} season.
                      </div>
                    )}
                  </div>
                </>
                ...
                <div className="input-group mb-1">
                    ...
                    {sortedDisplayedTeams.map((t) => {
                      return (
                        <option key={t.tid} value={t.tid}>
                          {showLoadingIndicator ? 'Loading...' : `${t.region} ${t.name}`}
                        </option>
                      )
                    })}
                  </select>
                  ...
                  <div className="input-group-append">
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        const t = displayedTeams[Math.floor(Math.random() * displayedTeams.length)]
              ...
              <div className="form-group">
                ...
                >
                  ...
                  {!Object.values(DIFFICULTY).includes(state.difficulty) ? (
                    <option value={state.difficulty}>Custom (from league file)</option>
                  ) : null}
                </select>
              ...
              <div className="text-center mt-3">
                <ActionButton
                  ...
                  size="lg"
                  type="submit"
                  disabled={state.creating || disableWhileLoadingLeagueFile || !!invalidSeasonPhaseMessage}
            ...
            {props.type === 'custom' || props.type === 'real' || props.type === 'legends' ? (
              <div
                className={classNames('col-sm-6 order-first order-sm-last mb-3 mb-sm-0', {
                    ...
                    <>
                      <ul className="list-group list-group-flush">
                        <li className="list-group-item bg-light">
                          ...
                          <p className="mb-0">
                            Players, teams, rosters, and contracts are generated from real data. Draft classes are
                            included up to today.
                          </p>
                        </li>
                        <li className="list-group-item bg-light">
                          <h3>Watch your league evolve over time</h3>
                          <p className="mb-0">
                            There were only 11 teams in {MIN_SEASON}, playing a very different brand of basketball than
                            today. Live through expansion drafts, league rule changes, team relocations, economic
                            growth, and changes in style of play.
                          </p>
                        </li>
                        <li className="list-group-item bg-light">
                          <h3>Every league is different</h3>
                          <p className="mb-0">
                            Draft prospects always start the same, but they have different career arcs in every league.
                            See busts meet their potentials, see injury-shortened careers play out in full, and see new
                            combinations of players lead to dynasties.
                          </p>
                        </li>
                      </ul>
                    </>
                    ...
                    <>
                      <ul className="list-group list-group-flush">
                        <li className="list-group-item bg-light">
                          <h3>Legends mode</h3>
                          <p>
                            Each team is filled with the best players from that franchise's history. Create a league
                            with players from only one decade, or the greatest players of all time.
                          </p>
                          <p className="mb-0">
                            <a href="https://bbgm.com/blog/2020/05/legends-leagues/">More details</a>
                          </p>
                        </li>
                      </ul>
                    </>
                      ...
                      <div className="form-group">
                        <select
                          ...
                          onChange={(event) => {
                            ...
                            if (newCustomize !== 'real' && newCustomize !== 'legends') {
                              dispatch({ type: 'clearLeagueFile' })
                            }
                          }}
                        ...
                        >
                          <option value="default">
                            {SPORT_HAS_REAL_PLAYERS ? 'Random players and teams' : 'Default'}
                          </option>
                          {SPORT_HAS_REAL_PLAYERS ? <option value="real">Real players and teams</option> : null}
                          {SPORT_HAS_LEGENDS ? <option value="legends">Legends</option> : null}
                          <option value="custom-rosters">Upload league file</option>
                          <option value="custom-url">Enter league file URL</option>
                        </select>
                        {state.customize === 'custom-rosters' || state.customize === 'custom-url' ? (
                          <p className="mt-3">
                            League files can contain teams, players, settings, and other data. You can create a league
                            file by going to Tools &gt; Export within a league, or by{' '}
                            <a href={`https://${WEBSITE_ROOT}/manual/customization/`}>creating a custom league file</a>.
                          </p>
                        ) : null}
                      </div>
                      {state.customize === 'custom-rosters' || state.customize === 'custom-url' ? (
