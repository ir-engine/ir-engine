...
const MultiTeamMode = ({ phase, teams, userTid, userTids }: View<'multiTeamMode'>) => {
  ...
  if (phase === PHASE.RESIGN_PLAYERS) {
    return (
      <div>
        <h2>Error</h2>
        <p>
          Changing your teams while re-signing players currently breaks things. Please play until free agency and then
  ...
  if (userTids.length === 1) {
    statusText = (
      <span>
        <b className="text-danger">Multi team mode disabled!</b> To enable it, select all the teams you want to control
        below.
      </span>
    )
    ...
    statusText = (
      <span>
        <b className="text-success">Multi team mode enabled!</b> To disable it, unselect all but one team or click the
        button below.
      </span>
    )
  } else {
  ...
  return (
    <>
      <p>
        Here you can switch from controlling one team to controlling multiple teams. Why would you want to do this?
      ...
      <ul>
        ...
        <li>
          ...
          - a bunch of people coordinate on Discord/Reddit/etc to run a whole league of teams, and then one person
          manually controls all the teams in the game
        </li>
      </ul>

      <p>
        {statusText} Use shift+click to select adjacent teams, or ctrl+click (command+click on Mac) to select individual
        teams.
      </p>
      ...
      <p>These actions will apply to all teams controlled by multi team mode.</p>

      <div className="btn-group">
        <button className="btn btn-light-bordered" onClick={() => handleAutoSort(userTids)}>
          Auto sort rosters
        </button>
        <button className="btn btn-light-bordered" onClick={() => handleResetPT(userTids)}>
          Reset playing time
        </button>
      </div>
    </>
