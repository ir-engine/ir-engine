...
const redirectNegotiationOrRoster = async (cancelled: boolean) => {
  ...
  if (count > 0) {
    ...
    // After signing player, go back to free agents screen, cause you probably need more
...
const sign = async (pid: number, amount: number, exp: number) => {
  const errorMsg = await toWorker('main', 'acceptContractNegotiation', pid, Math.round(amount * 1000), exp)
...
const Negotiation = ({
  ...
  if (resigning && !hardCap) {
    message = (
      <p>
        You are allowed to go over the salary cap to make this deal because you are re-signing{' '}
        <a href={helpers.leagueUrl(['player', player.pid])}>{player.name}</a> to a contract extension.{' '}
        <b>
          If you do not come to an agreement here, <a href={helpers.leagueUrl(['player', player.pid])}>{player.name}</a>{' '}
          will become a free agent.
        </b>{' '}
        He will then be able to sign with any team, and you won't be able to go over the salary cap to sign him.
      </p>
    )
  } else {
    const extra = !hardCap ? (
      <>
        {' '}
        because <a href={helpers.leagueUrl(['player', player.pid])}>{player.name}</a> is a free agent
      </>
    ) : null

    message = (
      <p>
        You are not allowed to go over the salary cap to make this deal (unless it is for a minimum contract){extra}.
      </p>
    )
  }
  ...
  return (
    <>
      ...
      <p className="mt-2">
        ...
        {!challengeNoRatings ? `; Overall: ${player.ratings.ovr}; Potential: ${player.ratings.pot}` : null}
      </p>
          ...
          <div className="list-group">
            {contractOptions.map((contract, i) => {
              return (
                <div
                  key={i}
                  className={classNames('d-flex align-items-center list-group-item', {
                    'list-group-item-success': contract.smallestAmount
                  })}
                >
                  <div className="flex-grow-1">
                    {helpers.formatCurrency(contract.amount, 'M')} per year
                    <span className="d-none d-sm-inline">, through {contract.exp}</span> ({contract.years}{' '}
                    {contract.years === 1 ? 'season' : 'seasons'})
                  </div>

                  <button className="btn btn-success" onClick={() => sign(player.pid, contract.amount, contract.exp)}>
                    Sign
      ...
      <button className="btn btn-danger mt-3" onClick={() => cancel(player.pid)}>
