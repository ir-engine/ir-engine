...
const formatMaybeInteger = (x: number) => (Number.isInteger(x) ? String(x) : x.toFixed(1))
...
const LeagueStats = ({
  ...
  const basicColNames = tid < 0 ? ['Season', '# Teams', 'stat:gp', 'W', 'L'] : ['Season', 'stat:gp', 'W', 'L']
  if (otl) {
  ...
  const rows = seasons.map((s) => {
    ...
    const data: { [key: string]: ReactNode } = {
      season:
        abbrev === 'all' ? (
          <a href={helpers.leagueUrl(['team_stats', s.season, teamOpponent, playoffs])}>{s.season}</a>
        ) : (
          <a href={helpers.leagueUrl(['roster', `${abbrev}_${tid}`, s.season])}>{s.season}</a>
        )
    }
    ...
    data.avgAge = Number.isNaN(s.stats.avgAge) ? null : s.stats.avgAge.toFixed(1)
