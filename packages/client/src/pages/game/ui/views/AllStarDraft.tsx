...
const PlayersTable = ({
  ...
  const colNames = ['Name', 'Team', 'Age', 'Ovr', ...stats.map((stat) => `stat:${stat}`), '#AS']
  if (name !== 'Remaining') {
    colNames.unshift('#')
  }
  if (showDraftCol) {
  ...
  const rows = playersAugmented
    ...
    .map((p, i) => {
      const data = [
        ...
        <a href={helpers.leagueUrl(['roster', `${p.abbrev}_${p.tid}`])}>{p.abbrev}</a>,
        p.age,
  ...
  return <DataTable cols={cols} defaultSort={[0, 'asc']} name={`AllStarDraft:${name}`} rows={rows} />
}
...
const AllStars = ({
  ...
  const draftType = !spectator && teams.some((t) => userTids.includes(t[0].tid)) ? 'user' : 'auto'
  ...
  const startDraft = useCallback(async () => {
    ...
    if (!userTids.includes(teams[0][0].tid)) {
      const { finalized: finalized2, pid } = await toWorker('main', 'allStarDraftOne')
      reveal(pid)
      setActuallyFinalized(finalized2)
    }
  }, [draftType, reveal, teams, userTids])

  const userDraftingBothTeams = userTids.includes(teams[0][0].tid) && userTids.includes(teams[1][0].tid)
  const onDraft = useCallback(
    async (pid: number) => {
      ...
      if (!userDraftingBothTeams) {
        const { finalized: finalized3, pid: pid2 } = await toWorker('main', 'allStarDraftOne')
        if (pid2 !== undefined) {
          await wait(1000)
          reveal(pid2)
        }
  ...
  const usersTurn =
    started &&
    draftType === 'user' &&
    ((teams[0].length + revealed0.length > teams[1].length + revealed1.length && userTids.includes(teams[1][0].tid)) ||
      (teams[0].length + revealed0.length <= teams[1].length + revealed1.length && userTids.includes(teams[0][0].tid)))

  const numPlayers = teams[0].length + teams[1].length + remaining.filter((p) => p.injury.gamesRemaining <= 0).length

  return (
    <>
      <p>
        The top {numPlayers} players in the league play in the All-Star Game. If any of them are injured, they are still
        All-Stars, but an additional All-Star will be selected as a replacement to play in the game.
      </p>
      <p>
        The players are split into two teams, captained by the top two players. The teams are filled by a draft. Just
        for fun, if a captain is on your team, you get to draft for him! Otherwise, the captains get to choose.
      </p>
      {actuallyFinalized ? (
