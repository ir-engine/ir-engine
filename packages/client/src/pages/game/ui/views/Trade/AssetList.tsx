...
const genPickRows = (picks: Picks, handleToggle: HandleToggle, userOrOther: UserOrOther) => {
...
const AssetList = ({
  ...
  const playerCols = getCols(
    ['', 'X', 'Name', 'Pos', 'Age', 'Ovr', 'Pot', 'Contract', 'Exp', ...stats.map((stat) => `stat:${stat}`)],
    {
      '': {
        sortSequence: [],
        noSearch: true
  ...
  const playerRows = genPlayerRows(roster, handleToggle, userOrOther, stats, challengeNoRatings)
  const pickRows = genPickRows(picks, handleToggle, userOrOther)

  const userOrOtherKey = `${userOrOther[0].toUpperCase()}${userOrOther.slice(1)}`

  return (
    <div className="row">
      <div className="col-xl-9">
        <Dropdown className="d-inline-block">
          <Dropdown.Toggle variant="secondary" id={`trade-players-bulk-${userOrOtherKey}`} className="btn-sm">
            Bulk exclude
          </Dropdown.Toggle>
          <Dropdown.Menu>
            <Dropdown.Item
      ...
      <div className="col-xl-3">
        <Dropdown className="d-inline-block">
          <Dropdown.Toggle variant="secondary" id={`trade-picks-bulk-${userOrOtherKey}`} className="btn-sm mb-2">
            Bulk exclude
          </Dropdown.Toggle>
          <Dropdown.Menu>
            <Dropdown.Item
