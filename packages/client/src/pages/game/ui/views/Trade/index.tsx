...
const Trade = (props: View<'trade'>) => {
  ...
  const handleBulk = async (
    ...
    if (type === 'clear') {
      ...
      ids[key] = Array.from(new Set([...ids[key], ...picks.map((dp) => dp.dpid)]))
    }
  ...
  const handleClickPropose = async () => {
    const [accepted, message] = await toWorker('main', 'proposeTrade', false)
    setState((prevState) => ({
      ...prevState,
      accepted,
      message,
  ...
  const updateSummaryHeight = useCallback(() => {
    if (summaryControls.current && summaryText.current) {
      ...
      if (window.matchMedia('(min-width:768px)').matches) {
        ...
        const newHeight = window.innerHeight - 60 - 24 - summaryControls.current.clientHeight
  ...
  const otherTeamName = otherTeam ? `${otherTeam.region} ${otherTeam.name}` : 'Other team'
    ...
    <>
      <div className="row">
        <div className="col-md-9">
          {showResigningMsg ? (
            <p>
              You can't trade players whose contracts expired this season, but their old contracts still count against
              team salary caps until they are either re-signed or become free agents.
            </p>
          ) : null}

          <p>If a player has been signed within the past 14 days, he is not allowed to be traded.</p>
        ...
        <div className="col-md-3">
          <div className="trade-affix">
            <Summary ref={summaryText} salaryCap={salaryCap} summary={summary} />
            ...
            <div ref={summaryControls}>
              ...
              {state.message ? (
                <div
                  className={classNames('alert mb-0', state.accepted ? 'alert-success' : 'alert-info', {
              ...
              >
                {!noTradingAllowed ? (
                  ...
                  <p className="alert alert-danger">You're not allowed to make trades in spectator mode.</p>
                ) : otherTeamsWantToHire ? (
                  <p className="alert alert-danger">
                    You're not allowed to make trades while you have{' '}
                    <a href={helpers.leagueUrl(['new_team'])}>open job offers from other teams</a>.
                  </p>
                ) : (
                  <p className="alert alert-danger">
                    You're not allowed to make trades{' '}
                    {phase === PHASE.AFTER_TRADE_DEADLINE ? 'after the trade deadline' : 'now'}.
                  </p>
                )}
              </div>
            </div>
