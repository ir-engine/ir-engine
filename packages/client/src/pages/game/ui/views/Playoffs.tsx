...
const Playoffs = ({
  ...
  for (let i = 0; i < series.length; i++) {
    if (series[i].length === 0) {
      ...
      numGamesPlayoffSeriesReflected[numGamesPlayoffSeriesReflected.length - 1 - i] = undefined
    }
  }
  ...
  if (numRounds === 0) {
    return finalMatchups ? (
      ...
      <p>
        Your league is configured to not have any playoff series. The league champion will be the 1st place team in the
        regular season. If you would like to change this, go to{' '}
        <a href={helpers.leagueUrl(['settings'])}>league settings</a> and change the "# Playoff Games" setting.
      </p>
    )
  }
    ...
    <div style={{ maxWidth }}>
          ...
          <tfoot>
            <tr className="text-center text-muted">
              {numGamesPlayoffSeriesReflected.map((numGames, i) => {
                ...
                return (
                  <td key={i}>
                    <div style={numRoundsWithPlayIn === 1.5 ? tdStyle : undefined}>{text}</div>
                  </td>
                )
              })}
            </tr>
      ...
      {playIns ? (
        <>
          ...
          <p className="mb-2">
            The winner{playInPlural} of the {playIns[0][0].home.seed}/{playIns[0][0].away.seed} game{playInPlural} make
            {playInPluralAlt} the playoffs. Then the loser{playInPlural} play{playInPluralAlt} the winner{playInPlural}{' '}
            of the {playIns[0][1].home.seed}/{playIns[0][1].away.seed} game{playInPlural} for the final playoffs spot
            {playInPlural}.
          </p>
              ...
              <ResponsiveTableWrapper key={i}>
                <table className="table-sm" style={width100}>
                  <tbody>
                    ...
                    <tr>
                      <td style={tdStyle}>
                        <PlayoffMatchup numGamesToWinSeries={1} season={season} series={playIn[1]} userTid={userTid} />
                      </td>
                    </tr>
                  </tbody>
                </table>
