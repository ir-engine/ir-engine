...
const SeasonLink = ({ pid, season }: { pid: number; season: number }) => {
  return <a href={helpers.leagueUrl(['player_game_log', pid, season])}>{season}</a>
}
...
const StatsTable = ({
  ...
  const cols = getCols([
    ...
    'Team',
    'Age',
    ...stats.map((stat) => `stat:${stat.endsWith('Max') ? stat.replace('Max', '') : stat}`)
  ])
  ...
  return (
    <>
      ...
      <DataTable
        ...
        cols={cols}
        defaultSort={[0, 'asc']}
        footer={['Career', null, null, ...stats.map((stat) => formatStatGameHigh(careerStats, stat))]}
        hideAllControls
        name={`Player:${name}`}
            ...
            data: [
              {
                ...
                value: (
                  <>
                    ...
                    <SeasonIcons season={ps.season} awards={p.awards} playoffs={playoffs} />
                  </>
                )
              },
              <a href={helpers.leagueUrl(['roster', `${ps.abbrev}_${ps.tid}`, ps.season])}>{ps.abbrev}</a>,
              ps.age,
...
const Player2 = ({
  ...
  useTitleBar({
    ...
    dropdownCustomURL: (fields) => {
      ...
      const parts =
        fields.playerProfile === 'gameLog' ? ['player_game_log', player.pid, gameLogSeason] : ['player', player.pid]
  ...
  return (
    <>
      ...
      {statTables.map(({ name, onlyShowIf, stats, superCols }) => (
        <StatsTable key={name} name={name} onlyShowIf={onlyShowIf} stats={stats} superCols={superCols} p={player} />
      ))}
              ...
              data: [
                {
                  ...
                  value: (
                    <>
                      ...
                      {r.injuryIndex !== undefined && player.injuries[r.injuryIndex] ? (
                        <span className="badge badge-danger badge-injury" title={player.injuries[r.injuryIndex].type}>
                          +
                        </span>
                      ) : null}
                    </>
                ...
                r.abbrev ? (
                  <a href={helpers.leagueUrl(['roster', `${r.abbrev}_${r.tid}`, r.season])}>{r.abbrev}</a>
                ) : null,
                r.age,
                r.pos,
                showRatings ? r.ovr : null,
                showRatings ? r.pot : null,
                ...ratings.map((rating) => (showRatings ? (r as any)[rating] : null)),
