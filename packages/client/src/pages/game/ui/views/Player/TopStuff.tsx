...
const Relatives = ({ pid, relatives }: { pid: number; relatives: Player['relatives'] }) => {
      ...
      {relatives.map((rel) => {
        return (
          <Fragment key={rel.pid}>
            {helpers.upperCaseFirstLetter(rel.type)}: <a href={helpers.leagueUrl(['player', rel.pid])}>{rel.name}</a>
            <br />
          </Fragment>
        )
      })}
      <a href={helpers.leagueUrl(['frivolities', 'relatives', pid])}>(Family details)</a>
      <br />
    </>
  )
}
...
const StatsSummary = ({
  ...
  if (season !== undefined) {
    ...
    const playerStats = p.stats.filter((ps) => !ps.playoffs && ps.season === season)
    ps = playerStats.at(-1)
  }
  ...
  if (!ps) {
    if (p.tid === PLAYER.RETIRED) {
      ...
      const playerStats = p.stats.filter(
        (ps) =>
          !ps.playoffs &&
          (ps.season === currentSeason || (ps.season === currentSeason - 1 && phase === PHASE.PRESEASON))
      )
      ps = playerStats.at(-1)
    }
  }
    ...
    <div className="player-stats-summary">
      <table className="table table-sm table-condensed table-nonfluid text-center mt-3 mb-0">
        ...
        {ps ? (
          <tbody>
            <tr>
              <th className="table-separator-right text-left" title={showPeakSeason ? String(ps.season) : undefined}>
...
> & {
  ...
  if (player.draft.round > 0) {
    draftInfo = (
      <>
        Draft: <a href={helpers.leagueUrl(['draft_history', player.draft.year])}>{player.draft.year}</a> - Round{' '}
        {player.draft.round} (Pick {player.draft.pick}) by{' '}
        <a href={helpers.leagueUrl(['draft_team_history', `${player.draft.abbrev}_${player.draft.tid}`])}>
          {player.draft.abbrev}
        </a>
        <br />
      </>
    ...
    draftInfo = (
      <>
        Undrafted: <a href={helpers.leagueUrl(['draft_history', player.draft.year])}>{player.draft.year}</a>
        <br />
      </>
    )
  }
  ...
  if (season !== undefined || showContract) {
    ...
    contractInfo = (
      <>
        {freeAgent ? 'Asking for' : 'Contract'}: {amount !== undefined ? helpers.formatCurrency(amount, 'M') : '???'}/yr
      ...
      <div className="d-flex align-items-center">
        {injured ? (
          <span
            ...
            title={`${player.injury.type} (out ${player.injury.gamesRemaining} more ${
        ...
        <SkillsBlock className={injured ? undefined : 'skills-alone'} skills={skills} />
        <WatchBlock className="ml-2" pid={player.pid} watch={player.watch} />
        {player.tid === PLAYER.FREE_AGENT || player.tid === PLAYER.UNDRAFTED || player.tid >= PLAYER.FREE_AGENT ? (
          <Mood
            className="ml-2"
            defaultType={player.tid === PLAYER.FREE_AGENT || player.tid === PLAYER.UNDRAFTED ? 'user' : 'current'}
            p={player}
          />
        ) : null}
      </div>
  ...
  const college = player.college && player.college !== '' ? player.college : 'None'
    ...
    <>
      ...
      {!spectator && freeAgent ? (
        <button
          ...
          title={willingToSign ? undefined : `${player.name} refuses to negotiate with you`}
        >
          Negotiate Contract
        </button>
      ) : null}
      ...
      <div className="d-sm-flex">
        <div className="player-bio">
          <div className="d-flex">
            ...
            >
              <PlayerPicture face={player.face} imgURL={player.imgURL} colors={teamColors} jersey={teamJersey} />
            </div>
            <div>
              <strong>
                {player.ratings.at(-1).pos}, {teamURL ? <a href={teamURL}>{teamName}</a> : teamName}
                {player.jerseyNumber ? (
                  <>
                    ,{' '}
                    <a href={helpers.leagueUrl(['frivolities', 'most', 'jersey_number', player.jerseyNumber])}>
                      #{player.jerseyNumber}
                    </a>
                  </>
                ) : null}
              ...
              <a
                href={helpers.leagueUrl([
                  ...
                  'most',
                  'country',
                  window.encodeURIComponent(helpers.getCountry(player.born.loc))
                ])}
              >
                {player.born.loc}
              ...
              {draftInfo}
              College:{' '}
              <a href={helpers.leagueUrl(['frivolities', 'most', 'college', window.encodeURIComponent(college)])}>
                {college}
              </a>
              <br />
              Experience:{' '}
              {player.experience === 0 ? 'none' : `${player.experience} year${player.experience > 1 ? 's' : ''}`}
              <br />
              {contractInfo}
              {statusInfo}
            </div>
        ...
        <div className="mt-3 mt-sm-0 text-nowrap">
          {showRatingsOverview ? <RatingsOverview ratings={player.ratings} season={season} /> : null}
