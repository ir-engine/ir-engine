...
const RatingsOverview = ({ ratings, season }: { ratings: any[]; season?: number }) => {
  ...
  if (season === undefined) {
    ...
    currentSeason = ratings.find((row) => row.season === season) ?? ratings.at(-1)
  }
  ...
  return (
    <div className="ratings-overview">
      <div className="d-flex justify-content-between">
        <h2 className="mr-3">
          Overall: <RatingWithChange change={currentSeason.ovr - lastSeason.ovr}>{currentSeason.ovr}</RatingWithChange>
        </h2>
        <h2>
          Potential:{' '}
          <RatingWithChange change={currentSeason.pot - lastSeason.pot}>{currentSeason.pot}</RatingWithChange>
        </h2>
      </div>
      <div className="d-flex justify-content-between">
                    ...
                    {categories.map(({ label, rating }, j) => (
                      <tr key={j}>
                        ...
                        <td className="p-0 pl-1">
                          <RatingWithChange change={(currentSeason as any)[rating] - (lastSeason as any)[rating]}>
