...
export const getValue = (obj: any, key: View<'most'>['extraCols'][number]['key']) => {
  return typeof key === 'string' ? obj[key] : key.length === 2 ? obj[key[0]][key[1]] : obj[key[0]][key[1]][key[2]]
}
...
const Most = ({
  ...
  const rows = players.map((p) => {
    ...
    const draftPick = p.draft.round > 0 ? `${p.draft.round}-${p.draft.pick}` : ''
    ...
    return {
      ...
      data: [
        p.rank,
        {
          value: (
            <div className="d-flex">
              <PlayerNameLabels disableWatchToggle jerseyNumber={p.jerseyNumber} pid={p.pid} watch={p.watch}>
                {p.name}
              </PlayerNameLabels>
        ...
        ...extraCols.map((x) => {
          ...
          if (x.colName === 'Team') {
            return <a href={helpers.leagueUrl(['team_history', `${value.abbrev}_${value.tid}`])}>{value.abbrev}</a>
          }
          return value
        }),
        p.ratings.at(-1).pos,
  ...
  return (
    <>
      ...
      {type === 'goat' ? (
        <GOATFormula awards={extraProps.awards} formula={extraProps.goatFormula} stats={extraProps.stats} />
      ) : null}
      ...
      <DataTable cols={cols} defaultSort={[0, 'asc']} name={`Most_${type}`} rows={rows} superCols={superCols} />
    </>
  )
}
