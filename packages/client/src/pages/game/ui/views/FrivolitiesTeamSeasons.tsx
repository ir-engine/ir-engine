...
const FrivolitiesTeamSeasons = ({
  ...
  const rows = teamSeasons.map((ts) => {
    return {
      ...
      data: [
        ts.rank,
        <a href={helpers.leagueUrl(['roster', `${ts.abbrev}_${ts.tid}`, ts.season])}>
          {ts.region} {ts.name}
        </a>,
        ts.season,
        ts.won,
        ...
        ...(otl ? [ts.otl] : []),
        ...(ties ? [ts.tied] : []),
        ...(usePts ? [Math.round(ts.standingsPts), helpers.roundWinp(ts.ptsPct)] : [helpers.roundWinp(ts.winp)]),
        wrappedMovOrDiff(
          {
            pts: ts.pts * ts.gp,
  ...
  return (
    <>
      ...
      <DataTable cols={cols} defaultSort={[0, 'asc']} name={`FrivolitiesTeamSeasons_${type}`} nonfluid rows={rows} />
    </>
  )
}
