...
const ViewEvent = ({
  ...
  if (current.type === 'contraction') {
    return <TeamNameBlock all={all} current={current} teamInfoCache={teamInfoCache} />
  }
          ...
          {Object.entries(current.info).map(([key, value]) => {
            return (
              <tr key={key}>
                ...
                <td>
                  {key === 'confs' || key === 'divs'
                    ? (value as any[]).map((x, i) => <div key={i}>{x.name}</div>)
                    : Array.isArray(value)
  ...
  if (current.type === 'teamInfo') {
    return (
      <>
        <TeamNameBlock all={all} current={current} teamInfoCache={teamInfoCache} />
...
const ScheduledEvents = ({ scheduledEvents }: View<'scheduledEvents'>) => {
  ...
  if (scheduledEvents.length === 0) {
    return (
      <>
        ...
        <p>
          Eventually you will be able to add scheduled events here, but currently they are only available in historical
          "real players" leagues where they are created by default when making a new league.
        </p>
      </>
    )
  }
  ...
  const rows = scheduledEvents.map((scheduledEvent) => {
    return {
      ...
      data: [
        ...
        <ViewEvent all={scheduledEvents} current={scheduledEvent} teamInfoCache={teamInfoCache} />
      ]
    }
  })
  ...
  return (
    <>
      <p>
        Eventually this will support creating, updating, and deleting individual scheduled events. But for now, all you
        can do is view scheduled events and apply some bulk operations, like removing all scheduled team contractions.
      </p>
      <Dropdown>
        ...
        <Dropdown.Menu>
          <Dropdown.Item onClick={bulkDelete('all')}>All scheduled events</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('expansionDraft')}>Expansion teams</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('contraction')}>Team contractions</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('teamInfo')}>Team info changes</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('confs')}>Conference/division changes</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('finance')}>League finance changes</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('rules')}>League rule changes</Dropdown.Item>
          <Dropdown.Item onClick={bulkDelete('styleOfPlay')}>Style of play changes</Dropdown.Item>
        </Dropdown.Menu>
      </Dropdown>
      <DataTable cols={cols} defaultSort={[0, 'asc']} name="ScheduledEvents" rows={rows} />
    </>
  )
}
