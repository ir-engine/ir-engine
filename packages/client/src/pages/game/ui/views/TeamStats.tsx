...
const TeamStats = ({
  ...
  const rows = teams.map((t) => {
    const data: { [key: string]: ReactNode } = {
      '#': null,
      abbrev: (
        <a href={helpers.leagueUrl(['roster', `${t.seasonAttrs.abbrev}_${t.tid}`, season])}>{t.seasonAttrs.abbrev}</a>
      ),
      gp: t.stats.gp,
    ...
    data.avgAge = t.seasonAttrs.avgAge !== undefined ? t.seasonAttrs.avgAge.toFixed(1) : null

    for (const stat of stats) {
      const value = t.stats.hasOwnProperty(stat) ? (t.stats as any)[stat] : (t.seasonAttrs as any)[stat]
    ...
    for (const plusMinusCol of plusMinusCols) {
      if (data.hasOwnProperty(plusMinusCol)) {
        data[plusMinusCol] = <PlusMinus>{(t.stats as any)[plusMinusCol]}</PlusMinus>
      }
    }
    ...
    if (userTid === t.tid) {
      ...
      for (const [statType, value] of Object.entries(data)) {
        if ((!stats.includes(statType) && !otherStatColumns.includes(statType)) || !allStats[statType]) {
          continue
        }
...
TeamStats.propTypes = {
  ...
  teamOpponent: PropTypes.oneOf(['advanced', 'opponent', 'team', 'teamShotLocations', 'opponentShotLocations'])
    .isRequired,
