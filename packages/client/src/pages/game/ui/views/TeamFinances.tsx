...
import { BarGraph, DataTable, HelpPopover, MoreLinks, PlayerNameLabels } from '../components'
...
const FinancesForm = ({
  ...
  const handleChange =
    (name: Exclude<keyof typeof state, 'dirty' | 'saving'>) => (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const budgetAmounts = {
      ...
      coaching: helpers.bound(Math.round(parseFloat(state.coaching) * 100) * 10, 0, Infinity),
      facilities: helpers.bound(Math.round(parseFloat(state.facilities) * 100) * 10, 0, Infinity),
      health: helpers.bound(Math.round(parseFloat(state.health) * 100) * 10, 0, Infinity),
      scouting: helpers.bound(Math.round(parseFloat(state.scouting) * 100) * 10, 0, Infinity),

      // Already in [dollars]
      ticketPrice: helpers.bound(parseFloat(parseFloat(state.ticketPrice).toFixed(2)), 0, Infinity)
    }

    await toWorker('main', 'updateBudget', budgetAmounts, state.adjustForInflation, state.autoTicketPrice)
    ...
    <form onSubmit={handleSubmit} className="mb-3">
      <h3>
        ...
        <HelpPopover title="Revenue Settings">
          Set your ticket price too high, and attendance will decrease and some fans will resent you for it. Set it too
          low, and you're not maximizing your profit.
        </HelpPopover>
      </h3>
      {warningMessage}
      <div className="row">
        ...
        <div className="input-group input-group-sm float-left finances-settings-field">
          ...
          {state.autoTicketPrice ? (
            <input type="text" className="form-control" disabled value={formatTicketPrice(autoTicketPrice)} />
          ) : (
            <input
              type="text"
        ...
        <div className="float-left finances-settings-text">Leaguewide rank: #{t.budget.ticketPrice.rank}</div>
      </div>
      {phase === PHASE.PLAYOFFS ? (
        <div className="row mb-1 text-warning" style={paddingLeft100}>
          Playoffs price:{' '}
          {helpers.formatCurrency(
            getAdjustedTicketPrice(state.autoTicketPrice ? autoTicketPrice : parseFloat(state.ticketPrice), true)
          )}
        </div>
      ) : null}
      <div className="row mt-1 mb-3" style={paddingLeft100}>
        <div className="form-check">
          ...
          <HelpPopover title="Auto ticket price" className="ml-1">
            <p>
              When enabled, your ticket price will be set to the maximum value possible while still selling out most
              games.
            </p>
            <p>
              In the playoffs, ticket prices automatically adjust to account for increased demand. That happens
              regardless of the "auto ticket price" setting.
            </p>
          </HelpPopover>
        </div>
      </div>
      <h3>
        ...
        <HelpPopover title="Expense Settings">
          ...
          <p>
            Facilities: Better training facilities make your players happier and other players envious; stadium
            renovations increase attendance.
          </p>
        </HelpPopover>
      </h3>
      <p>
        Click the ? above to see what exactly each category does. Effects are based on your spending rank over the past
        three seasons.
      </p>
      {warningMessage}
      <div className="row">
      ...
      <div className="row mt-1" style={paddingLeft100}>
        <div className="form-check">
          ...
          <HelpPopover title="Inflation adjustment" className="ml-1">
            When enabled, all your revenue and expense settings will automatically change whenever the salary cap
            changes. This will generally maintain your ranks, although expansion teams and changes made by AI teams can
            still result in your ranks changing.
          </HelpPopover>
        </div>
      </div>
      {tid === userTid && !spectator ? (
        <div className="row mt-5" style={paddingLeft100}>
          <button className="btn btn-large btn-primary" disabled={formDisabled || state.saving}>
            Save Revenue and
            <br />
            Expense Settings
          </button>
...
const PayrollInfo = ({
  ...
  View<'teamFinances'>,
  'hardCap' | 'luxuryPayroll' | 'luxuryTax' | 'minContract' | 'minPayroll' | 'payroll' | 'salaryCap'
>) => {
  return (
    <p>
      The current payroll (<b>{helpers.formatCurrency(payroll, 'M')}</b>) is {payroll > minPayroll ? 'above' : 'below'}{' '}
      the minimum payroll limit (<b>{helpers.formatCurrency(minPayroll, 'M')}</b>){hardCap ? ' and' : ','}{' '}
      {payroll > salaryCap ? 'above' : 'below'} the salary cap (<b>{helpers.formatCurrency(salaryCap, 'M')}</b>)
      {hardCap ? null : (
        <>
          , and {payroll > luxuryPayroll ? 'above' : 'below'} the luxury tax limit (
      ...
      {hardCap ? (
        <HelpPopover title="Payroll Limits">
          <p>
            The salary cap is a hard cap, meaning that you cannot exceed it, even when re-signing your own players or
            making trades. The only exception is that you can always sign players to minimum contracts ($
            {minContract}
            k/year), so you are never stuck with a team too small to play.
          </p>
          <p>
            Teams with payrolls below the minimum payroll limit will be assessed a fine equal to the difference at the
            end of the season.
          </p>
        </HelpPopover>
      ) : (
        <HelpPopover title="Payroll Limits">
          <p>
            The salary cap is a soft cap, meaning that you can exceed it to re-sign your own players, to sign free
            agents to minimum contracts ($
            {minContract}
            k/year), and when making certain trades; however, you cannot exceed the salary cap to sign a free agent for
            more than the minimum.
          </p>
          <p>
            Teams with payrolls below the minimum payroll limit will be assessed a fine equal to the difference at the
            end of the season. Teams with payrolls above the luxury tax limit will be assessed a fine equal to{' '}
    ...
    <>
      ...
      {budget ? null : (
        <p className="text-danger">
          The budget is disabled in this league, so most of the information usually shown here is hidden.
        </p>
      )}
      ...
      <div className="row">
        <div className="col-md-3 col-sm-2">
          ...
          <div className="bar-graph-small">
            <BarGraph data={barData.won} labels={barSeasons} ylim={[0, numGames]} />
          </div>
          <br />
          <br />
          <h3>
            ...
            <HelpPopover title="Hype">
              "Hype" refers to fans' interest in your team. If your team is winning or improving, then hype increases;
              if your team is losing or stagnating, then hype decreases. Hype influences attendance, various revenue
              sources such as merchandising, and the attitude players have towards your organization.
            </HelpPopover>
          </h3>
          <div id="bar-graph-hype" className="bar-graph-small">
            <BarGraph
              data={barData.hype}
              labels={barSeasons}
              tooltipCb={(val) => (typeof val === 'number' ? val.toFixed(2) : val)}
              ylim={[0, 1]}
            />
          </div>
          <br />
          ...
          <div id="bar-graph-pop" className="bar-graph-small">
            <BarGraph
              data={barData.pop}
              labels={barSeasons}
              tooltipCb={(val) => (typeof val === 'number' ? `${val.toFixed(1)}M` : val)}
              ylim={[0, 20]}
            />
          </div>
          <br />
          ...
          <div id="bar-graph-att" className="bar-graph-small">
            <BarGraph
              data={barData.att}
              labels={barSeasons}
              tooltipCb={(val) => (typeof val === 'number' ? helpers.numberWithCommas(Math.round(val)) : val)}
          ...
          <div className="col-lg-4 col-md-3 col-sm-3 mb-3">
            ...
            <div id="bar-graph-revenue" className="bar-graph-large">
              <BarGraph
                ...
                tooltipCb={(val) => (typeof val === 'number' ? helpers.formatCurrency(val / 1000, 'M', 1) : val)}
              />
            </div>
            <br />
            <br />
            ...
            <div id="bar-graph-cash" className="bar-graph-medium">
              <BarGraph
                data={barData.cash}
                labels={barSeasons}
                tooltipCb={(val) => (typeof val === 'number' ? helpers.formatCurrency(val, 'M', 1) : val)}
              />
            </div>
          </div>
        ) : null}
      ...
      <p>
        You can release players from <a href={helpers.leagueUrl(['roster'])}>your roster</a>. Released players who are
        still owed money are <i>shown in italics</i>.
      </p>

      <DataTable cols={cols} defaultSort={[2, 'desc']} name="TeamFinances" nonfluid footer={footer} rows={rows} />
    </>
  )
}
