...
import React, { useState, FormEvent, ChangeEvent, MouseEvent, ReactNode } from 'react'
import { PHASE, PLAYER, RATINGS, POSITIONS, MOOD_TRAITS, WEBSITE_ROOT } from '../../../common'
...
const copyValidValues = (
  ...
  if (target.hof !== source.hof) {
    ...
    target.awards = target.awards.filter((award) => !award.type.includes('Hall of Fame'))
    if (!target.hof && source.hof) {
      // Add to HoF
      target.awards.push({
        season,
...
const CustomizePlayer = (props: View<'customizePlayer'>) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    const updatedRatingsOrAge = copyValidValues(state.p, p, props.minContract, props.phase, props.season)
    ...
    setState((prevState) => {
      ...
      if (type === 'root') {
        ...
        if (p.moodTraits.includes(field)) {
          p.moodTraits = p.moodTraits.filter((trait: string) => trait !== field)
        } else {
  ...
  const handleChangeAppearanceOption = (event: ChangeEvent<HTMLSelectElement>) => {
  ...
  const { challengeNoRatings, originalTid, playerMoodTraits, teams } = props
      ...
      <div className="row">
        ...
        <div className="col-sm-8">
          <p>
            ...
            <a href={`http://dumbmatter.com/facesjs/editor.html#${faceHash}`} rel="noopener noreferrer" target="_blank">
              the face editor
            </a>{' '}
            and copy the results back here. Team colors set there will be overridden here.
          </p>
          <textarea
            ...
            value={p.face as any}
          />
          <button type="button" className="btn btn-secondary mt-1" onClick={randomizeFace}>
            Randomize
          </button>
        </div>
      </div>
    ...
    pictureDiv = (
      <div className="form-group">
        ...
        <span className="text-muted">
          Your image must be hosted externally. If you need to upload an image, try using{' '}
          <a href="http://imgur.com/" rel="noopener noreferrer" target="_blank">
            imgur
          </a>
          . For ideal display, crop your image so it has a 2:3 aspect ratio (such as 100px wide and 150px tall).
        </span>
      </div>
    )
  }
  ...
  const adjustRatings = (amount: number) => async (event: MouseEvent) => {
    ...
    setState((prevState) => {
      ...
      for (const key of keys) {
        ...
        newRatings[key] = helpers.bound(parseInt(oldRatings[key]) + amount, 0, 100)
      }
  ...
  return (
    <>
      <p>
        Here, you can {originalTid === undefined ? 'create a custom player with' : 'edit a player to have'} whatever
        attributes and ratings you want. If you want to make a whole league of custom players, you should probably
        create a <a href={`https://${WEBSITE_ROOT}/manual/customization/`}>custom League File</a>.
      </p>
                  ...
                  <div className="input-group-append">
                    <button
                      ...
                      onClick={async (event) => {
                        ...
                        const { firstName, lastName } = await toWorker('main', 'getRandomName', p.born.loc)
              ...
              <div className="col-sm-3 form-group">
                <label>
                  ...
                  <HelpPopover title="Height (inches)">
                    Height (inches) is just for show. The height rating is what actually gets used in game simulations.
                  </HelpPopover>
                </label>
                <input
                  type="text"
              ...
              <div className="col-sm-3 form-group">
                <label>
                  ...
                  <HelpPopover title="Weight (lbs)">
                    Weight (lbs) is just for show. The height and strength ratings are what actually gets used in game
                    simulations.
                  </HelpPopover>
                </label>
                <input
                  ...
                  <div className="input-group-append">
                    <button
                      ...
                      onClick={async (event) => {
                        ...
                        const pos = await toWorker('main', 'getAutoPos', p.ratings[r])
                  ...
                  <div className="input-group-append">
                    <button
                      ...
                      onClick={async (event) => {
                        ...
                        const country = await toWorker('main', 'getRandomCountry')
                  ...
                  <div className="input-group-append">
                    <button
                      ...
                      onClick={async (event) => {
                        ...
                        const college = await toWorker('main', 'getRandomCollege')
          ...
          <div className="col-md-5 mb-3">
            <div className="float-right d-flex flex-column">
              <button
                ...
                onClick={async (event) => {
                  event.preventDefault()
                  const { hgt, ratings } = await toWorker('main', 'getRandomRatings', (p as any).age, undefined)
              ...
              <div className="ml-1 btn-group">
                <button type="button" className="btn btn-secondary btn-sm" onClick={adjustRatings(-1)} disabled={true}>
            ...
            <RelativesForm handleChange={handleChange} relatives={p.relatives} />
          </div>
        </div>
