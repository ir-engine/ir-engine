...
export const genView = (type: 'college' | 'country' | 'draftPosition' | 'jerseyNumbers') => {
  return ({ challengeNoRatings, infos, stats, userTid, displayStat }: View<'colleges'>) => {
    useTitleBar({
      title:
        type === 'college'
          ? 'Colleges'
    ...
    const rows = infos.map((c) => {
      ...
      return {
        ...
        data: [
          ...
          >
            {type === 'draftPosition' && c.name === 'undrafted' ? 'none' : c.name}
          </a>,
          c.numPlayers,
          c.numActivePlayers,
          c.numHof,
          ...
          {
            value: (
              <PlayerNameLabels jerseyNumber={p.jerseyNumber} pid={p.pid} watch={p.watch} disableWatchToggle>
                {p.name}
              </PlayerNameLabels>
            ),
            classNames: {
          ...
          {
            value: <a href={helpers.leagueUrl(['team_history', `${abbrev}_${p.legacyTid}`])}>{abbrev}</a>,
            classNames: {
    ...
    return (
      <>
        <p>
          Players who have played for your team are <span className="text-info">highlighted in blue</span>. Active
          players are <span className="text-success">highlighted in green</span>. Hall of Famers are{' '}
