...
const StatsRow = ({ p, ...props }: { i: number; p: any }) => {
  ...
  return <BoxScoreRow className={classes} onClick={toggleClicked} p={p} {...props} />
}
...
export const NoGamesMessage = ({ warnAboutDelete }: { warnAboutDelete: boolean }) => (
  <div className="alert alert-info d-inline-block" style={{ maxWidth: 600 }}>
    No games found for this season.
    {warnAboutDelete ? (
      <>
        {' '}
        By default, box scores from old seasons are automatically deleted after 3 years.{' '}
        <a href={helpers.leagueUrl(['settings'])}>You can change this behavior on the League Settings page.</a>
      </>
    ) : null}
  </div>
)
      ...
      <tbody>
        {gamesList.abbrev !== abbrev ? (
          ...
          gamesList.games.map((gm) => {
            ...
            const oppAbbrev = abbrev === 'special' ? 'ASG' : teamInfoCache[gm.teams[other].tid]?.abbrev

            return (
              <tr key={gm.gid} className={gm.gid === gid ? 'table-info' : undefined}>
...
const GameLog = ({ abbrev, boxScore, currentSeason, gamesList, season, tid }: View<'gameLog'>) => {
  const dropdownTeamsKey = 'teamsAndSpecial'
  ...
  useTitleBar({
    ...
    dropdownCustomURL: (fields) => {
      return helpers.leagueUrl(['game_log', fields[dropdownTeamsKey], fields.seasons, boxScore.gid])
    }
  })

  const { currentGidInList, nextGid, prevGid } = findPrevNextGids(gamesList.games, boxScore.gid)

  const noGamesAndNoBoxScore = season < currentSeason && gamesList.games.length === 0 && boxScore.gid < 0

  return (
    <>
      {tid >= 0 ? <MoreLinks type="team" page="game_log" abbrev={abbrev} tid={tid} season={season} /> : null}
