...
const ExpansionDraft = ({
  ...
  const [numProtectedPlayers, setNumProtectedPlayers2] = useState(initialNumProtectedPlayers)
  ...
  const setTeams = async (newTeams: ExpansionDraftSetupTeam[]) => {
    const newNumProtectedPlayers = String(
      helpers.bound(parseInt(initialNumProtectedPlayers) - newTeams.length, 0, Infinity)
    )
    const newNumPerTeam = String(
      helpers.getExpansionDraftMinimumPlayersPerActiveTeam(newTeams.length, minRosterSize, numActiveTeams)
    )
    setTeams2(newTeams)
  ...
  if (phaseDisabled) {
    return <p>You can only do an expansion draft during the preseason or draft lottery phases.</p>
  }

  const handleInputChange =
    (i: number) => async (field: string, event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
  ...
  const addTeam = async (event: MouseEvent) => {
    ...
    const newTeam = builtInTeams.find((t) => t.abbrev === addTeamAbbrev) || defaultTeam
  ...
  const handleTakeControl = (i: number) => async (event: ChangeEvent<HTMLInputElement>) => {
    const newTeams = [...teams]
  ...
  const disableNumProtectedPlayersChange = teams.some((t) => t.takeControl)
  const defaultNumProtectedPlayers = String(minRosterSize - teams.length)
  if (disableNumProtectedPlayersChange && numProtectedPlayers !== defaultNumProtectedPlayers) {
  ...
  return (
    <>
      <p>
        In an expansion draft, new teams are added to the league. Rather than starting with no players, they draft
        players from existing teams. However the list of players available to be drafted is limited, each existing team
        has a chance to protect some of their players.
      </p>
      ...
      <form onSubmit={handleSubmit}>
                    ...
                    <div className="row">
                      <div className="col-6">
                        <div className="form-check mt-2">
                          ...
                          />
                          <label className="form-check-label" htmlFor={`expansion-control-team-${i}`}>
                            {multiTeamMode ? 'Add team to multi team mode' : 'Switch to controlling this team'}
                          </label>
                        </div>
                      </div>
                      <div className="col-6 text-right">
                        <button type="button" className="btn btn-danger" onClick={deleteTeam(i)}>
                          Remove Team
                        </button>
                      </div>
                    </div>
          ...
          <div className="col-xl-4 col-lg-6 mb-3">
            <div className="card">
              <div className="card-body">
                ...
                <button type="button" className="btn btn-primary mt-2" onClick={addTeam}>
                  Add Team
                </button>
              </div>
            </div>
        ...
        <div className="d-sm-flex">
          <div className="form-group mb-0 mr-sm-3">
            <label htmlFor="expansion-num-protected">Number of players each existing team can protect</label>
            <input
        ...
        <button type="submit" className="btn btn-primary mt-3" disabled={saving || teams.length === 0}>
