...
const PlayerGameLog = ({
  ...
  useTitleBar({
    ...
    dropdownCustomURL: (fields) => {
      const parts =
        fields.playerProfile === 'gameLog' ? ['player_game_log', player.pid, fields.seasons] : ['player', player.pid]

      return helpers.leagueUrl(parts)
    }
  })

  const cols = getCols(['#', 'Team', '@', 'Opp', 'Result', 'Record', '', ...stats.map((stat) => `stat:${stat}`)])
    ...
    return {
      ...
      data: [
        ...
        <>
          ...
          <a href={helpers.leagueUrl(['roster', `${game.abbrev}_${game.tid}`, season])}>{game.abbrev}</a>
        </>,
        game.away ? '@' : '',
        {
          value: (
            <>
              ...
              <a href={helpers.leagueUrl(['roster', `${game.oppAbbrev}_${game.oppTid}`, season])}>{game.oppAbbrev}</a>
            </>
          ),
        ...
        ...stats.map((stat) =>
          game.stats[stat] === undefined ? undefined : helpers.roundStat(game.stats[stat], stat, true)
        )
      ]
    }
  }
      ...
      {noGamesMessage ? (
        ...
        <>
          ...
          {rowsPlayoffs.length > 0 ? (
            <>
              <h2 className={rowsRegularSeason.length > 0 ? 'mt-5' : undefined}>Playoffs</h2>
              <DataTable
