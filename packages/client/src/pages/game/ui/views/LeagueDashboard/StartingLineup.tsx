...
import { PlayerNameLabels, RatingWithChange, ResponsiveTableWrapper } from '../../components'
...
const StartingLineup = ({
  ...
  starters,
  startersStats
}: Pick<View<'leagueDashboard'>, 'challengeNoRatings' | 'numPlayersOnCourt' | 'starters' | 'startersStats'>) => {
  const statCols = getCols(startersStats.map((stat) => `stat:${stat}`))

  return (
    <>
      <h2>{numPlayersOnCourt >= starters.length ? 'Starting Lineup' : 'Top Players'}</h2>
