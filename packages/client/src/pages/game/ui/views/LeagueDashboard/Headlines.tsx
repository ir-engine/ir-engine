  ...
  return function <Props>(component: ComponentType<Props>) {
    ...
    class Throttled extends Component<Props, State> {
      ...
      constructor(props: Props) {
        ...
        this.throttledSetState = throttle((nextState: State) => this.setState(nextState), wait)
      }
...
const Headlines = ({ events, season, userTid }: Pick<View<'leagueDashboard'>, 'events' | 'season' | 'userTid'>) => {
        ...
        <AnimatePresence initial={false}>
          {events.map((event) => {
            return (
              ...
              >
                <NewsBlock event={event} season={season} teams={teamInfoCache} userTid={userTid} />
              </m.div>
            )
          })}
        </AnimatePresence>
...
const ThrottledComponent = memo(throttleRender(2000)(Headlines), (prevProps, nextProps) => {
  ...
  if (prevProps.season !== nextProps.season || prevProps.userTid !== nextProps.userTid) {
    return false
  }
