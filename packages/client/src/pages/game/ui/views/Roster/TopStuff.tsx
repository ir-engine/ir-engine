...
import { RecordAndPlayoffs, RosterComposition, PlusMinus } from '../../components'
...
const TeamRating = ({ ovr, ovrCurrent }: { ovr: number; ovrCurrent: number }) => {
  ...
  const title = showCurrent ? 'Current rating, including injuries' : 'Rating when healthy'
  const rating = showCurrent ? ovrCurrent : ovr
  const className = showCurrent ? 'text-danger' : undefined

  return (
    <>
      <a className="cursor-pointer" title={title} onClick={() => setShowCurrent(!showCurrent)}>
...
> & {
  ...
  let marginOfVictory = t.stats.pts - t.stats.oppPts
  ...
  return (
    <>
        ...
        <div className="d-flex">
          ...
          <div>
            <div>
              ...
              <span title="Average margin of victory">Average MOV</span>: <PlusMinus>{marginOfVictory}</PlusMinus>
              <br />
              <span title="Average age, weighted by minutes played">Average age</span>:{' '}
      ...
      {season !== currentSeason ? (
        <p>
          Players still on this team are <span className="text-info">highlighted in blue</span>. Players in the Hall of
