...
import { PHASE, PLAYER, WEBSITE_ROOT } from '../../../common'
import {
  CountryFlag,
  HelpPopover,
  Mood,
...
const justDrafted = (p: View<'roster'>['players'][number], phase: Phase, season: number) => {
  return (
    p.contract.rookie &&
...
const handleRelease = async (p: View<'roster'>['players'][number], phase: Phase, season: number) => {
  ...
  if (proceed) {
    const errorMsg = await toWorker('main', 'releasePlayer', p.pid, wasPlayerJustDrafted)
    if (errorMsg) {
      logEvent({
        type: 'error',
        text: errorMsg,
...
const Roster = ({
  ...
  return (
    <>
      <MoreLinks type="team" page="roster" abbrev={abbrev} tid={tid} season={season} />
      ...
      {showSpectatorWarning ? (
        <p className="alert alert-danger d-inline-block">The AI will handle roster management in spectator mode.</p>
      ) : null}
      ...
      <SortableTable
        ...
        rowClassName={({ index, isDragged, value: p }) =>
          classNames({
            separator: index === numPlayersOnCourt - 1 && !isDragged,
            'table-danger': p.hof,
            ...
            {editable ? (
              <th title="Playing Time Modifier">
                ...
                <HelpPopover title="Playing Time Modifier">
                  <p>
                    Your coach will divide up playing time based on ability and stamina. If you want to influence his
                  ...
                  <p>
                    ...
                    <br />
                    <span style={ptStyles['1']}>&nbsp;&nbsp;&nbsp; Let Coach Decide</span>
                    <br />
            ...
            {showRelease ? (
              <th>
                ...
                <HelpPopover title="Release Player">
                  <p>
                    To free up a roster spot, you can release a player from your team. You will still have to pay his
                    salary (and have it count against the salary cap) until his contract expires (you can view your
                    released players' contracts in your <a href={helpers.leagueUrl(['team_finances'])}>Team Finances</a>
                    ).
                  </p>
                  {!hardCap ? (
                    <p>
                      However, if you just drafted a player and the regular season has not started yet, his contract is
                      not guaranteed and you can release him for free.
                    </p>
                  ) : null}
                </HelpPopover>
              </th>
        ...
        row={({ value: p }) => {
          ...
          return (
            <>
              ...
              <td>{p.age}</td>
              <td>
                {showRatings ? <RatingWithChange change={p.ratings.dovr}>{p.ratings.ovr}</RatingWithChange> : null}
              </td>
              <td>
                {showRatings ? <RatingWithChange change={p.ratings.dpot}>{p.ratings.pot}</RatingWithChange> : null}
              </td>
              {season === currentSeason ? (
                <td
                  style={{
                    fontStyle: justDrafted(p, phase, currentSeason) ? 'italic' : 'normal'
                  }}
                  title={
