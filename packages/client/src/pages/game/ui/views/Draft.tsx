...
import { DataTable, DraftAbbrev, MoreLinks, PlayerNameLabels, RosterComposition } from '../components'
...
const Draft = ({
  ...
  useTitleBar({
    title: fantasyDraft ? 'Fantasy Draft' : expansionDraft ? 'Expansion Draft' : 'Draft'
  })
  ...
  const userRemaining = remainingPicks.some((p) => userTids.includes(p.draft.tid))
  ...
  if (fantasyDraft || expansionDraft) {
    colsUndrafted.splice(6, 0, ...getCols(['Contract', 'Exp', ...stats.map((stat) => `stat:${stat}`)]))
  }
  ...
  const rowsUndrafted = undrafted.map((p) => {
    const data = [
      p.rank,
      <PlayerNameLabels pid={p.pid} injury={p.injury} skills={p.ratings.skills} watch={p.watch}>
        {p.name}
      </PlayerNameLabels>,
      p.ratings.pos,
      p.age,
    ...
    if (expansionDraft) {
      data.splice(3, 0, <a href={helpers.leagueUrl(['roster', `${p.abbrev}_${p.tid}`])}>{p.abbrev}</a>)
    }
  ...
  const colsDrafted = getCols(['Pick', 'Team']).concat(colsUndrafted.slice(1, -1))
  ...
  const rowsDrafted = drafted.map((p, i) => {
    const data = [
      ...
      {
        searchValue: `${teamInfoCache[p.draft.tid]?.abbrev} ${teamInfoCache[p.draft.originalTid]?.abbrev}`,
        sortValue: `${p.draft.tid} ${p.draft.originalTid}`,
        value: <DraftAbbrev originalTid={p.draft.originalTid} tid={p.draft.tid} />
      },
      p.pid >= 0 ? (
        <PlayerNameLabels pid={p.pid} injury={p.injury} skills={p.ratings.skills} watch={p.watch}>
          {p.name}
        ...
        <TradeButton
          ...
          disabled={drafting}
          tid={p.draft.tid}
          visible={!fantasyDraft && !expansionDraft && !userTids.includes(p.draft.tid) && !spectator}
        />
      ),
    ...
    if (fantasyDraft || expansionDraft) {
      data.splice(
        7,
        0,
        ...(p.pid >= 0 ? [helpers.formatCurrency(p.contract.amount, 'M'), p.contract.exp] : [null, null]),
        ...stats.map((stat) =>
          p.pid >= 0 && p.stats && typeof p.stats[stat] === 'number' ? helpers.roundStat(p.stats[stat], stat) : null
        )
      )
    }

    if (expansionDraft) {
      data.splice(2, 0, <a href={helpers.leagueUrl(['roster', `${p.prevAbbrev}_${p.prevTid}`])}>{p.prevAbbrev}</a>)
    }
    ...
    return {
      ...
      data,
      classNames: {
        'table-info': userTids.includes(p.draft.tid) || userTids.includes(p.prevTid)
      }
    }
  })
  ...
  const colClass = fantasyDraft || expansionDraft ? 'col-12 col-xl-6' : 'col-sm-6'
              ...
              {challengeNoDraftPicks && !fantasyDraft && !expansionDraft ? (
                <div>
                  <p className="alert alert-danger d-inline-block">
                    <b>Challenge Mode:</b> Your team does not get any draft picks unless you acquire them in a trade.
                  </p>
                </div>
              ) : null}
              {spectator ? (
                <div>
                  <p className="alert alert-danger d-inline-block">
                    In spectator mode you can't make draft picks, you can only watch the draft.
                  </p>
                </div>
              ) : null}
              {expansionDraftFilteredTeamsMessage ? (
                <div>
                  <p className="alert alert-warning d-inline-block">{expansionDraftFilteredTeamsMessage}</p>
                </div>
              ) : null}
              <DraftButtons spectator={spectator} userRemaining={userRemaining} usersTurn={usersTurn} />
            </>
          ) : (
            <>
              <p id="draft-buttons">
                <span className="alert alert-success d-inline-block mb-0">The draft is over!</span>
              </p>
              {fantasyDraft || expansionDraft ? (
                <p>
                  <span className="alert alert-warning d-inline-block mb-0">
                    Draft results from {fantasyDraft ? 'fantasy' : 'expansion'} drafts are only temporarily viewable.
                    When you navigate away from this page or proceed to the next phase of the game, you cannot come back
                    to this page.
                  </span>
                </p>
              ) : null}
