...
type OfferProps = {
  ...
  handleClickNegotiate: (tid: number, otherPids: number[], otherDpids: number[]) => Promise<void>
  i: number
  stats: string[]
} & OfferType
  ...
  if (players.length > 0) {
    ...
    offerPlayers = (
      <div className="col-md-8">
        <DataTable cols={cols} defaultSort={[5, 'desc']} hideAllControls name="TradingBlockOffer" rows={rows} />
      </div>
    )
  }
  ...
  return (
    <div className="mt-4">
      ...
      <p>
        {won}-{lost}
        {otl > 0 ? <>-{otl}</> : null}
        {tied > 0 ? <>-{tied}</> : null}, {strategy}, {helpers.formatCurrency(payroll / 1000, 'M')} payroll
      </p>
      <div className="row">
        {offerPlayers}
        {offerPicks}
        {picks.length === 0 && players.length === 0 ? <div className="col-12">Nothing.</div> : null}
      </div>
...
const TradingBlock = (props: View<'tradingBlock'>) => {
  ...
  const handleClickAsk = async () => {
    ...
    const offers: OfferType[] = await toWorker('main', 'getTradingBlockOffers', state.pids, state.dpids)
  ...
  const handleClickNegotiate = async (tid: number, otherPids: number[], otherDpids: number[]) => {
  ...
  const { challengeNoRatings, challengeNoTrades, gameOver, spectator, phase, stats, userPicks, userRoster } = props
  ...
  if ((phase >= PHASE.AFTER_TRADE_DEADLINE && phase <= PHASE.PLAYOFFS) || phase === PHASE.FANTASY_DRAFT || gameOver) {
    return (
      <div>
        <h2>Error</h2>
        <p>
          You're not allowed to make trades {phase === PHASE.AFTER_TRADE_DEADLINE ? 'after the trade deadline' : 'now'}.
        </p>
      </div>
    )
  }
  ...
  const cols = getCols(
    ['', 'Name', 'Pos', 'Age', 'Ovr', 'Pot', 'Contract', 'Exp', ...stats.map((stat) => `stat:${stat}`)],
    {
      '': {
        sortSequence: [],
        noSearch: true
    ...
    <>
      ...
      <div className="row mb-3">
        <div className="col-md-9">
          <DataTable cols={cols} defaultSort={[6, 'desc']} name="TradingBlock" rows={rows} />
        </div>
      ...
      <div className="text-center">
        <button className="btn btn-lg btn-primary" disabled={state.asking} onClick={handleClickAsk}>
      ...
      {state.offers.length > 0 ? (
        <div className="text-center">
          ...
          <button className="btn btn-lg btn-primary" disabled={state.asking} onClick={handleClickAskBottom}>
