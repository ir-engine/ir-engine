...
import { confirm, helpers, toWorker, logEvent, realtimeUpdate, getCols } from '../util'
...
const PlayerList = ({
  ...
  return (
    <>
      <p>
        Check the box to the left of a player to protect him. Any players who are not protected may be selected in the
        expansion draft.
      </p>
      {upcomingFreeAgentsText}
      <p>
      ...
      <div>
        <DataTable cols={cols} defaultSort={[5, 'desc']} name="ProtectPlayers" rows={rows} hideAllControls nonfluid />
      </div>
    </>
  )
}
...
const ProtectPlayers = ({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (numRemaining > 0 && protectedPids.length < players.length && !spectator) {
      ...
      for (const tid of userTids) {
        ...
        const numRemaining2 = expansionDraft.numProtectedPlayers - protectedPids2.length
        if (numRemaining2 > 0) {
          const result = await confirm(
  ...
  const upcomingFreeAgentsText =
    nextPhase !== undefined && nextPhase > PHASE.PLAYOFFS ? (
      <p>
        <span className="text-warning">Upcoming free agents are available to be protected or drafted.</span> If drafted,
        they will never refuse to re-sign with their expansion team.
      </p>
    ) : null
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        {expansionTeam ? (
          <>
            <p>Your team is an expansion team, so you have no players to protect.</p>
            {upcomingFreeAgentsText}
            <p>Max number of selected players per existing team: {expansionDraft.numPerTeam ?? '???'}</p>
          </>
        ...
        <button type="submit" className="btn btn-primary" disabled={(!expansionTeam && numRemaining < 0) || saving}>
          Start Expansion Draft
        </button>

        <button type="button" className="btn btn-light-bordered ml-2" disabled={saving} onClick={handleCancel}>
