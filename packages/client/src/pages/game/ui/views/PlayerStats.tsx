...
export const formatStatGameHigh = (ps: any, stat: string, statType?: string, defaultSeason?: number) => {
  if (stat.endsWith('Max')) {
    ...
    const row = ps[stat] as unknown as [number, number] | [number, number, string, number, number]

    const abbrev = row.length > 3 ? row[2] : ps.abbrev
    const tid = row.length > 3 ? row[3] : ps.tid
    const season = row.length > 3 ? row[4] : ps.season !== undefined ? ps.season : defaultSeason

    return (
      <a href={helpers.leagueUrl(['game_log', `${abbrev}_${tid}`, season as any, row[1]])}>
...
const PlayerStats = ({
  ...
  const rows = players.map((p) => {
    ...
    const statsRow = stats.map((stat) => formatStatGameHigh(p.stats, stat, statType, season))
  ...
  return (
    <>
      <MoreLinks type="playerStats" page="player_stats" season={season} statType={statType} keepSelfLink />
