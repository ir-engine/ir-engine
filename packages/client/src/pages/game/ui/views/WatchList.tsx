...
const WatchList = ({ challengeNoRatings, flagNote, players, playoffs, statType, stats }: View<'watchList'>) => {
  ...
  const cols = getCols(
    ['', 'Name', 'Pos', 'Age', 'Team', 'Ovr', 'Pot', 'Contract', 'Exp', ...stats.map((stat) => `stat:${stat}`), 'Note'],
    {
      Note: {
        width: '100%'
      }
  ...
  const rows = players.map((p) => {
    ...
    return {
      ...
      data: [
        ...
        contract,
        exp,
        ...stats.map((stat) => helpers.roundStat(p.stats[stat], stat, statType === 'totals')),
        {
          value: (
            <div
  ...
  return (
    <>
      <Dropdown className="float-right my-1">
        <Dropdown.Toggle id="watch-list-other-reports" className="btn-light-bordered">
          Other Reports
        </Dropdown.Toggle>
        <Dropdown.Menu>
      ...
      <p>
        Click the watch icon <span className="glyphicon glyphicon-flag" /> to add or remove a player from this list.
      </p>
      <p>
      ...
      <button className="btn btn-danger mb-3" disabled={clearing} onClick={clearWatchList}>
        Clear Watch List
      </button>

      <DataTable cols={cols} defaultSort={[5, 'desc']} name="WatchList" pagination rows={rows} />
    </>
  )
}
