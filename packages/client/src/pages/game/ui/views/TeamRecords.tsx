...
const isHistorical = (t: { root: boolean; disabled?: boolean }) => !t.root || t.disabled

const TeamRecords = ({ byType, filter, teams, ties, otl, usePts, userTid }: View<'teamRecords'>) => {
    ...
    .map((t, i) => {
      return {
        ...
        data: [
          ...
          ...(otl ? [t.otl] : []),
          ...(ties ? [t.tied] : []),
          ...(usePts ? [t.pts, helpers.roundWinp(t.ptsPct)] : [helpers.roundWinp(t.winp)]),
          t.playoffs,
          t.lastPlayoffs,
          t.finals,
          t.lastFinals,
  ...
  return (
    <>
      ...
      <DataTable cols={cols} defaultSort={[0, 'asc']} name="TeamRecords" pagination={false} rows={rows} />
    </>
  )
}
