...
const UpcomingFreeAgents = ({
  ...
  const rows = players.map((p) => {
    return {
      ...
      data: [
        ...
        ...(phase === PHASE.RESIGN_PLAYERS ? [] : [helpers.formatCurrency(p.contract.amount, 'M')]),
  ...
  return (
    <>
      ...
      <p>
        Projected {season} cap space: {helpers.formatCurrency(projectedCapSpace / 1000, 'M')}
      </p>

      {phase !== PHASE.RESIGN_PLAYERS ? (
        <p>
          Keep in mind that many of these players will choose to re-sign with their current team rather than become free
          agents. Also, even if a player is &gt;99% willing to re-sign with his team, he still may become a free agent
          if his team does not want him.
        </p>
      ) : null}

      <p>
        "Projected mood" is based on projected salary demands, not on projected future team performance or any other
        factors.
      </p>
