...
import { DataTable, MoreLinks, PlayerNameLabels, TeamLogoInline } from '../components'
  ...
  const ret = (
    <div className="d-flex">
      <div className="mr-auto">
        ...
        (<a href={helpers.leagueUrl(['roster', `${award.abbrev}_${award.tid}`, season])}>{award.abbrev}</a>)
      </div>
...
const teamName = (t: View<'historyAll'>['seasons'][number]['champ'], season: number) => {
  if (t) {
    return (
      <>
        <a href={helpers.leagueUrl(['roster', `${t.abbrev}_${t.tid}`, season])}>{t.region}</a> ({t.won}-{t.lost}
        {t.otl > 0 ? <>-{t.otl}</> : null}
...
const formatTeam = (t: View<'historyAll'>['seasons'][number]['champ'], season: number, userTid: number) => {
  if (!t) {
    return null
  }
...
const HistoryAll = ({ awards, seasons, userTid }: View<'historyAll'>) => {
  ...
  const cols = getCols(['Season', 'League Champion', 'Runner Up', ...awards.map((award) => `award:${award}`)])
  ...
  const rows = seasons.map((s) => {
    ...
    return {
      key: s.season,
      data: [seasonLink, champEl, runnerUpEl, ...awards.map((award) => awardName(s[award], s.season, userTid))]
    }
  })
