...
const ResetPassword = ({ token }: View<'resetPassword'>) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setState((state2) => ({
            ...
            globalMessage: (
              <>
                Invalid password reset token. <a href="/account/lost_password">Request another and try again.</a>
              </>
            ),
            showForm: false
          }))
        ...
        for (const error of Object.keys(data.errors)) {
          if (error === 'password') {
            ...
            updatedState.errorMessagePassword =
              updatedState.errorMessagePassword === undefined ? '' : updatedState.errorMessagePassword // So it gets highlighted too
            updatedState.errorMessagePassword2 = data.errors[error]
          }
        }
  ...
  return (
    <>
      ...
      <p>
        If you are having trouble with this, please <a href={`mailto:${EMAIL_ADDRESS}`}>email {EMAIL_ADDRESS}</a>.
      </p>
    </>
  )
}
