...
const HeadToHead = ({ abbrev, season, teams, tid, ties, type, otl, totals, userTid }: View<'headToHead'>) => {
  useTitleBar({
    title: 'Head-to-Head',
  ...
  const cols = getCols([
    ...
    'PA/g',
    'Diff',
    ...(type === 'regularSeason' ? [] : ['Rounds Won', 'Rounds Lost', 'Finals Won', 'Finals Lost'])
  ])
  ...
  const makeRow = (t: typeof totals) => {
    ...
    return [
      ...
      ...(type === 'regularSeason' ? [] : [t.seriesWon, t.seriesLost, t.finalsWon, t.finalsLost])
    ]
  }
  ...
  return (
    <>
      ...
      <p>
        View <a href={helpers.leagueUrl(['head2head_all', season, type])}>head-to-head results for all teams</a> in one
        giant table.
      </p>

      <DataTable cols={cols} defaultSort={[0, 'asc']} name="HeadToHead" nonfluid rows={rows} footer={footer} />
    </>
  )
}
