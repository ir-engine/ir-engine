...
import { ACCOUNT_API_URL, STRIPE_PUBLISHABLE_KEY, fetchWrapper } from '../../common'
...
const AccountUpdateCard = (props: View<'accountUpdateCard'>) => {
  ...
  useEffect(() => {
    ;(async () => {
      if (!window.Stripe) {
        ...
        ;(window.Stripe as stripe.StripeStatic).setPublishableKey(STRIPE_PUBLISHABLE_KEY)
      }
  ...
  if (goldCancelled) {
    errorMessage = 'Cannot update card because your GM Gold account is cancelled.'
  }
  if (errorMessage) {
    return (
      <>
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit}>
        {state.formError ? <div className="alert alert-danger">{state.formError}</div> : null}
        ...
        <div style={{ maxWidth: '300px' }}>
          ...
          <div className="form-group" style={{ maxWidth: '100px' }}>
            ...
            <input type="text" onChange={handleChange('cvc')} value={state.cvc} id="cvc" className="form-control" />
          </div>
          ...
          <button type="submit" disabled={state.disabled} className="btn btn-primary">
            Update Card
          </button>
        </div>
      </form>
