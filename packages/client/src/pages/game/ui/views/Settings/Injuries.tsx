...
import { confirm, downloadFile, logEvent, resetFileInput, toWorker } from '../../util'
...
const ImportButton = ({
  ...
  <button className="btn btn-light-bordered" style={{ position: 'relative', overflow: 'hidden' }} onClick={() => {}}>
    Import
    <input
        ...
        reader.onload = (event2) => {
          try {
            ...
            if (
              ...
              setErrorMessage('File should be a CSV file with columns: name, frequency, games')
              return
            }
...
const Controls = ({
  ...
  position: 'top' | 'bottom'
  setInjuries: (injuries: InjuriesText | ((injuries: InjuriesText) => InjuriesText)) => void
}) => {
  ...
  return (
    <>
      <div className="d-flex justify-content-between">
        <div className="btn-group">
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item
                onClick={async () => {
                  setInjuries(formatInjuries(await toWorker('main', 'getDefaultInjuries')))
                }}
              >
                Default
              </Dropdown.Item>
        ...
        <div className="btn-group">
          <ImportButton setErrorMessage={setImportErrorMessage} setInjuries={setInjuries} />
          <ExportButton injuries={injuries} />
        </div>
      </div>

      {importErrorMessage ? <div className="text-danger mt-3">{importErrorMessage}</div> : null}
    </>
  )
}
...
const parseAndValidate = (injuriesText: InjuriesText): InjuriesSetting => {
  ...
  for (const row of injuries) {
    if (isInvalidNumber(row.frequency)) {
      throw new Error(`Injury "${row.name}" has an invalid frequency - must be a positive number.`)
    }

    if (isInvalidNumber(row.games)) {
      throw new Error(`Injury "${row.name}" has an invalid number of games - must be a positive number.`)
    }
  }
...
const Injuries = ({
  ...
  const [injuries, setInjuriesRaw] = useState(() => formatInjuries(defaultValue))
  ...
  const handleCancel = async () => {
    if (dirty) {
      const result = await confirm('Are you sure you want to discard your changes?', {
        okText: 'Discard',
        cancelText: 'Cancel'
      })
      if (!result) {
  ...
  const handleChange = (key: 'name' | 'frequency' | 'games', i: number) => (event: ChangeEvent<HTMLInputElement>) => {
    setInjuries((rows) =>
      rows.map((row, j) => {
        if (i !== j) {
          return row
  ...
  return (
    <>
      <button className="btn btn-secondary" type="button" disabled={disabled} onClick={handleShow}>
        Customize
      </button>
      ...
      <Modal show={show} onHide={handleCancel} animation={animation} scrollable>
        ...
        <Modal.Body>
          <p>
            Injury rate is determined by the "Injury Rate" setting, which is viewable on the main League Settings page.
            When an injury occurs, the type of injury is randomly selected from this list. The probability of a type
            being selected is its "frequency" value divided by the sum of all frequencies.
          </p>
          <p>
            "Games" is the average number of games that will be missed. There is some variability based on luck and
            health spending.
          </p>

          <Controls position="top" injuries={injuries} setInjuries={setInjuries} />
          ...
          {injuries.length > 0 ? (
                      ...
                      <div className="form-row mt-1 flex-grow-1" key={i}>
                        ...
                        <div className="col-3">
                          <input
                            ...
                            className={classNames('form-control', {
                              'is-invalid': isInvalidNumber(parseFloat(injury.frequency))
                            })}
                        ...
                        <div className="col-3">
                          <input
                            type="text"
                            className={classNames('form-control', {
                              'is-invalid': isInvalidNumber(parseFloat(injury.games))
                            })}
                            value={injury.games}
            ...
            <div className="mt-3 text-danger">You must define at least one injury type.</div>
          )}

          {injuries.length > 0 ? <Controls position="bottom" injuries={injuries} setInjuries={setInjuries} /> : null}
        </Modal.Body>
        <Modal.Footer>
          <button className="btn btn-secondary" onClick={handleCancel}>
            Cancel
          </button>
          <button className="btn btn-primary" onClick={handleSave} disabled={injuries.length === 0}>
            Save
          </button>
        </Modal.Footer>
      </Modal>
