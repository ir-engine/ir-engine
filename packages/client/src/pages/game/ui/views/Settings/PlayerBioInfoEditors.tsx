...
import { PlayerBioInfoRowButton, smallColStyle } from './PlayerBioInfoCountries'
...
const parseAndValidateRaces = (races: RaceRow[]) => {
  ...
  for (const row of races) {
    ...
    if (Number.isNaN(number)) {
      throw new Error(`Invalid frequency "${row.frequency}" for race "${row.race}"`)
    }
  }
}
...
export const RacesEditor = ({
  ...
  const handleChange = (field: 'frequency', i: number) => (event: ChangeEvent<HTMLInputElement>) => {
    setRowsEdited((rows) =>
      rows.map((row, j) => {
        if (i !== j) {
          return row
      ...
      <Modal.Body>
        {defaults ? (
          <p className="alert alert-warning">
            Default races apply only to custom countries you create, not any of the built-in countries in the game.
            Built-in countries all have their own predefined default races.
          </p>
        ) : null}
        ...
        >
          ...
          {rowsEdited.map((rows, i) => (
            <div key={rows.race} className="form-row mt-2 align-items-center">
              <div className="col-6">{helpers.upperCaseFirstLetter(rows.race)}</div>
              <div className="col-6">
                <input
                  type="text"
...
const isInvalidFractionSkipCollege = (fractionSkipCollege: string, defaults: boolean) => {
...
const parseAndValidateColleges = (colleges: CollegeRow[], fractionSkipCollege: string, defaults: boolean) => {
  for (const row of colleges) {
    const number = parseFloat(row.frequency)
    if (Number.isNaN(number)) {
      throw new Error(`Invalid frequency "${row.frequency}" for college "${row.name}"`)
    }
  }

  if (isInvalidFractionSkipCollege(fractionSkipCollege, defaults)) {
    throw new Error(`Invalid fraction skip college value "${fractionSkipCollege}"`)
  }
}
...
export const CollegesEditor = ({
  ...
  const [fractionSkipCollegeEdited, setFractionSkipCollegeEdited] = useState(fractionSkipCollege)

  const handleCancel = async () => {
    // Reset for next time
    setRowsEdited(lastSavedState.current?.rowsEdited ?? [...rows])
    setFractionSkipCollegeEdited(lastSavedState.current?.fractionSkipCollegeEdited ?? fractionSkipCollegeEdited)
  ...
  const handleChange = (field: 'name' | 'frequency', i: number) => (event: ChangeEvent<HTMLInputElement>) => {
    setRowsEdited((rows) =>
      rows.map((row, j) => {
        if (i !== j) {
          return row
  ...
  const isInvalidFraction = isInvalidFractionSkipCollege(fractionSkipCollegeEdited, defaults)
      ...
      <Modal.Body>
        <form onSubmit={handleSave}>
          <div className="form-group">
            <label htmlFor="fractionSkipCollege">Fraction of players who skip college</label>
            <input
              type="text"
              className={classNames('form-control', {
            ...
            {isInvalidFraction ? (
              <span className="form-text text-danger">
                {defaults ? 'Value must be between 0 and 1.' : 'Value must be blank (default) or between 0 and 1.'}
              </span>
            ) : (
        ...
        <CollegesControls defaultRows={defaultRows} position="top" rows={rowsEdited} onSave={setRowsEdited} />
            ...
            <div key={i} className="d-flex">
              <div className="d-flex mt-2 flex-grow-1">
                <div className="flex-grow-1">
                  <input type="text" className="form-control" value={row.name} onChange={handleChange('name', i)} />
                </div>
        ...
        <CollegesControls defaultRows={defaultRows} position="bottom" rows={rowsEdited} onSave={setRowsEdited} />
      </Modal.Body>
      <Modal.Footer>
        <button className="btn btn-secondary" onClick={handleCancel}>
          Cancel
...
type NameRow = PlayerBioInfoState['countries'][number]['names']['first'][number]
...
const parseAndValidateNames = (names: NameRows) => {
  for (const key of ['first', 'last'] as const) {
    ...
    for (const row of names[key]) {
      const number = parseFloat(row.frequency)
      if (Number.isNaN(number)) {
        throw new Error(`Invalid frequency "${row.frequency}" for ${key} name "${row.name}"`)
      }
    }
  }
}
...
export const NamesEditor = ({
  ...
  const setRowsEditedWrapper = (type: 'first' | 'last') => (rowsNew: NameRow[]) => {
    setRowsEdited((rows) => ({
      ...rows,
      [type]: rowsNew
    }))
    ...
    <>
      <Modal.Body>
        ...
        {rowsEdited[firstOrLast].length > 0 ? (
            ...
            {rowsEdited[firstOrLast].map((row, i) => (
              <div key={i} className="d-flex">
                ...
                <PlayerBioInfoRowButton
                  className="text-danger"
                  onClick={() => {
                    setRowsEditedWrapper(firstOrLast)(rowsEdited[firstOrLast].filter((row2) => row !== row2))
                  }}
                  title="Delete"
                  icon="glyphicon-remove"
                />
          ...
          <div className="mt-3 text-danger">You must define at least one {firstOrLast} name.</div>
        )}
      ...
      <Modal.Footer>
        ...
        <button className="btn btn-primary" onClick={handleSave} disabled={rowsEdited[firstOrLast].length === 0}>
          Save Names
        </button>
      </Modal.Footer>
    </>
