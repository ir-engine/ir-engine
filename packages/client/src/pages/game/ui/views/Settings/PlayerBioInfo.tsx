...
import { CollegesEditor, NamesEditor, RacesEditor } from './PlayerBioInfoEditors'
...
export const formatPlayerBioInfoState = (playerBioInfo: PlayerBioInfo | undefined, defaults: Defaults | undefined) => {
  ...
  const mergedCountries = mergeCountries(playerBioInfo, defaults.namesCountries, defaults.namesGroups, defaults.groups)

  const defaultMergedCountries =
    playerBioInfo === undefined
      ? mergedCountries
      : mergeCountries(undefined, defaults.namesCountries, defaults.namesGroups, defaults.groups)
  ...
  const defaultColleges2 = playerBioInfo?.default?.colleges ?? defaults.colleges

  const defaultFractionSkipCollege2 = playerBioInfo?.default?.fractionSkipCollege ?? 0.98
  ...
  for (const [country, frequency] of Object.entries(frequencies)) {
    ...
    if (builtIn) {
      if (allDefaults) {
        ...
        if (namesCountry) {
          defaultNames = builtIn && isEqual(names.first, namesCountry.first) && isEqual(names.last, namesCountry.last)
        }
      }
    }
    ...
    const defaultRaces2 = defaults.races[country] ?? playerBioInfo?.default?.races ?? defaults.races.USA
    ...
    const fractionSkipCollege =
      ...
      (country === 'USA' || country === 'Canada' ? 0.02 : defaultFractionSkipCollege2)
    const fractionSkipCollegeText =
      fractionSkipCollege === defaultFractionSkipCollege2 ? '' : String(fractionSkipCollege)
  ...
  const defaultRacesText = objectToArray(playerBioInfo?.default?.races ?? defaults.races.USA, 'race', 'race')
...
export const parseAndValidate = (state: PlayerBioInfoState) => {
  ...
  for (const row of state.countries) {
    if (output.frequencies.hasOwnProperty(row.country)) {
      throw new Error(`Country names must be unique, but you have multiple countries named "${row.country}"`)
    }

    const frequency = parseFloat(row.frequency)
    if (Number.isNaN(frequency)) {
      throw new Error(`Invalid frequency "${row.frequency}" for country "${row.country}"`)
    }
    ...
    for (const type of ['first', 'last'] as const) {
      if (row.names[type].length === 0) {
        throw new Error(`You must define at least one ${type} name for the country "${row.country}".`)
      }
...
export const prune = (info: Required<PlayerBioInfo>, defaults: Defaults): PlayerBioInfo | undefined => {
  ...
  for (const [name, country] of Object.entries(info.countries)) {
    ...
    if (
      ((name === 'USA' || name === 'Canada') && country.fractionSkipCollege === 0.02) ||
...
const PlayerBioInfo2 = ({
  ...
  const [infoState, setInfoStateRaw] = useState<PlayerBioInfoState | undefined>()
  const [defaultsState, setDefaultsState] = useState<PlayerBioInfoState | undefined>()
  ...
  const setInfoState = (infoState: PlayerBioInfoState | ((infoState: PlayerBioInfoState) => void)) => {
  ...
  const loadDefaults = async () => {
    ...
    const defaultsState = defaultValue === undefined ? infoState : formatPlayerBioInfoState(undefined, defaults)
  ...
  const handleCancel = async () => {
    if (dirty) {
      const result = await confirm('Are you sure you want to discard all your changes?', {
        okText: 'Discard',
        cancelText: 'Cancel'
      })
      if (!result) {
      ...
      // Reset for next time
      setInfoStateRaw(lastSavedState.current ?? formatPlayerBioInfoState(defaultValue, defaults))
      setDirty(false)
    }
  ...
  const handleSave = (event: { preventDefault: () => void; stopPropagation: () => void }) => {
  ...
  const handleChange =
    (key: 'country' | 'frequency' | 'games', i: number) => (event: ChangeEvent<HTMLInputElement>) => {
      setInfoState((data) => ({
        ...data,
        countries: data.countries.map((row, j) => {
          ...
          if (key === 'country') {
            ...
            extraProps.builtIn = !!defaults?.countries.hasOwnProperty(event.target.value)
          }
  ...
  // Just sets the default to false. Might be better to compare values, like it does in prune.
  const handleChange2 =
    (key: 'colleges' | 'fractionSkipCollege' | 'names' | 'races', i: number | 'default') => (rows: any) => {
      const defaultProp = `default${helpers.upperCaseFirstLetter(key)}` as const
        ...
        if (key === 'fractionSkipCollege') {
          ...
          setInfoState((data) => ({
            ...
            countries: data.countries.map((row) => {
              ...
              if (defaultProp !== 'defaultFractionSkipCollege' && row[defaultProp]) {
                return {
                  ...row,
                  [key]: [...rows]
                }
  ...
  if (infoState && defaults && defaultsState) {
    ...
    if (pageInfo.name !== 'countries') {
      const countryName = pageInfo.index === 'default' ? 'Default' : infoState.countries[pageInfo.index].country
      title += ` - ${countryName} - ${helpers.upperCaseFirstLetter(pageInfo.name)}`
    }

    modal = (
      <Modal size="lg" show={show} onHide={handleCancel} animation={animation} scrollable>
        ...
        {pageInfo.name === 'countries' ? (
            ...
            onSetDefault={(type, i) => {
              ...
              if (type === 'colleges') {
                ...
                if (defaults.races[country]) {
                  array = objectToArray(defaults.races[country], 'race', 'race')
                } else {
                  array = [...infoState.defaultRaces]
                }
              } else if (type === 'names') {
                const row = defaultsState.countries.find((row) => row.country === country)
                if (!row) {
              ...
              setInfoState((data) => ({
                ...
                countries: data.countries.map((row, j) => {
                  ...
                  const defaultProp = `default${helpers.upperCaseFirstLetter(type)}`
          ...
          <RacesEditor
            ...
            rows={pageInfo.index === 'default' ? infoState.defaultRaces : infoState.countries[pageInfo.index].races}
            onCancel={onCancel}
          ...
          <>
            <CollegesEditor
              defaultRows={pageInfo.index === 'default' ? defaultsState.defaultColleges : infoState.defaultColleges}
              defaults={pageInfo.index === 'default'}
              rows={
                pageInfo.index === 'default' ? infoState.defaultColleges : infoState.countries[pageInfo.index].colleges
              }
              onCancel={onCancel}
              ...
              onSaveFractionSkipCollege={handleChange2('fractionSkipCollege', pageInfo.index)}
            />
          </>
        ) : pageInfo.name === 'names' ? (
          <>
            <NamesEditor
              defaultRows={
                defaultsState.countries.find((row) => row.country === infoState.countries[pageInfo.index].country)
                  ?.names
              }
  ...
  return (
    <>
      <button className="btn btn-secondary" type="button" disabled={disabled} title={title} onClick={handleShow}>
        Customize
      </button>
