...
import React, { Fragment, useState, ReactNode, ChangeEvent, FormEvent, useEffect } from 'react'
import { ActionButton, HelpPopover, StickyBottomButtons } from '../../components'
...
const encodeDecodeFunctions = {
  ...
  floatValuesOrCustom: {
    stringify: (value: number, values: Values) => {
      ...
      return JSON.stringify([values.every(({ key }) => key !== stringValue), stringValue])
    },
    parse: (value: string) => {
...
const Input = ({
  ...
  const commonProps = {
    ...
    onChange,
    style:
      !decoration && type !== 'rangePercent' && type !== 'floatValuesOrCustom' && !maxWidth ? inputStyle : undefined,
    value
  }
  ...
  if (type === 'bool') {
    ...
    const switchTitle = checked ? 'Enabled' : 'Disabled'
    inputElement = (
    ...
    inputElement = (
      <div className="d-flex" style={inputStyle}>
        ...
        <div>
          <input type="range" {...commonProps} className="form-control-range" min="0" max="1" step="0.05" />
        </div>
      </div>
    )
  } else if (values) {
    if (type === 'floatValuesOrCustom') {
      ...
      const selectValue = parsed[0] || values.every(({ key }) => key !== parsed[1]) ? 'custom' : parsed[1]
      inputElement = (
        <div className="input-group" style={inputStyle}>
          <select {...commonProps} className="form-control" value={selectValue} style={{ width: 60 }}>
  ...
  return (
    <>
      <div className="d-flex align-items-center" style={{ minHeight: 33 }}>
        ...
        <div className={classNames('ml-auto', maxWidth ? 'w-100' : undefined)}>{formElement}</div>
      </div>
      {description ? <div className="text-muted settings-description mt-1">{description}</div> : null}
...
const SettingsButton = ({
  ...
  return (
    <button type="button" className={classNames('btn btn-secondary', className)} disabled={disabled} onClick={onClick}>
      {children}
    </button>
  )
}
...
const SettingsForm = ({
  ...
  const handleChange = (name: Key, type: FieldType) => (event: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    let value: string
    if (type === 'bool') {
  ...
  const categories: {
    ...
    },
    {
      ...
      helpText: (
        <>
          <p>
            Changing these settings will only apply to the current season if the regular season or playoffs have not
            started yet. Otherwise, changes will be applied for next year.
          </p>
          <p>
            The schedule is set by first accounting for "# Division Games" and "# Conference Games" for each team. Then,
            remaining games are filled with any remaining teams (non-conference teams, plus maybe division and
    ...
    },
    {
      ...
      helpText: (
        <>
          <p>
            This lets you randomly change your league's financial settings (salary cap, min payroll, luxury tax payroll,
            min contract, max contract) every year before the draft. It works by picking a{' '}
            <a
            ...
            based on the parameters set below (min, max, average, and standard deviation).
          </p>
          <p>
            If you have any scheduled events containing specific finance changes then these settings will be ignored
            until all those scheduled events have been processed. Basically this means that for historical real players
            leagues, these inflation settings will only take effect once your league moves into the future.
          </p>
        </>
      )
    },
    ...
    },
    {
      ...
      helpText: (
        <>
          <p>
            The{' '}
            <a href="https://thetournament.com/elam-ending" rel="noopener noreferrer" target="_blank">
              Elam Ending
            </a>{' '}
            is a new way to play the end of basketball games. In the final period of the game, when the clock goes below
            a certain point ("Minutes Left Trigger"), the clock is turned off. The winner of the game will be the team
            that first hits a target score. That target is determined by adding some number of points ("Target Points to
            Add") to the leader's current score.
          </p>
          <p>By default, the trigger is 4 minutes remaining and the target points to add is 8.</p>
          <p>
            The Elam Ending generally makes the end of the game more exciting. Nobody is trying to run out the clock.
            Nobody is trying to foul or call strategic timeouts or rush shots. It's just high quality basketball, every
    ...
    <div className="settings-wrapper mt-lg-2">
      <form onSubmit={handleFormSubmit} style={{ maxWidth: 2100 }}>
        {categories.map((category) => {
          ...
          const catOptions = groupedSettings[category.name]
              ...
              <div className="row mb-5 mb-md-3">
                {catOptions.map(
                  ({ customForm, decoration, description, descriptionLong, key, maxWidth, name, type, values }, i) => {
                      ...
                      if (key === 'stopOnInjuryGames') {
                        ...
                        customFormNode = (
                          <div style={inputStyle} className="d-flex align-items-center">
                            <div className="custom-control custom-switch" title={checked ? 'Enabled' : 'Disabled'}>
                              <input
                                type="checkbox"
                                className="custom-control-input"
                                checked={checked}
                                ...
                                value={state[key2]}
                              />
                              <label className="custom-control-label" htmlFor={id + '2'}></label>
                            </div>
                    ...
                    return (
                      <div key={key} className="settings-col col-md-6 col-xxl-4 d-flex">
                        <div
        ...
        <StickyBottomButtons>
          <div className="btn-group ml-auto">
            {onCancel ? (
              <button className="btn btn-secondary" type="button" disabled={submitting} onClick={onCancel}>
                Cancel
              </button>
            ) : null}
            <ActionButton type="submit" disabled={submitting} processing={!!newLeague && submitting}>
              {saveText}
            </ActionButton>
          </div>
        </StickyBottomButtons>
