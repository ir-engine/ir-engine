      ...
      onChange={(event) => {
        ...
        reader.onload = async (event2) => {
          try {
            ...
            setInfoState(formatPlayerBioInfoState(info.gameAttributes.playerBioInfo, defaults))
          } catch (error) {
...
const ExportButton = ({ defaults, infoState }: { defaults: Defaults; infoState: PlayerBioInfoState }) => (
  <button
...
type SetInfoState = (infoState: PlayerBioInfoState | ((infoState: PlayerBioInfoState) => PlayerBioInfoState)) => void
...
const Controls = ({
  ...
  const currentCountryNames = new Set(infoState.countries.map((row) => row.country))
  const defaultCountriesAvailable = defaultsState.countries.filter((row) => !currentCountryNames.has(row.country))
  ...
  return (
    <>
      <div className="d-flex justify-content-between">
        <div className="btn-group">
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item
                onClick={async () => {
                  setInfoState(formatPlayerBioInfoState(await toWorker('main', 'getDefaultInjuries'), defaults))
                }}
              >
                Default
              </Dropdown.Item>
        ...
        <div className="btn-group">
          <ImportButton defaults={defaults} setErrorMessage={setImportErrorMessage} setInfoState={setInfoState} />
  ...
  return (
    <>
      <Modal.Body ref={wrapperRef}>
        <p>
          By default, leagues can have players from any of the built-in countries. Each built-in country comes with
          built-in names and races, and they all share the same default colleges list. Here you can edit any of that, or
          add custom countries.
        </p>
        <p>
          The probability of a new player being from a certain country being selected is its "frequency" value divided
          by the sum of all frequencies. Names, colleges, and races work the same way.
        </p>
        ...
        {infoState.countries.length > 0 ? (
          <form onSubmit={handleSave} className="my-3">
            ...
            <div className="d-flex font-weight-bold" style={{ marginRight: 55 }}>
                ...
                <div className="d-flex mt-2 flex-grow-1">
                  ...
                  <div style={smallColStyle}>
                    <input
                      ...
                      className={classNames('form-control', {
                        'is-invalid': isInvalidNumber(parseFloat(country.frequency))
                      })}
                  ...
                  {(['names', 'colleges', 'races'] as const).map((key) => {
                    ...
                    if (key === 'names' && !country.builtIn) {
                      ...
                      return (
                        <div style={smallColStyle} key={key}>
                          <button className="btn btn-secondary w-100" onClick={onClickCustom}>
                            Custom
                          </button>
                        </div>
                      )
                    ...
                    return (
                      <div style={smallColStyle} key={key}>
                        <Dropdown>
                          <Dropdown.Toggle variant="secondary" id={`dropdown-${key}-${country.id}`} className="w-100">
                            {country[`default${helpers.upperCaseFirstLetter(key)}` as const] ? 'Default' : 'Custom'}
                          </Dropdown.Toggle>
                          ...
                          <Dropdown.Menu>
                            ...
                            <Dropdown.Item onClick={onClickCustom}>Custom</Dropdown.Item>
                          </Dropdown.Menu>
                        </Dropdown>
                      </div>
                    )
          ...
          <div className="mt-3 text-danger">You must define at least one country.</div>
        )}
      ...
      <Modal.Footer>
        ...
        <button className="btn btn-primary" onClick={handleSave} disabled={infoState.countries.length === 0}>
          Save
        </button>
      </Modal.Footer>
    </>
