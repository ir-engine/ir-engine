...
const Icon = ({ onClick, watch }: { onClick?: () => void; watch?: boolean }) => {
  return (
    <span
...
const RatingsStatsPopover = ({ disableWatchToggle, season, pid, watch }: Props) => {
  ...
  if (name) {
    ...
    nameBlock = (
      <div className="d-flex">
        {jerseyNumber ? (
          <div className="text-muted jersey-number-popover align-self-end mr-1">{jerseyNumber}</div>
        ) : null}
        <a href={helpers.leagueUrl(['player', pid])} className="font-weight-bold text-truncate">
          {name}
        </a>
        {ratings !== undefined ? <div className="ml-1">{ratings.pos}</div> : null}
        {abbrev !== undefined && tid !== undefined && tid !== PLAYER.RETIRED ? (
          <a
            href={helpers.leagueUrl(['roster', `${abbrev}_${tid}`, ratings ? ratings.season : undefined])}
            className="ml-1"
          >
            {abbrev}
          </a>
        ) : null}
        {age !== undefined ? <div className="ml-1 flex-shrink-0">{age} yo</div> : null}
        {!disableWatchToggle && typeof watch === 'boolean' ? <WatchBlock pid={pid} watch={watch} /> : null}
      </div>
    )
  }
  ...
  const modalBody = <RatingsStats ratings={ratings} stats={stats} type={type} />
  ...
  const renderTarget = ({ onClick }: { onClick?: () => void }) => <Icon onClick={onClick} watch={watch} />
