...
const RatingsStats = ({ challengeNoRatings, ratings, stats, type }: Props) => {
  const seasonPrefix = typeof type === 'number' ? `${type} ` : type === 'career' ? 'Peak ' : ''
  const seasonPrefix2 = type === 'career' || type === 'draft' ? 'Career ' : seasonPrefix
  ...
  if (challengeNoRatings) {
    ...
    ratingsBlock = (
      <div className="row mb-2">
        ...
        <div className="col-4">
          <span style={gradientStyle(ratings.pot)}>Pot: {Math.round(ratings.pot)}</span>
          <br />
