    ...
    <>
      {boxScore.teams.map((t: any) => {
        ...
        for (let i = 0; i < t.players.length; i++) {
          const p = t.players[i]
          const addToHealthy = !injuredToBottom || p.injury.gamesRemaining === 0 || p.min > 0 || p.injury.playingThrough

          const row = (
            <Row
              key={p.pid}
              lastStarter={addToHealthy && rowsHealthy.length + 1 === (boxScore.numPlayersOnCourt || 5)}
        ...
        return (
          <div key={t.abbrev} className="mb-3">
            <h2>
              {t.tid >= 0 ? (
                <a href={helpers.leagueUrl(['roster', `${t.abbrev}_${t.tid}`, boxScore.season])}>
                  {t.region} {t.name}
                </a>
              ) : (
                <>
      ...
      {boxScore.gameOver !== false && boxScore.clutchPlays && boxScore.clutchPlays.length > 0
