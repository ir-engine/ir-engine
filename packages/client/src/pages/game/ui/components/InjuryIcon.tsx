...
const InjuryIcon = ({
  ...
  if (injury.gamesRemaining === -1) {
    ...
    return (
      <span className={`badge badge-${colorClass} badge-injury ${className}`} title={injury.type}>
        +
      </span>
    )
  }
  ...
  if (injury.gamesRemaining > 0 || injury.type !== 'Healthy') {
    let title = `${injury.type}, out ${injury.gamesRemaining} more ${timeBetweenGames(injury.gamesRemaining)}`
    if (injury.playingThrough) {
      title += ', played through injury'
    }

    return (
      <span className={`badge badge-${colorClass} badge-injury ${className}`} title={title}>
