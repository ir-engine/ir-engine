  ...
  const wonPtsLink = (value: number) => {
    ...
    return (
      <a
        ...
        href={helpers.leagueUrl(['game_log', `${team.abbrev}_${team.tid}`, season, gid])}
      >
        {value}
      </a>
    )
    ...
    >
      <div className="playoff-matchup-logo d-flex align-items-center justify-content-center flex-shrink-0">
        {!team.pendingPlayIn && (team.imgURL || team.imgURLSmall) ? (
          <img className="mw-100 mh-100" style={lost ? faded : notFaded} src={team.imgURLSmall ?? team.imgURL} alt="" />
        ) : null}
      </div>
      ...
      {team.pendingPlayIn ? (
        <div className="align-self-start">
          {expandTeamName ? (
            ...
            <>
              <span className="d-xxl-none" title="Play-in team, to be determined">
                TBD
              </span>
        ...
        <div className="mr-1 overflow-hidden">
          <a
            ...
            href={helpers.leagueUrl(['roster', `${team.abbrev}_${team.tid}`, season])}
          >
            {expandTeamName ? (
              team.region
            ) : (
          ...
          <span className="text-muted">
            ...
            {team.regularSeason.otl !== undefined && team.regularSeason.otl > 0 ? <>-{team.regularSeason.otl}</> : null}
            {team.regularSeason.tied !== undefined && team.regularSeason.tied > 0 ? (
      ...
      {!showWon && showPts && typeof team.pts === 'number' ? wonPtsLink(team.pts) : null}
    </li>
  )
}
...
const PlayoffMatchup = ({
  ...
  const homeWon = !series.away || (series.home.hasOwnProperty('won') && series.home.won === numGamesToWinSeries)
  const awayWon = !!series.away && series.away.hasOwnProperty('won') && series.away.won === numGamesToWinSeries
  const showPts =
    !!series.away && series.away.pts !== undefined && series.home.pts !== undefined && numGamesToWinSeries === 1
  const showWon = !!series.away && numGamesToWinSeries > 1

  const gid = series.gids && series.gids.length > 0 ? series.gids.at(-1) : undefined
