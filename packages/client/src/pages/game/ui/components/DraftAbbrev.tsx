...
const DraftAbbrev = ({
  ...
  const args1 = season === undefined ? ['roster', `${abbrev}_${tid}`] : ['roster', `${abbrev}_${tid}`, season]
  ...
  return (
    <>
      <a href={helpers.leagueUrl(args1)}>{abbrev}</a> (from <a href={helpers.leagueUrl(args2)}>{originalAbbrev}</a>)
    </>
  )
}
