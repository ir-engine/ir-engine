...
import { helpers, localActions, menuItems, safeLocalStorage, useLocalShallow } from '../util'
import DropdownLinks from './DropdownLinks'
import LogoAndText from './LogoAndText'
import PlayMenu from './PlayMenu'

const PhaseStatusBlock = () => {
  const { liveGameInProgress, phase, phaseText, statusText } = useLocalShallow((state) => ({
...
const NavBar = ({ updating }: { updating: boolean }) => {
  const { lid, gold, hasViewedALeague, spectator, playMenuOptions, popup, username, viewInfo } = useLocalShallow(
    (state) => ({
      lid: state.lid,
      gold: state.gold,
  ...
  const userBlock = username ? (
    <Nav.Link href="/account">
      <span className="glyphicon glyphicon-user" /> <span className="d-none d-lg-inline">{username}</span>
    </Nav.Link>
  ) : (
    <Nav.Link href="/account/login_or_register">
      <span className="glyphicon glyphicon-user" /> <span className="d-none d-lg-inline">Login/Register</span>
    </Nav.Link>
  )

  return (
    <Navbar bg="light" expand="sm" fixed="top" className="navbar-border flex-nowrap" role="banner">
      <button
          ...
          <OverlayTrigger
            ...
            overlay={
              <Popover id="popover-welcome">
                <Popover.Title className="text-primary font-weight-bold">Welcome to {GAME_NAME}!</Popover.Title>
                <Popover.Content>
                  To advance through the game, use the Play button at the top. The options shown will change depending
                  on the current state of the game.
                </Popover.Content>
              </Popover>
            }
          >
            <PlayMenu lid={lid} spectator={spectator} options={playMenuOptions} />
          </OverlayTrigger>
        </Nav>
      ) : null}
      ...
      <div className="d-none d-sm-flex">
        <DropdownLinks inLeague={inLeague} lid={lid} menuItems={menuItems} />
      </div>
