...
import React, { useCallback, useEffect, useReducer, useRef, useState, ChangeEvent } from 'react'
import { MAX_SUPPORTED_LEAGUE_VERSION, GAME_NAME, WEBSITE_ROOT } from '../../common'

// This is dynamically resolved with rollup-plugin-alias
// @ts-ignore
import schema from '../assets/files/league-schema.json'
...
const ErrorMessage = ({ error }: { error: Error | null }) => {
  ...
  return (
    <>
      ...
      <a href={`https://${WEBSITE_ROOT}/manual/faq/#latest-version`} rel="noopener noreferrer" target="_blank">
...
const LeagueFileUpload = ({ disabled, enterURL, hideLoadedMessage, onDone, onLoading }: LeagueFileUploadProps) => {
        ...
        <div className="form-row">
          ...
          <div className="col-auto">
            <button
              ...
              disabled={disabled || state.status === 'loading' || state.status === 'parsing'}
            >
              Load
            </button>
          </div>
      ...
      <div style={styleStatus}>
        ...
        {state.jsonSchemaErrors.length > 0 ? (
          <p className="alert alert-warning mt-3">
            Warning: {state.jsonSchemaErrors.length} JSON schema validation errors. More detail is available in the
            JavaScript console. Also, see{' '}
            <a href={`https://${WEBSITE_ROOT}/manual/customization/json-schema/`}>the manual</a> for more information.
            You can still use this file, but these errors may cause bugs.
          </p>
        ) : null}
        {state.status === 'loading' ? <p className="alert alert-info mt-3">Loading league file...</p> : null}
        {state.status === 'parsing' ? <p className="alert alert-info mt-3">Parsing league file...</p> : null}
        {state.status === 'done' && !hideLoadedMessage ? <p className="alert alert-success mt-3">Loaded!</p> : null}
      </div>
    </>
  )
}
