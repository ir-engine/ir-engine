...
const Notification = ({
  ...
  return (
    <div className={classNames('notification', extraClass)} ref={notificationElement}>
      {title ? (
        <>
          <strong>{title}</strong>
          <br />
      ...
      <button className="notification-close" onClick={remove} title="Dismiss notification">
        &times;
      </button>
    </div>
  )
...
const Notifications = () => {
  const { stickyFooterAd, stickyFormButtons, userTids } = useLocalShallow((state) => ({
    ...
    const unbind2 = emitter.on('notification', (notification) => {
      ...
      setNotifications((currentNotifications) => {
        ...
        if (numPersistentKept > MAX_NUM_NOTIFICATIONS - 1) {
          newNotifications = newNotifications.slice(numPersistentKept - (MAX_NUM_NOTIFICATIONS - 1))
        }
