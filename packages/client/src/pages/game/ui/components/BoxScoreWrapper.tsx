...
const TeamNameLink = ({
  ...
  return t.tid >= 0 ? (
    <a href={helpers.leagueUrl(['roster', `${t.abbrev}_${t.tid}`, season])}>{children}</a>
  ) : (
    <>{children}</>
  )
}
...
const HeadlineScore = ({ boxScore }: any) => {
  ...
  const t0 = boxScore.won && boxScore.won.name ? boxScore.won : boxScore.teams[0]
  const t1 = boxScore.lost && boxScore.lost.name ? boxScore.lost : boxScore.teams[1]

  return (
    <>
      <h2>
        {t0.playoffs ? <span className="text-muted">{t0.playoffs.seed}. </span> : null}
        <TeamNameLink season={boxScore.season} t={t0}>
          <span className="d-none d-sm-inline">{t0.region} </span>
          {t0.name}
        </TeamNameLink>{' '}
        {t0.pts}, {t1.playoffs ? <span className="text-muted">{t1.playoffs.seed}. </span> : null}
    ...
    <table className="table table-bordered table-sm mb-2 mb-sm-0">
      <thead>
        ...
        <tr>
          ...
          <th title="Four Factors: Free Throws Made Over Field Goal Attempts">FT/FGA</th>
        </tr>
      </thead>
      <tbody>
        {teams.map((t, i) => {
          ...
          return (
            <tr key={t.abbrev}>
              <td className={efg > efg2 ? 'table-success' : undefined}>{helpers.roundStat(efg, 'efg')}</td>
              <td className={tovp < tovp2 ? 'table-success' : undefined}>{helpers.roundStat(tovp, 'tovp')}</td>
              <td className={orbp > orbp2 ? 'table-success' : undefined}>{helpers.roundStat(orbp, 'orbp')}</td>
              <td className={ftpFga > ftpFga2 ? 'table-success' : undefined}>{helpers.roundStat(ftpFga, 'ftpFga')}</td>
            </tr>
          )
        })}
      </tbody>
...
const NextButton = ({
  ...
  useEffect(() => {
    ...
    const whatever = async () => {
      if (autoGoToNext && nextGid !== undefined) {
        ...
        setTimeout(() => {
          realtimeUpdate([], helpers.leagueUrl(['game_log', `${abbrev}_${tid}`, boxScore.season, nextGid]))
  ...
  const canPlay = playMenuOptions.some((option) => option.id === 'day' || option.id === 'week')
  ...
  return (
    <div className="ml-4">
      ...
      phase <= PHASE.PLAYOFFS ? (
        <button className="btn btn-light-bordered" disabled={!canPlay} onClick={simNext}>
          Sim
          <br />
          Next
        </button>
...
const DetailedScore = ({
  ...
  const qtrs: string[] = boxScore.teams[0].ptsQtrs.map((pts: number, i: number) => {
    return i < boxScore.numPeriods ? `${i + 1}` : `OT${i - boxScore.numPeriods + 1}`
  })
  qtrs.push('F')
          ...
          <table className="table table-bordered table-sm mb-2 mb-sm-0">
            <thead>
              <tr>
                <th />
                {qtrs.map((qtr, i) => (
                  <th key={qtr} className={i < qtrs.length - 1 ? 'text-muted' : undefined}>
                    {qtr}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {boxScore.teams.map((t: any) => (
                <tr key={t.abbrev}>
                  <th>
                    {t.tid >= 0 ? (
                      <a href={helpers.leagueUrl(['roster', t.abbrev, boxScore.season])}>{t.abbrev}</a>
                    ) : (
                      t.abbrev
                    )}
                  </th>
...
const PlayoffRecord = ({
  ...
  if (numGamesToWinSeries === undefined || numGamesToWinSeries <= 1 || !t0.playoffs || !t1.playoffs) {
    return null
  }
  ...
  const winningAbbrevLink = <a href={helpers.leagueUrl(['roster', winning.abbrev, season])}>{winning.abbrev}</a>
...
const BoxScoreWrapper = ({
  ...
  const forceRowUpdate =
    playIndex !== undefined && prevPlayIndex.current !== undefined && playIndex - prevPlayIndex.current > 1
    ...
    (e) => {
      if (showNextPrev) {
        ...
        if (e.keyCode === 37 && boxScore && prevGid !== undefined) {
          // prev
          realtimeUpdate([], helpers.leagueUrl(['game_log', `${abbrev}_${tid}`, boxScore.season, prevGid]))
  ...
  return (
    <>
      ...
      <BoxScore boxScore={boxScore} Row={Row} forceRowUpdate={forceRowUpdate} injuredToBottom={injuredToBottom} />
