...
const PlayerNameLabels = (props: {
  ...
  const { children, disableWatchToggle, injury, jerseyNumber, pid, pos, season, skills, style, watch } = props
  ...
  return (
    <span style={style}>
      ...
      {pid !== undefined ? <a href={helpers.leagueUrl(['player', pid])}>{children}</a> : children}
      <InjuryIcon injury={injury} />
      <SkillsBlock skills={skills} />
      {pid !== undefined ? (
        <RatingsStatsPopover disableWatchToggle={disableWatchToggle} pid={pid} season={season} watch={watch} />
      ) : null}
    </span>
  )
}
