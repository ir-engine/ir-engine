...
const MoreLinks = (
  ...
  const { season: currentSeason } = useLocalShallow((state2) => ({
  ...
  if (props.type === 'team') {
    ...
    links = [
      {
        url: season !== undefined ? ['roster', `${abbrev}_${tid}`, season] : ['roster', `${abbrev}_${tid}`],
        name: 'Roster'
      },
      {
      ...
      {
        url: season !== undefined ? ['game_log', `${abbrev}_${tid}`, season] : ['game_log', `${abbrev}_${tid}`],
        name: 'Game Log'
      },
      {
      ...
      {
        url: season !== undefined ? ['transactions', `${abbrev}_${tid}`, season] : ['transactions', `${abbrev}_${tid}`],
        name: 'Transactions'
      },
      {
        url: season !== undefined ? ['news', `${abbrev}_${tid}`, season] : ['news', `${abbrev}_${tid}`],
        name: 'News Feed'
      }
    ]
    ...
    if (page === 'team_history') {
      links.push({
        url: ['player_stats', `${abbrev}_${tid}`, 'career', 'passing'],
    ...
    links = [
      ...
      {
        url: ['draft_scouting'],
        name: draftType === 'freeAgents' ? 'Upcoming Prospects' : 'Draft Scouting'
      }
    ]
    if (!NO_LOTTERY_DRAFT_TYPES.includes(draftType)) {
      links.push({
        url: season !== undefined ? ['draft_lottery', season] : ['draft_lottery'],
        name: 'Draft Lottery'
      })
    }
    links.push({
    ...
    links = [
      {
        url: season !== undefined ? ['player_stat_dists', season] : ['player_stat_dists'],
    ...
    if (season === undefined || page !== 'player_stats') {
      ...
      links.unshift({
        url: ['player_stats', 'all', 'career', statType === undefined ? 'totals' : statType],
        name: 'Career Totals'
      })
    }
