...
import { local, localActions, realtimeUpdate, toWorker, useLocalShallow } from '../util'
...
const loadFramerMotionFeatures = () => import('../util/framerMotionFeatures').then((res) => res.default)
...
const Controller = () => {
  ...
  const updatePage = useCallback(
    async (Component: any, id: string, inLeague: boolean, context: Context) => {
      const updateEvents = context.state.updateEvents !== undefined ? context.state.updateEvents : []
      ...
      if (
        ...
        (idLoaded.current === id && idLoading.current !== id && idLoading.current !== undefined)
      ) {
      ...
      const results = await toWorker('main', 'runBefore', id, context.params, ctxBBGM, updateEvents, prevData)
      ...
      if (prevData.errorMessage || (results && results.hasOwnProperty('errorMessage'))) {
    ...
    return local.subscribe(
      async (viewInfo: LocalStateUI['viewInfo'] | null) => {
        if (viewInfo !== undefined && viewInfo !== null) {
          try {
            await updatePage(viewInfo.Component, viewInfo.id, viewInfo.inLeague, viewInfo.context)
          } catch (error) {
            viewInfo.cb(error)
            return
          }
