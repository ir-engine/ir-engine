...
const Item = SortableElement(
  ({ col, hidden, onToggleHidden }: { col?: Col; hidden?: boolean; onToggleHidden: () => void }) => {
    let title
    if (col) {
      title = col.title
      if (col.desc) {
    ...
    return (
      <div className="form-check">
        <input className="form-check-input" type="checkbox" checked={!hidden} onChange={onToggleHidden} />
...
const Container = SortableContainer(({ children, isDragged }: { children: any[]; isDragged: boolean }) => {
  return (
    <ul
...
const CustomizeColumns = ({
  ...
  return (
    <Modal animation={false} centered show={show} onHide={onHide}>
      ...
      <Modal.Body>
        <p>Click and drag to reorder columns, or use the checkboxes to show/hide columns.</p>
        {hasSuperCols ? (
        ...
        >
          {colOrder.map(({ colIndex, hidden }, i) => {
            ...
            return <Item key={colIndex} index={i} onToggleHidden={onToggleHidden(i)} hidden={hidden} col={col} />
          })}
        </Container>
      </Modal.Body>
      <Modal.Footer>
