...
import React, { SyntheticEvent, MouseEvent, ReactNode, useState, useEffect, useCallback } from 'react'
...
const DataTable = ({
  ...
  const processRows = () => {
    const filterFunctions = state.enableFilters
      ? state.filters.map((filter, i) =>
          createFilterFunction(filter, cols[i] ? cols[i].sortType : undefined, cols[i] ? cols[i].searchType : undefined)
        )
      : []
      ...
      : rows.filter((row) => {
          ...
          if (state.enableFilters) {
            for (let i = 0; i < row.data.length; i++) {
              ...
              if (filterFunctions[i] && filterFunctions[i](row.data[i]) === false) {
                return false
              }
            }
          }
    ...
    const colOrderFiltered = state.colOrder.filter(({ hidden, colIndex }) => !hidden && cols[colIndex])

    return rowsOrdered.map((row, i) => {
      return {
        ...row,
        data: colOrderFiltered.map(({ colIndex }) => (colIndex === rankCol ? i + 1 : row.data[colIndex]))
      }
    })
  }
  ...
  const handleExportCSV = () => {
    const colOrderFiltered = state.colOrder.filter(({ hidden, colIndex }) => !hidden && cols[colIndex])
    const columns = colOrderFiltered.map(({ colIndex }) => cols[colIndex].title)
    const rows = processRows().map((row) => row.data.map((val) => getSearchVal(val, false)))
  ...
  const handleFilterUpdate = (event: SyntheticEvent<HTMLInputElement>, i: number) => {
  ...
  useEffect(() => {
    if (addFilters !== undefined && addFilters.length === state.filters.length) {
  ...
  }, [addFilters, setStatePartial, state.enableFilters, state.filters, state.settingsCache])
    ...
    <>
      <CustomizeColumns
        ...
        onSortEnd={({ oldIndex, newIndex }) => {
          const newOrder = arrayMoveImmutable(state.colOrder, oldIndex, newIndex)
          setStatePartial({
            colOrder: newOrder
          })
          state.settingsCache.set('DataTableColOrder', newOrder)
      ...
      >
        <>
          {pagination && !hideAllControls ? <PerPage onChange={handlePerPage} value={state.perPage} /> : null}
          <Controls
        ...
        <ResponsiveTableWrapper className={pagination ? 'fix-margin-pagination' : null} nonfluid={nonfluid}>
          <table
        ...
        {!hideAllControls ? (
          <>
            ...
            {pagination ? (
              <Info end={end} numRows={numRowsFiltered} numRowsUnfiltered={rows.length} start={start} />
            ) : null}
            {pagination ? (
              <Pagination
...
DataTable.propTypes = {
  ...
  defaultSort: PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.number, PropTypes.string])).isRequired,
