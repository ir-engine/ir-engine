...
const Header = ({
  ...
  return (
    <thead>
      {superCols ? <SuperCols colOrder={colOrder} superCols={superCols} /> : null}
      <tr>
        {colOrder.map(({ colIndex }) => {
          const { classNames: colClassNames, desc, sortSequence, title, width } = cols[colIndex]
      ...
      {enableFilters ? (
        <FilterHeader colOrder={colOrder} cols={cols} filters={filters} handleFilterUpdate={handleFilterUpdate} />
      ) : null}
    </thead>
  )
}
...
Header.propTypes = {
  ...
  handleFilterUpdate: PropTypes.func.isRequired,
  sortBys: PropTypes.arrayOf(PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.number, PropTypes.string]))).isRequired,
