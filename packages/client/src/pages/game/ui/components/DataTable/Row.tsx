  ...
  return (
    ...
    >
      {row.data.map((value = null, i) => {
        ...
        const actualValue = value !== null && value.hasOwnProperty('value') ? value.value : value
        ...
        const singleButton = actualValue && actualValue.type === 'button' && actualValue.props.onClick

        // Expand clickable area of checkboxes/buttons to the whole td
        if (singleCheckbox || singleButton) {
          props.onClick = (event: MouseEvent) => {
            if (event.target && (event.target as any).tagName === 'TD' && !actualValue.props.disabled) {
              if (singleCheckbox) {
                actualValue.props.onChange()
              } else {
