...
const BoxScoreRow = ({
  ...
  const showDNP = p.min === 0 && (!liveGameInProgress || (p.injury.gamesRemaining > 0 && !p.injury.playingThrough))

  const statCols = showDNP ? (
    <td colSpan={15} className="text-center">
      DNP - {p.injury.gamesRemaining === 0 || p.injury.playingThrough ? "Coach's decision" : p.injury.type}
    </td>
  ) : (
    <>
