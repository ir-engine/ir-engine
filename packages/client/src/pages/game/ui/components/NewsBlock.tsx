...
const Badge = ({ type }: { type: LogEventType }) => {
  ...
  return <span className={`badge badge-news m-2 ml-auto align-self-start ${className}`}>{text}</span>
}
...
const NewsBlock = ({
  ...
  if (event.tid !== undefined) {
    ...
    if (teamInfo) {
      const rosterURL = helpers.leagueUrl(['roster', `${teamInfo.abbrev}_${event.tid}`, season])

      teamName = (
        <>
          {teamInfo.imgURL || teamInfo.imgURLSmall ? (
            <a href={rosterURL} className="p-1 d-flex align-items-center" style={logoStyle}>
              <img className="mw-100 mh-100" src={teamInfo.imgURLSmall ?? teamInfo.imgURL} alt="" />
            </a>
          ) : null}
    ...
    teamName = event.tids.map((tid) => {
      ...
      const rosterURL = helpers.leagueUrl(['roster', `${teamInfo.abbrev}_${tid}`, season])
    ...
    <div className="card">
      <div
        className={classNames(
          ...
          event.tids && event.tids.includes(userTid) ? 'table-info' : 'card-header p-0'
        )}
      >
        {teamName}
      ...
      <div className="d-flex">
        {event.p && event.p.imgURL !== '/src/pages/game/ui/assets/img/blank-face.png' ? (
          <div
            style={{
              maxHeight: 90,
              width: 60,
