...
type RowClassName<Value> = (a: { index: number; isDragged: boolean; value: Value }) => string | undefined
...
const ReorderHandle = SortableHandle(
  ({ highlight, isDragged, selected }: { highlight: boolean; isDragged: boolean; selected: boolean }) => {
    return (
      <td
...
const Row = SortableElement(
  (props: {
    ...
    const { className, disabled2, highlight, i, isDragged, row, selected, value } = props
    ...
    return (
      ...
      >
        {disabled2 ? null : <ReorderHandle highlight={highlight} isDragged={isDragged} selected={selected} />}
        {row({
          index: i,
          value
        })}
    ...
    return (
      <tbody>
        {values.map((value, index) => {
          const className: string | undefined = rowClassName ? rowClassName({ index, isDragged, value }) : undefined
