...
const RecordAndPlayoffs = ({
  ...
  const seasonText =
    option !== 'noSeason' ? (
      <span>
        <a href={helpers.leagueUrl(['roster', `${abbrev}_${tid}`, season])}>{season}</a>:{' '}
      </span>
    ) : null
  ...
  const recordText = <a href={helpers.leagueUrl(['standings', season])}>{record}</a>
  const extraText =
    numConfs !== undefined &&
    ...
    playoffRoundsWon >= 0 ? (
      <span>
        ...
        <a href={helpers.leagueUrl(['playoffs', season])}>
          {helpers.roundsWonText(playoffRoundsWon, numPlayoffRounds, numConfs).toLowerCase()}
        </a>
      </span>
    ) : null
  return (
