...
const PlayMenu = ({ lid, spectator, options }: Props) => {
  ...
  return (
    <Dropdown className={`play-button-wrapper${window.mobile ? ' dropdown-mobile' : ''}`} as={Nav.Item}>
      <Dropdown.Toggle
        className={classNames('play-button text-white', spectator ? 'bg-danger' : 'bg-success')}
        id="play-button"
        as={Nav.Link}
      >
        Play
        ...
        {options.map((option, i) => {
          return (
            <Dropdown.Item
              key={i}
              href={option.url}
              onClick={(event: MouseEvent<any>) => handleOptionClick(option, event)}
              className="kbd-parent"
            >
              {option.label}
              {option.key ? <span className="text-muted kbd">Alt+{option.key.toUpperCase()}</span> : null}
            </Dropdown.Item>
          )
        })}
      </Dropdown.Menu>
