...
const Mood = ({ className, defaultType, maxWidth, p }: Props) => {
  ...
  const initialType = defaultType === 'current' && canShowCurrent ? 'current' : 'user'
  ...
  const modalHeader = <a href={helpers.leagueUrl(['player', p.pid])}>{p.name}</a>
  let modalBody = null
  if (mood) {
    ...
    modalBody = (
      <>
        {mood && mood.traits.length > 0 ? (
          <p className="mb-2">Priorities: {mood.traits.map((trait) => MOOD_TRAITS[trait].toLowerCase()).join(', ')}</p>
        ) : null}
            ...
            {helpers.keys(componentsRounded).map((key) => {
              ...
              return (
                <tr key={key} className={highlightColor(componentsRounded[key])}>
                  <td className="text-right p-0">{plusMinus(componentsRounded[key])}</td>
    ...
    return (
      <button
        className={classNames(
          'btn btn-xs d-flex',
          className,
          initialType === 'user' ? 'btn-light-bordered-primary' : 'btn-light-bordered',
          {
            'w-100': maxWidth
          }
        )}
        onClick={onClick}
      >
        <span className={`text-right ${highlightColor(sum)}`} data-no-row-highlight="true" style={plusMinusStyle}>
          {plusMinus(sum)}
        </span>
