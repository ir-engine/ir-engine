...
const RosterSalarySummary = ({
  ...
  return (
    <div className="mb-3">
      ...
      <b className={actualCapSpace > 0 ? 'text-success' : undefined}>{helpers.formatCurrency(actualCapSpace, 'M')}</b>{' '}
      in cap space
      {capSpace < 0 ? (
        <>
          {' '}
          (<b className="text-danger">{helpers.formatCurrency(Math.abs(capSpace), 'M')}</b> over the cap)
        </>
      ) : null}
      .{' '}
      <HelpPopover title="Cap Space">
        <p>"Cap space" is the difference between your current payroll and the salary cap.</p>
        <p>
          {hardCap
            ? 'You '
