...
import React, { memo, ReactNode, useCallback, useEffect, useRef, useState, MouseEvent } from 'react'
import { helpers, localActions, menuItems, useLocalShallow } from '../util'
import type { MenuItemLink, MenuItemHeader, MenuItemText } from '../../common/types'
...
const MenuGroup = ({ children }: { children: ReactNode }) => <ul className="nav flex-column">{children}</ul>

MenuGroup.propTypes = {
  children: PropTypes.any.isRequired
}

const makeAnchorProps = (menuItem: MenuItemLink, onMenuItemClick: () => void) => {
  let href
  let rel
  let target
...
const MenuItem = ({
  ...
  if (menuItem.type === 'header') {
    const children = menuItem.children
      .map((child, i) => (
        <MenuItem key={i} lid={lid} menuItem={child} onMenuItemClick={onMenuItemClick} pageID={pageID} root={false} />
      ))
...
const SideBar = memo(({ pageID }: Props) => {
  ...
  const { lid, sidebarOpen } = useLocalShallow((state) => ({
    lid: state.lid,
  ...
  return (
    <>
      ...
      <nav className="bg-light sidebar" id="sidebar" ref={getNode} aria-label="side navigation">
        <div className="sidebar-sticky">
          {menuItems.map((menuItem, i) => (
            <MenuItem key={i} lid={lid} menuItem={menuItem} onMenuItemClick={closeHandler} pageID={pageID} root />
          ))}
        </div>
      </nav>
    </>
