...
const ScoreBox = ({
  ...
  const { challengeNoRatings, homeCourtAdvantage, season, teamInfoCache, userTid } = useLocalShallow((state) => ({
  ...
  if (game.teams[0].pts !== undefined && game.teams[1].pts !== undefined) {
    if (game.teams[0].pts > game.teams[1].pts) {
      ...
      winner = 1
    } else if (typeof game.teams[1].pts === 'number' && game.teams[1].pts === game.teams[0].pts) {
      winner = -1
    }
  }
  ...
  const hasOvrs = !small && !challengeNoRatings && game.teams[0].ovr !== undefined && game.teams[1].ovr !== undefined

  const userInGame = userTid === game.teams[0].tid || userTid === game.teams[1].tid

  let spreads: [string | undefined, string | undefined] | undefined
  if (game.teams[0].ovr !== undefined && game.teams[1].ovr !== undefined && (!small || !final)) {
    let spread

    // From @nicidob https://github.com/nicidob/bbgm/blob/master/team_win_testing.ipynb
    // Default homeCourtAdvantage is 1
    spread = roundHalf((2 / 5) * (game.teams[0].ovr - game.teams[1].ovr) + 3.3504 * homeCourtAdvantage)
    ...
    >
      {header ? (
        <div className="d-flex justify-content-end score-box-header text-muted">
          ...
          {spreads ? (
            <div
              className={classNames('text-right p-1', small ? 'score-box-score' : 'score-box-spread')}
      ...
      >
        {tradeDeadline ? (
          <div className="score-box-deadline p-1 d-flex align-items-center">Trade deadline</div>
        ) : allStarGame && !final ? (
          [1, 2].map((i) => (
            <div className="d-flex align-items-center" key={i}>
              <div className="score-box-logo" />
              <div className={classNames('p-1', { 'pr-5': small })}>
                <a href={helpers.leagueUrl(['all_star_draft'])}>{small ? `AS${i}` : `All-Star Team ${i}`}</a>
              </div>
            </div>
          ))
        ) : (
          [1, 0].map((i) => {
            ...
            if (allStarGame) {
              imgURL = undefined
              teamName = small ? `AS${i === 0 ? 2 : 1}` : `All-Star Team ${i === 0 ? 2 : 1}`
              rosterURL = helpers.leagueUrl(['all_star_history'])
            } else {
              imgURL = teamInfoCache[t.tid]?.imgURLSmall ?? teamInfoCache[t.tid]?.imgURL
              teamName = small
                ? teamInfoCache[t.tid]?.abbrev
                : `${teamInfoCache[t.tid]?.region} ${teamInfoCache[t.tid]?.name}`
              rosterURL = helpers.leagueUrl(['roster', `${teamInfoCache[t.tid]?.abbrev}_${t.tid}`])
            }

            // For @MikeHoudini on Discord
            const userTeamClass = t.tid === userTid && final ? 'user-team' : undefined

            return (
              <div key={i} className={classNames('d-flex align-items-center', scoreClass, userTeamClass)}>
                {imgURL || allStarGame ? <TeamLogoInline imgURL={imgURL} style={{ marginLeft: 1 }} /> : null}
                <div className="flex-grow-1 p-1 text-truncate">
                  {t.playoffs ? <span className="text-dark">{t.playoffs.seed}. </span> : null}
                  <a href={rosterURL}>{teamName}</a>
                  {!small ? getRecord(t) : null}
                </div>
                {hasOvrs ? <div className="p-1 text-right">{t.ovr}</div> : null}
                {spreads ? (
                  <div className={classNames('text-right p-1 pr-2', small ? 'score-box-score' : 'score-box-spread')}>
                    {spreads[i]}
                  </div>
                ) : null}
                {final ? (
                  <div
                    className={classNames('score-box-score p-1 text-right font-weight-bold', scoreClass, userTeamClass)}
                  >
                    <a
                      href={helpers.leagueUrl([
                        'game_log',
                        allStarGame ? 'special' : `${teamInfoCache[t.tid]?.abbrev}_${t.tid}`,
                        gameSeason,
                        game.gid
                      ])}
                    >
      ...
      {small && overtimes ? (
        <div className="text-right text-muted px-1 d-flex align-items-center" style={{ height: 28 }}>
          {overtimes}
        </div>
      ) : null}
      {!small && overtimes ? <div className="text-right text-muted p-1">{overtimes}</div> : null}
    </div>
  )
  ...
  if (actionText) {
    return (
      <div className={classNames('d-flex', className)}>
        {scoreBox}
        <button
          className={classNames('btn score-box-action', actionHighlight ? 'btn-success' : 'btn-secondary')}
