...
const TopMenuToggle = ({ long, openID, short, toggle }: TopMenuToggleProps) => {
  ...
  return (
    <Dropdown.Toggle as={Nav.Link} id="whatever" onMouseEnter={handleMouseEnter}>
...
const TopMenuDropdown = ({
  ...
  return (
    <Dropdown show={openID === long} onToggle={toggle} as={Nav.Item}>
      <TopMenuToggle long={long} short={short} openID={openID} toggle={toggle} />
      <Dropdown.Menu alignRight>
        {!hideTitle ? <Dropdown.Header className="d-none d-sm-block d-md-none">{long}</Dropdown.Header> : null}
        {children}
      </Dropdown.Menu>
    </Dropdown>
  )
...
const MenuItem = ({
  ...
  if (menuItem.type === 'link') {
    ...
    return <Dropdown.Item {...anchorProps}>{getText(menuItem.text)}</Dropdown.Item>
  }
...
const DropdownLinks = memo(({ className, hideTitle, inLeague, lid, menuItems }: DropdownLinksProps) => {
  ...
  return (
    <Nav navbar className={classNames(className, 'dropdown-links navbar-nav')}>
