...
export const ChannelConnectionService = {
  provisionChannelServer: (instanceId?: string, channelId?: string) => {
  ...
  connectToChannelServer: (channelId: string, isHarmonyPage?: boolean) => {
    return async (dispatch: Dispatch, getState: any): Promise<any> => {
      try {
  ...
  resetChannelServer: () => {
    return async (dispatch: Dispatch): Promise<any> => {
      ...
      dispatch(ChannelConnectionAction.channelServerDisconnected())
    }
  }
}
