...
export const mediaStreamReducer = (_, action: MediaStreamActionType) => {
  Promise.resolve().then(() => mediaStreamReceptor(action))
  return state.attach(Downgraded).value
}

function mediaStreamReceptor(action: MediaStreamActionType): any {
  state.batch((s) => {
    switch (action.type) {
