...
const ieee754 = {read: function (buffer, offset, isLE, mLen, nBytes) {
  let e, m
...
class HttpRangeFetcher {
  ...
  async getRange(key, position = 0, length, options = {}) {
    if (length === undefined) {
      ...
      if (stat.size === undefined) throw new Error(`length not specified, and could not determine size of the remote file`);
    ...
    const lastChunk = Math.floor((position + length - 1) / this.chunkSize); // fetch them all as necessary
  ...
  _makeBuffer(chunkResponses, chunksOffset, length) {
    ...
    let trimEnd = first.length + buffers.reduce((sum, buf) => sum + buf.length, 0) + last.length - length;
  ...
  async _getChunk(key, chunkNumber, requestOptions) {
    ...
    let fetchEnd = fetchStart + this.chunkSize; // clamp the end of the fetch to the size if we have a cached size for the file
    ...
    const freshChunk = await freshPromise; // gather the stats for the file from the headers
...
function crossFetchBinaryRange(url, start, end, options = {}) {
  ...
  return fetch(url, fetchOptions).then(res => {
    ...
    const bufPromise = res.buffer ? res.buffer() : res.arrayBuffer().then(arrayBuffer => Buffer.from(arrayBuffer)); // return the response headers, and the data buffer
...
function parseCacheControl(field) {
  ...
  const parsed = {};
  const invalid = field.toLowerCase().replace( // eslint-disable-next-line no-control-regex,no-useless-escape
...
class CacheSemantics {
  ...
  calculateChunkExpirationDate(chunkResponse) {
    ...
    const basePlus = ttl => new Date(baselineDate.getTime() + ttl); // results that are not really cacheable expire after the minimum time to live
...
class AggregatingFetcher {
  ...
  _aggregateAndDispatch() {
    Object.entries(this.requestQueues).forEach(([url, requests]) => {
      ...
      const requestsToDispatch = []; // look to see if any of the requests are aborted, and if they are, just
      ...
      for (let i = 0; i < requestsToDispatch.length; i += 1) {
        ...
        if (currentRequestGroup && this._canAggregate(currentRequestGroup, next)) {
          ...
          if (currentRequestGroup) this._dispatch(currentRequestGroup); // and start on a new one
