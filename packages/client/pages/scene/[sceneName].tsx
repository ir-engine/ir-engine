...
const EditorRoomPage = (props: Props) => {
  const { sceneName } = useRouter().query as any;
  ...
  useEffect(() => {
    ...
    if (currentLocation.id != null &&
      ...
      provisionInstanceServer(currentLocation.id, undefined, sceneName);
  }, [locationState]);
      ...
      if (instanceId != null) {
        client.service('instance').get(instanceId)
          .then((instance) => {
            ...
            provisionInstanceServer(instance.locationId, instanceId, sceneName);
          });
      }
    }
  }, [appState]);
