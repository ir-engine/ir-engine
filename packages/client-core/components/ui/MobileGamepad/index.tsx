...
import { GamepadAxes, GamepadButtons } from "@xr3ngine/engine/src/input/enums/InputEnums";
...
export const MobileGamepad: FunctionComponent<MobileGamepadProps> = ({ hovered }: MobileGamepadProps) => {
  ...
  useEffect(() => {
    ...
    stickLeft.on("end", ( e, data) => {
      const event = new CustomEvent("stickmove", { "detail": { stick: GamepadAxes.Left, value: { x: 0, y: 0, angleRad: 0 } } });
      document.dispatchEvent(event);
    });
