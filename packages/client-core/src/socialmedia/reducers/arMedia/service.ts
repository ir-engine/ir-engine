...
import {
  ...
  addAdminArMedia,
  removeArMediaItem,
  fetchingArMediaItem, retrievedArMediaItem
} from './actions';
...
export function createArMedia(mediaItem:any, files: any){
  return async (dispatch: Dispatch): Promise<any> => {
    try {
      ...
      const newItem = await client.service('ar-media').create({...mediaItem, manifestId: manifest.file_id, audioId: audio.file_id, dracosisId: dracosis.file_id, previewId: preview.file_id });
