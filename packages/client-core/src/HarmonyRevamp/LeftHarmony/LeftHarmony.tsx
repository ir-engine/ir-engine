...
const LeftHarmony = (props: Props) => {
  ...
  React.useEffect(() => {
    if (chatState.targetObjectType.value === 'party' && chatState.targetObjectId?.value !== partyState.party.id?.value)
      setShowChat(false)
  }, [partyState.party.id?.value])

  React.useEffect(() => {
    if (
      chatState.targetObjectType.value === 'group' &&
      !groupSubState.groups.value.find((group) => group.id === chatState.targetObjectId?.value)
    )
      setShowChat(false)
  }, [groupSubState.groups.value])

  React.useEffect(() => {
    if (
      chatState.targetObjectType.value === 'user' &&
      !friendSubState.friends.value.find((friend) => friend.id === chatState.targetObjectId?.value)
    )
      setShowChat(false)
  }, [friendSubState.friends.value])
    ...
    <>
      ...
      {create && (
        <Dialog fullWidth={true} maxWidth={'md'} open={create} onClose={() => handleCloseModal()}>
          <InviteModal handleCloseModal={handleCloseModal} invite={invite} selfUser={selfUser} party={party} />
        </Dialog>
      )}
