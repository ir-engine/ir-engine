...
const SentInvite = (props: Props) => {
  ...
  const rows = invites.map((el, index) => createData(el.id, el.invitee ? el.invitee.name : "", el.groupName, el.token, el.passcode, el.inviteType));
        ...
        <TableBody>
          {(rowsPerPage > 0
            ...
            <TableRow key={`index_${index}`}>
              ...
              <TableCell align="right"><a href="#h" onClick={() => deleteInvite(row)}> <Delete className="text-danger" /> </a></TableCell>
            </TableRow>
          ))}
          {emptyRows > 0 && (
