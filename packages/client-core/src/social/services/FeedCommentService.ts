    ...
    switch (action.type) {
      ...
      case 'ADD_FEED_COMMENT_FIRES':
        return s.merge({
          feeds: {
            ...s.feeds.value,
            feedComments: s.feeds.feedComments.value.map((item) => {
      ...
      case 'ADD_FEED_COMMENT':
        return s.merge({
          feeds: {
            ...s.feeds.value,
            feedComments: [action.comment, ...(s.feeds.feedComments.value || [])]
          }
        })
      case 'COMMENT_FIRES':
        return s.merge({
