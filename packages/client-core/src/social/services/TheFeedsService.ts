...
store.receptors.push((action: TheFeedsActionType): any => {
  state.batch((s) => {
    switch (action.type) {
      ...
      case 'ADD_THEFEEDS':
        return s.merge({ thefeeds: [...s.thefeeds.value, action.thefeeds] })
      case 'UPDATE_THEFEEDS':
        return s.merge({
          thefeeds: s.thefeeds.value.map((thefeeds) => {
      ...
      case 'REMOVE_THEFEEDS':
        return s.merge({
          thefeeds: [...s.thefeeds.value.filter((thefeeds) => thefeeds.id !== action.thefeeds)]
        })
    }
  }, action.type)
})
