...
const chatReducer = (state = immutableState, action: ChatAction): any => {
  ...
  switch (action.type) {
    case LOADED_CHANNELS:
      ...
      return state.set('channels', updateMap)
    case LOADED_CHANNEL:
      localAction = action as LoadedChannelsAction
      ...
      updateMap.set('channels', updateMapChannels)
      returned = state.set('channels', updateMap)
      if (channelType === 'instance')
        returned = returned.set('fetchingInstanceChannel', false).set('instanceChannelFetched', true)
      return returned
    case CREATED_MESSAGE:
      ...
      if (updateMapChannelsChild == null) {
        ...
        updateMapChannelsChild.messages =
          ...
          updateMapChannels.get('updateNeeded') === true
            ? [localAction.message]
      ...
      returned = state.set('channels', updateMap).set('updateMessageScroll', true)
