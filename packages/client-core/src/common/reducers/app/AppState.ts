...
export const AppReducer = (_, action: AppActionType) => {
  Promise.resolve().then(() => appReceptor(action))
  return state.attach(Downgraded).value
}

const appReceptor = (action: AppActionType): void => {
  state.batch((s) => {
    switch (action.type) {
      case 'SET_APP_LOADED':
...
export const useAppState = () => useState(state)
