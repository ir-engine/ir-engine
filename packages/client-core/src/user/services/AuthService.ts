...
export const registerAuthServiceActions = () => {
  registerState(AuthState)
  addActionReceptor(function AuthServiceReceptor(action) {
...
accessAuthState().attach(() => ({
  ...
  init: () => ({
    onSet(arg) {
      ...
      if (state.isLoggedIn) {
        dispatchAction(
          StoredLocalAction.storedLocalAction({
            newState: {
