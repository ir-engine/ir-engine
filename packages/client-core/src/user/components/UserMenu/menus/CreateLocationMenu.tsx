...
const CreateLocationMenu = ({ location, changeActiveMenu,updateLocationDetail }) => {
    ...
    const getLocationTypes = () => {
        return client.service('location-type').find().then(res => {
            if (res && res.data && Array.isArray(res.data)) {
    ...
    const deleteLocation = () => {
        client.service('location').remove(location.id).then(_ => {
    ...
    const validate = () => {
        ...
        if (location.maxUsersPerInstance != null && isNaN(location.maxUsersPerInstance)) (error as any).maxUsersPerInstance = t('user:usermenu.newLocation.validNumber');
			...
			<section className={styles.locationPanel}>
                ...
                <FormControl className={styles.formControl} size="small">
                    ...
                    <Select
                        ...
                        onChange={handleChange}
                    >
                        {scenes.map(scene => <MenuItem key={scene.sid} value={scene.sid}>{`${scene.name} (${scene.sid})`}</MenuItem>)}
                    </Select>
                </FormControl>
                <FormControl className={styles.formControl} size="small">
                ...
                {location.id && <Tooltip title={location.isLobby ? t('user:usermenu.newLocation.tooltipCanNotBeDeleted') : ''} arrow placement="bottom" className={styles.tooltip}>
