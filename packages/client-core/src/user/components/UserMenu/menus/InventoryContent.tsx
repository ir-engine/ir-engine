  ...
  return (
    ...
    <Box sx={{ p: 2 }} className={`${classes.root} ${classes.contents}`}>
      ...
      {data.length !== 0 ? <Grid container spacing={2} className={`${classes.p10} ${classes.contents}`}>
            ...
            {inventory.length !== 0 ? <Stack>
              {inventory.map((value: any, index: number) => (
                <Card key={index} onClick={() => {
                  ...
                  <Stack justifyContent="center" alignItems="center" className={`${selectedid === value.user_inventory.userInventoryId ? classes.selecteditem : ""}`}>
        ...
        <Grid item md={6}>
          {url !== "" && metadata.length !== 0 && <Stack justifyContent="center" alignItems="center">
                ...
                {metadata.length !== 0 && <>
                  {JSON.parse(metadata).map((val, index) => (
                    <Grid item key={index} xs={6} md={6}>
                      ...
                      <Stack>{(val.trait_type !== 'personality' && val.trait_type !== 'age') ?
            ...
            <Stack justifyContent="center" alignItems="center" spacing={1} direction="row" className={classes.p10}>
              ...
              <Button variant="outlined" disabled={isLoadingtransfer} onClick={() => handleTransfer(userid, selectedid)}>{isLoadingtransfer ? <CircularProgress size={30} /> : "Transfer"}</Button>
            </Stack>
          </Stack>}
          {url === "" && metadata.length === 0 && <Stack sx={{ color: 'black' }}><Typography>No item selected</Typography></Stack>}
        </Grid>
