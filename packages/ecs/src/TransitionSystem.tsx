...
function getPropertyTypeFromSchema(componentJsonID: ComponentJsonID, propertyPath: ComponentPropertyPath) {
  ...
  if (!component) return
  const schemaPropertyPath = propertyPath.replaceAll('.', '.properties.')
  const schema = resolveObject(component.schema?.properties, schemaPropertyPath)
  if (schema[Kind] === 'Object' || schema[Kind] === 'Class') return 'Object'
}
