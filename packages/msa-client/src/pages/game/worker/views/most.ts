...
export const getMostXPlayers = async ({
	...
	const stats = ["gp", "min", "pts", "trb", "ast", "per", "ewa", "ws", "ws48"];
...
const updatePlayers = async (
	...
	if (
		...
		if (type === "at_pick") {
			...
			description = "These are the undrafted players or second round picks who had the best careers.";
			extraCols.push({
				key: ["most", "extra"],
				colName: "Team",
			});
			...
			filter = p =>
				p.draft.round === 0 || p.draft.round >= 2;
			getValue = p => {
				const value = playerValue(p).value;
