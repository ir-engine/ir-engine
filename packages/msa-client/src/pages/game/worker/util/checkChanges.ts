...
const checkChanges = async (conditions: Conditions) => {
	...
	if (CURRENT_VERSION > lastChangesVersion) {
		const changes = (await fetchWrapper({
			url: "https://bbgm.com/changelog.php",
			method: "GET",
			data: {
		...
		if (changes && changes.length > 0) {
				...
				if (links) {
					...
					if (link) {
						...
						if (link.startsWith("/basketball/")) {
							...
							url = link.replace("/", "https://bbgm.com/");
						} else {
							url = link;
						}
			...
			text += `<p class="mt-1 mb-0"><a href="https://bbgm.com/changelog/" rel="noopener noreferrer" target="_blank">${moreText}</a></p>`;
