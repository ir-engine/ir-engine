...
const Gallery = () => {
  ...
  const updateTokensOnSale = useAppState(useCallback(({ updateTokensOnSale }) => updateTokensOnSale, []))
  ...
  useEffect(() => {
    if (tokensOnSale) setTokeata([...tokensOnSale])
  }, [tokensOnSale])
  ...
  const filterData = (opt: string) => {
    ...
    // }
    backUpPData = backUpPData?.sort((a: any, b: any) =>
      opt === 'asc'
        ? BigNumber.from(a.price)
            .toString()
            .localeCompare(BigNumber.from(b.price).toString(), undefined, { numeric: true })
    ...
    <Box>
      ...
      <div>
        <Accordian title="TRAIT">
          {traitcategories &&
            Object.keys(traitcategories).map((ky: any, index: number) => {
              return (
                <div
                  style={{ display: 'flex', paddingBottom: '8px', borderBottom: '1px solid grey', paddingTop: '9px' }}
                >
                  ...
                  <Button onClick={() => setOrder('price')} variant="filter" disabled={order === 'price'}>
                    ...
                    {traitcategories[ky].map((data: any, ix: number) => (
                      <div
                        onClick={(e) => selectedTrait(ky, data, ix)}
                        style={{
                          ...
                          fontSize: '15px',
                          borderRadius: '25px',
                          backgroundColor: data.clicked === true ? '#0074cc' : '#524f4f',
                          cursor: 'pointer'
                        }}
                      >
                        {data.value}
