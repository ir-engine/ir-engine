...
export async function up(knex: Knex): Promise<void> {
  ...
  if (tableExists === false) {
    ...
    await trx.schema.createTable(locationAuthorizedUserPath, (table) => {
      //@ts-ignore
...
export async function down(knex: Knex): Promise<void> {
  ...
  const tableExists = await trx.schema.hasTable(locationAuthorizedUserPath)

  if (tableExists === true) {
    await trx.schema.dropTable(locationAuthorizedUserPath)
  }
