...
import { projectPublishPath } from '@ir-engine/common/src/schemas/projects/project-publish.schema'
...
export async function up(knex: Knex): Promise<void> {
  ...
  const tableExists = await knex.schema.hasTable(projectPublishPath)

  if (!tableExists) {
    await knex.schema.createTable(projectPublishPath, (table) => {
      //@ts-ignore
...
export async function down(knex: Knex): Promise<void> {
  ...
  if (tableExists === true) {
    await knex.schema.dropTable(projectPublishPath)
  }
