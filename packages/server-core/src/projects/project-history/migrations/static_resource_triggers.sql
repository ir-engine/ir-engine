...
DROP PROCEDURE IF EXISTS `delete_static_resource_history`;
CREATE PROCEDURE delete_static_resource_history (
  IN projectName VARCHAR(255), 
  IN staticResourceId CHAR(36), 
  IN staticResourceType VARCHAR(255),
  IN url VARCHAR(255), 
...
sp: BEGIN
  ...
  ELSE
    SET actionType = 'RESOURCE_REMOVED';
  END IF;
...
DROP TRIGGER IF EXISTS `after_static_resource_delete`;
CREATE TRIGGER after_static_resource_delete
AFTER DELETE ON `static-resource`
FOR EACH ROW
BEGIN
  -- Call the stored procedure with the necessary parameters
  CALL delete_static_resource_history(
    OLD.project,  -- projectName
    OLD.id,  -- staticResourceId
    OLD.type,  -- staticResourceType
    OLD.key,  -- url
