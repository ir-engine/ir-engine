...
import { MaterialInterface } from '@etherealengine/common/src/interfaces/MaterialInterface'
...
export class Material extends Service<MaterialInterface> {
  app: Application
  public docs: any
  ...
  // @ts-ignore
  async create(data: CreateMaterialType, params?: UserParams): Promise<MaterialInterface> {
    const self = this
    const query = {
      $select: ['id']
    } as any
  ...
  async find(params?: Params): Promise<MaterialInterface[] | Paginated<MaterialInterface>> {
  ...
  async remove(id: string, params?: UserParams): Promise<MaterialInterface> {
    ...
    return (await super.remove(id)) as MaterialInterface
  }
}
