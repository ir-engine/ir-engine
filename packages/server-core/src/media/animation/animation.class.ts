...
import { AnimationInterface } from '@etherealengine/common/src/interfaces/AnimationInterface'

import { Application } from '../../../declarations'
import { UserParams } from '../../user/user/user.class'
import { NotFoundException, UnauthenticatedException } from '../../util/exceptions/exception'

export type CreateAnimationType = {
  name?: string
  tags?: string[]
  duration?: number
  src?: string
...
export class Animation extends Service<AnimationInterface> {
  app: Application
  public docs: any
  ...
  // @ts-ignore
  async create(data: CreateAnimationType, params?: UserParams): Promise<AnimationInterface> {
    const self = this
    const query = {
      $select: ['id']
    } as any
  ...
  async find(params?: Params): Promise<AnimationInterface[] | Paginated<AnimationInterface>> {
  ...
  async remove(id: string, params?: UserParams): Promise<AnimationInterface> {
    ...
    return (await super.remove(id)) as AnimationInterface
  }
}
