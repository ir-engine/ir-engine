...
export class S3IPFSProvider implements StorageProviderInterface {
  bucket = config.ipfs.fleekKeys.bucket
  cacheDomain = `${config.ipfs.fleekKeys.storageDomain}/${config.ipfs.fleekKeys.bucket}/`
  //cacheDomain = `${config.ipfs.fleekKeys.bucket}.${config.ipfs.fleekKeys.storageDomain}/`
      ...
      if (query) {
        promises.push(
          new Promise(async (resolve) => {
            const url = `https://${this.cacheDomain}${key}`
