...
import { CubemapInterface } from '@etherealengine/common/src/interfaces/CubemapInterface'

import { Application } from '../../../declarations'
import { UserParams } from '../../user/user/user.class'
import { NotFoundException, UnauthenticatedException } from '../../util/exceptions/exception'

export type CreateCubemapType = {
  name?: string
  tags?: string[]
  duration?: number
  src?: string
...
export class Cubemap extends Service<CubemapInterface> {
  app: Application
  public docs: any
  ...
  // @ts-ignore
  async create(data: CreateCubemapType, params?: UserParams): Promise<CubemapInterface> {
    const self = this
    const query = {
      $select: ['id']
    } as any
  ...
  async find(params?: Params): Promise<CubemapInterface[] | Paginated<CubemapInterface>> {
  ...
  async remove(id: string, params?: UserParams): Promise<CubemapInterface> {
    ...
    return (await super.remove(id)) as CubemapInterface
  }
}
