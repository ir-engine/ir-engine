  ...
  return (
    <div className={`flex flex-col gap-y-2 ${fullWidth ? 'w-full' : 'w-fit'}`}>
      ...
      >
        ...
        <div
          className={twMerge(
            'flex w-full items-center gap-x-2 rounded-md border-[0.5px] border-ui-outline bg-white text-xs placeholder-text-tertiary transition-colors duration-300 dark:bg-ui-background',
            heights[height],
            disabled
        ...
        >
          <input
            spellCheck={false}
            className="placeholder-text-[#616161] dark:placeholder-text-text-tertiary peer order-2 h-full w-full bg-inherit pt-0.5 outline-none autofill:bg-inherit"
            ref={ref}
            id={inputId}
            disabled={disabled}
            readOnly={readOnly}
