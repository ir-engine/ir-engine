...
export interface ToolbarProps {
  ...
  cycleCamera: () => void
  drawBody: (val: boolean) => void
  isDrawingBody: boolean
  drawFace: (val: boolean) => void
  isDrawingFace: boolean
  drawHands: (val: boolean) => void
      ...
      <div className="flex-1">
        <div className="btn-group">
          <button
            ...
            className={twMerge('btn', detectingStatus === 'active' && 'btn-active')}
            onClick={() => {
              if (detectingStatus !== 'loading') toggleDetecting()
            }}
          >
            {detectingStatus === 'loading' ? (
              <LoadingCircle className="block w-30 h-30" message="" />
            ) : (
              <svg
                version="1.0"
