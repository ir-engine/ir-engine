...
export const BallControllerComponent = defineComponent({
  ...
  schema: S.Object({
    ...
    maxSpeed: S.Number(5), // Maximum speed the ball can reach
    direction: T.Vec3(), // Current movement direction
    boostMultiplier: S.Number(1.5), // Speed multiplier when boost is active
    linearDamping: S.Number(0.2), // Damping factor for linear velocity (0-1)
    angularDamping: S.Number(0.1) // Damping factor for angular velocity (0-1)
  })
})
