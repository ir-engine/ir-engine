...
import { Types, UUIDComponent } from '@ir-engine/ecs'
...
export const getHandTarget = (entity: Entity, hand: XRHandedness): HandTargetReturn => {
  ...
  const targetEntity = NameComponent.entitiesByName[networkComponent.ownerId + '_' + hand].values().next().value // todo, how should be choose which one to use?
