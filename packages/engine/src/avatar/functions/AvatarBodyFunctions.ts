import { Bone, Euler, Matrix4, Object3D, Quaternion, Scene, Skeleton, SkeletonHelper, Vector3 } from 'three';
...
const getModel = ((entity) => {
  const character = getMutableComponent(entity, CharacterComponent);
 
  console.log("Character container is ");
  console.log(character.modelContainer);

  let scene;

  // if (character.modelContainer) {
  //   console.log("o.character.modelContainer.scene");
    
  //   scene = character.modelContainer;
  // } else {
    scene = new Scene();
...
export const retargetSkeleton = (entity, model, options) => {
  ...
  console.log("Skinned meshes length is", skinnedMeshes.length);
  ...
  const helper = new SkeletonHelper( skinnedMeshes[0].skeleton.bones[0] );
  
  Engine.scene.add( helper );

  const modelBones = createModelBones(skeleton);

  console.log("model boesn are");
  ...
  console.log("armatureDirection", armatureDirection)
...
export const initializeAvatarRig = (entity, model) => {
  ...
  if(!hasComponent(entity, CharacterComponent) || !getComponent(entity, CharacterComponent).modelContainer)
    return setTimeout(() => initializeAvatarRig(entity, model), 100);
