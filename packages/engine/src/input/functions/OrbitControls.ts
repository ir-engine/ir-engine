...
export class OrbitControls extends EventDispatcher {
  ...
  private readonly updateLastPosition: Vector3
  private readonly updateOffset: Vector3
  private readonly updateQuat: Quaternion
  private readonly updateLastQuaternion: Quaternion
  private readonly updateQuatInverse: Quaternion

  private readonly panLeftV: Vector3
  private readonly panUpV: Vector3
  private readonly panInternalOffset: Vector3
  ...
  update () {
    ...
    (this.spherical as (any)).theta = Math.max(this.minAzimuthAngle, Math.min(this.maxAzimuthAngle, (this.spherical as any).theta));
  ...
  pan (deltaX: number, deltaY: number) {
    ...
    if (this._checkPerspectiveCamera(this.object)) {
      ...
      // orthographic
      this.panLeft(deltaX * ((this.object as any).right - (this.object as any).left) / (this.object as any).zoom / (element as any).clientWidth, (this.object as any).matrix)
      this.panUp(deltaY * ((this.object as any).top - (this.object as any).bottom) / (this.object as any).zoom / (element as any).clientHeight, (this.object as any).matrix)
    } else {
