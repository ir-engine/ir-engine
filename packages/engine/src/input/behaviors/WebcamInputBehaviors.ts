...
export const startLipsyncTracking = () => {
  ...
  audioProcessor.onaudioprocess = () => {
    ...
    for (let m = 0; m < BoundingFrequencyMasc.length - 1; m++) {
      for (let j = IndicesFrequencyMale[m]; j <= IndicesFrequencyMale[m + 1]; j++) { if (sensitivityPerPole[j] > 0) EnergyBinMasc[m] += sensitivityPerPole[j] }
      EnergyBinMasc[m] /= (IndicesFrequencyMale[m + 1] - IndicesFrequencyMale[m])
    }

    // Fem energy bin
    for (let m = 0; m < BoundingFrequencyFem.length - 1; m++) {
      for (let j = IndicesFrequencyMale[m]; j <= IndicesFrequencyMale[m + 1]; j++) { if (sensitivityPerPole[j] > 0) EnergyBinFem[m] += sensitivityPerPole[j] }
      EnergyBinMasc[m] /= (IndicesFrequencyMale[m + 1] - IndicesFrequencyMale[m])
