...
export class ActionSystem extends System {
  ...
  public execute (delta: number): void {
    ...
    this.queryResults.localClientInput.all?.forEach(entity => {
      ...
      receivedClientInput?.forEach((stateUpdate) => {
        ...
        input.data.forEach((value: any, key) => {
          if (value.type === InputType.BUTTON) { Network.instance.clientInputState.buttons.push({ input: key, value: value.value, lifecycleState: value.lifecycleState }) } else if (value.type === InputType.ONEDIM) { Network.instance.clientInputState.axes1d.push({ input: key, value: value.value, lifecycleState: value.lifecycleState }) } else if (value.type === InputType.TWODIM) { Network.instance.clientInputState.axes2d.push({ input: key, value: value.value, lifecycleState: value.lifecycleState }) } else if (value.type === InputType.SIXDOF) { Network.instance.clientInputState.axes6DOF.push({ input: key, value: value.value }) }
        })
    ...
    this.queryResults.localClientInput.added?.forEach(entity => {
      ...
      if (this._inputComponent === undefined) { return console.warn('Tried to execute on a newly added input component, but it was undefined') }
