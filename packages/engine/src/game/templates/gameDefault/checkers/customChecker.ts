  ...
  if (Array.isArray(entityArg)) {
    return entityArg.some((entityFinded) => {
      if (args.watchers != undefined) {
        if (
          args.watchers.some((componentArr) => componentArr.every((component) => hasComponent(entityFinded, component)))
        ) {
        } else {
          return false
        }
      ...
      if (args.checkers != undefined) {
        if (args.checkers.every((checker) => checker.function(entityFinded, checker.args, entityTarget))) {
        } else {
          return false
        }
      }
