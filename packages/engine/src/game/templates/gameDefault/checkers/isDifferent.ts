...
export const isDifferent: Checker = (entity: Entity, args?: any, entityTarget?: Entity): boolean => {
  ...
  if (!position || !positionStart) {
    console.error(
      'storage position came back undefined, something is not right',
      entity,
      getStorage(entity, TransformComponent)
    )
    return
  }
  ...
  const differensNow =
    Math.abs(positionStart.x - position.x) +
    Math.abs(positionStart.y - position.y) +
    Math.abs(positionStart.z - position.z)
