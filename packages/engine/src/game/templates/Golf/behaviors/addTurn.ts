...
export const addTurn: Behavior = (entityPlayer: Entity, args?: any, delta?: number, entityTarget?: Entity, time?: number, checks?: any): void => {
  ...
  const noOneTurn = Object.keys(game.gamePlayers).every(role => game.gamePlayers[role].every(entity => !(hasComponent(entity, State.YourTurn) || hasComponent(entity, State.Waiting))))
