...
export const setupPlayerInput = (entityPlayer: Entity) => {
  ...
  inputs.schema.behaviorMap.set(GolfInput.TELEPORT, (entity: Entity) => {
  ...
  if (isDev) {
    ...
    inputs.schema.behaviorMap.set(teleportballkey, (entity: Entity) => {
      const { gameName, ownedObjects } = getComponent(entity, GamePlayer)
    ...
    inputs.schema.behaviorMap.set(teleportballOut, (entity: Entity) => {
