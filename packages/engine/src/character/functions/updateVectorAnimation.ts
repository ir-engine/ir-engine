...
export const updateVectorAnimation = (entity, delta: number): void => {
  ...
  if (!animationComponent.mixer || (actor.modelContainer.children.length === 0)) return
...
*/
  ...
  animationsValues.forEach(value => {
    ...
    if (!animationComponent.currentAnimationAction.includes(action)) { animationComponent.currentAnimationAction.push(action) }
