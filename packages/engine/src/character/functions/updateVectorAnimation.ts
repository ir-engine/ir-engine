    ...
    for (let mi = 0; mi < animation.value.length - 1; mi++) {
      if (animation.value[mi] <= giveSpeed && giveSpeed <= animation.value[mi + 1]) {
        for (let ip = 0; ip < customProLength; ip++) {
          weight +=
            animationMapLinear(giveSpeed, animation.value, animation[animation.customProperties[ip]], mi) *
            (ip == 0 ? 1 - dontHasHit : dontHasHit)
        }
      }
    }
...
export const updateVectorAnimation = (entity, delta: number): void => {
  ...
  const objectValues = animationComponent.updateAnimationsValues(
    entity,
    animationComponent.animationsSchema,
    delta * animationSpeedMultiplier
  )
  // math to correct all animations
  const animationsValues = mathMixesAnimFromSchemaValues(
    entity,
    animationComponent.animationsSchema,
    objectValues,
    delta,
...
export const changeAnimation: Behavior = (entity, args: {}, deltaTime: number): void => {
  ...
  if (typeof avatarAnimation.loop !== 'undefined') newAction.setLoop(avatarAnimation.loop, Infinity)
  ...
  newAction.reset().setEffectiveWeight(1).setEffectiveTimeScale(1).fadeIn(0.01).play()
