...
export const copyValue = <T>(src: T, dest: T): T => {
  dest = src
  return src
}
...
export function types(defaults: object = { value: { type: Types.Ref } }): ComponentSchema {
  return fromEntries(
    Object.entries(defaults).map((name, value) => [
      name,
      { type: Types[capitalize(typeof value)] || Types.Ref, default: value }
    ])
  )
}
