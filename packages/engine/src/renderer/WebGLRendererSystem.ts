...
interface EffectComposerWithSchema extends EffectComposer {
  ...
  getRenderer: () => any
  replaceRenderer: (renderer, updateDOM) => any
  createDepthTexture: () => any
  deleteDepthTexture: () => any
  createBuffer: (depthBuffer, stencilBuffer, type, multisampling) => any
  addPass: (renderPass: any) => any
  removePass: () => any
  removeAllPasses: () => any
  render: (delta: number) => any
  setSize: (width: number, height: number, arg2: boolean) => any
  reset: () => any
  dispose: () => any
