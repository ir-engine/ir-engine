...
export default async function WebGLRendererSystem(world: World) {
  ...
  const setNavigationDebugActions = createActionQueue(EngineRendererAction.setNavigationDebug.matches)
  ...
  return () => {
    ...
    for (const action of setNavigationDebugActions()) EngineRendererReceptor.setNavigationDebug(action)
