...
export default class OutlinePass extends Pass {
  ...
  constructor (resolution, scene, camera, selectedObjects, editorRenderer) {
    ...
    this.renderTargetEdgeBuffer = new WebGLRenderTarget(
      this.resolution.x,
      this.resolution.y,
      pars
    )
  ...
  render (renderer, writeBuffer, readBuffer, delta, maskActive) {
    if (this.selectedObjects.length > 0) {
      ...
      this.renderScene.traverse((object) => {
        if (
          (object.isMesh ||
            ...
            object.isSprite ||
            object.isPoints) &&
          !this.selectedRenderables.includes(object)
        ) {
      ...
      (this.depthMaskMaterial as any).uniforms.depthTexture.value = this.renderTargetDepthBuffer.texture;
