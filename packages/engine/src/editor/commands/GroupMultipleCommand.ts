...
import { serializeObject3DArray, serializeObject3D } from "../../common/utils/debug";
import reverseDepthFirstTraverse from "../../common/utils/reverseDepthFirstTraverse";
export default class GroupMultipleCommand extends Command {
  ...
  execute() {
    this.groupNode = this.editor.groupMultiple(
      this.objects,
      this.groupParent,
      this.groupBefore,
      false
  ...
  toString() {
    return `GroupMultipleObjectsCommand id: ${
      this.id
    } objects: ${serializeObject3DArray(
      this.objects
    )} groupParent: ${serializeObject3D(
      this.groupParent
    )} groupBefore: ${serializeObject3D(this.groupBefore)}`;
  }
}
