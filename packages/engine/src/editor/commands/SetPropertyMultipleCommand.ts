...
import { serializeObject3DArray, serializeProperty } from "../../common/utils/debug";
export default class SetPropertyMultipleCommand extends Command {
  ...
  execute() {
    this.editor.setPropertyMultiple(
      this.objects,
      this.propertyName,
      this.newValue,
      false
  ...
  toString() {
    return `SetPropertyMultipleCommand id: ${
      this.id
    } objects: ${serializeObject3DArray(this.objects)} propertyName: ${
      this.propertyName
