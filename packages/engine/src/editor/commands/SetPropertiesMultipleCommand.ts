...
import { serializeProperties, serializeObject3DArray } from "../../common/utils/debug";
export default class SetPropertiesMultipleCommand extends Command {
  ...
  undo() {
    for (let i = 0; i < this.objects.length; i++) {
      this.editor.setProperties(
        this.objects[i],
        this.objectsOldProperties[i],
        false,
        false
      );
    }
  ...
  toString() {
    return `SetPropertiesMultipleCommand id: ${
      this.id
