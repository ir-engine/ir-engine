...
import isInputSelected from "../../common/utils/isInputSelected";
  ...
  for (const sourceKey in source) {
    if (Object.prototype.hasOwnProperty.call(source, sourceKey)) {
      ...
      if (sourceKey === "event") {
        ...
        state[targetKey] =
          state[targetKey] !== undefined ? state[targetKey] : { x: 0, y: 0 };
        if (resetPosition) {
      ...
      state[targetKey] =
        state[targetKey] !== undefined ? state[targetKey] : value;
      if (reset) {
        resetKeys.push(targetKey);
      }
    }
...
export default class InputManager {
      ...
      if (keyboard) {
        ...
        if (hotkeys) {
          for (const binding in hotkeys) {
            if (!Object.prototype.hasOwnProperty.call(hotkeys, binding))
              continue;
            (Mousetrap as any).unbind(binding);
          }
        ...
        if (globalHotkeys) {
          for (const binding in globalHotkeys) {
            if (!Object.prototype.hasOwnProperty.call(globalHotkeys, binding))
              continue;
              (Mousetrap as any).unbindGlobal(binding);
          }
    ...
    if (keyboard) {
      ...
      if (globalHotkeys) {
        for (const binding in globalHotkeys) {
          ...
          (Mousetrap as any).bindGlobal(binding, () => {
            state[action] = true;
            return false;
          });
          initialState[action] = false;
  ...
  reset() {
    for (let i = 0; i < this.resetKeys.length; i++) {
      ...
      // BUG: this.initialState might not be correct
      if (typeof actionState === "object" && typeof this.initialState === "object") {
