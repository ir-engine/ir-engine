...
function resize (imageWidth, imageHeight, thumbWidth, thumbHeight) {
  ...
  const heightRatio = imageHeight / thumbHeight
...
export async function generateMediaThumbnail (
  ...
  if (background === 'transparent') {
    return await getCanvasBlob(canvas, 'image/png')
  } else {
    return await getCanvasBlob(canvas)
  }
}
export async function getCanvasBlob (canvas, fileType = 'image/jpeg', quality = 0.9) {
  if (canvas.msToBlob) {
    return await Promise.resolve(canvas.msToBlob())
  } else {
    return await new Promise(resolve => canvas.toBlob(resolve, fileType, quality))
  }
}
