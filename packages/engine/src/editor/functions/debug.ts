...
export function serializeColor (value) {
  return `Color { r: ${value.r}, g: ${value.g}, b: ${
    value.b
  }, hex: #${value.getHexString()}`
}
export function serializeVector4 (value) {
  return `Vector4 { x: ${value.x}, y: ${value.y}, z: ${value.z}, w: ${
    value.w
  } }`
}
export function serializeQuaternion (value) {
  return `Quaternion { x: ${value.x}, y: ${value.y}, z: ${value.z}, w: ${
    value.w
  } }`
}
...
export function serializeProperties (properties) {
  ...
  for (const propertyName in properties) {
    if (Object.prototype.hasOwnProperty.call(properties, propertyName)) {
      debugProperties[propertyName] = serializeProperty(
