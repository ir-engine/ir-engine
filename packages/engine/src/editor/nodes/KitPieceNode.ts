...
export default class KitPieceNode extends EditorNodeMixin(Model) {
  ...
  static async deserialize(editor, json, loadAsync, onError) {
    ...
    loadAsync(
      (async () => {
        const { kitId, pieceId, subPiecesConfig } = json.components.find(
          c => c.name === "kit-piece"
        ).props;
        await node.load(
  ...
  getMaterialIdForMaterialSlot(subPieceId, materialSlotId) {
    ...
    const materialSlot =
      subPiece && subPiece.materialSlots.find(m => m.id === materialSlotId);
    return materialSlot && materialSlot.value
      ? materialSlot.value.id
      : undefined;
  }
  async loadMaterialSlot(subPieceId, materialSlotId, materialId) {
    ...
    const materialSlot = subPiece.materialSlots.find(
      m => m.id === materialSlotId
    );
    if (!materialSlot) {
      return;
    }
  ...
  async loadGLTF(src, pieceId, subPiecesConfig = {}) {
    const loader = this.editor.gltfCache.getLoader(src, {
      addUnknownExtensionsToUserData: true
    });
    const { json } = await loader.getDependency("root");
    if (!Array.isArray(json.nodes)) {
      throw new Error("glTF file has no nodes.");
    }
    const nodeIndex = json.nodes.findIndex(nodeDef =>
      isKitPieceNode(nodeDef, pieceId)
    );
    if (nodeIndex == -1) {
    ...
    clonedPiece.traverse(object => {
      ...
      if (kitAltMaterials) {
        ...
        if (defaultMaterials.length !== altMaterials.length) {
          console.warn(
            `Kit subPiece ${name} has ${
              defaultMaterials.length
            } default materials and ${
              altMaterials.length
            } alternate materials. They should be the same.`
          );
        }
        const primitives = [];
        object.traverse(o => {
        ...
        if (defaultMaterials.length !== primitives.length) {
          console.warn(
            `Kit subPiece ${name} has ${
              ...
              primitives.length
            } primitives. They should be the same.`
          );
        }
        const materialSlots = [];
        for (let i = 0; i < defaultMaterials.length; i++) {
          const { id, name, material: defaultMaterialIndex } = defaultMaterials[
            i
          ];
          ...
          const defaultValue = materialIds.find(
            m => m.index === defaultMaterialIndex
          );
          // This could produce a duplicate material option if the default material is already in the list
          options.push(defaultValue);
          const value =
            materialIds.find(m => m.id === subPieceConfig[id]) || defaultValue;
          materialSlots.push({
            id,
            name,
            object: primitive,
      ...
      for (const { object, value } of subPiece.materialSlots) {
        if (value) {
          pendingMaterials.push(
            loader
              .getDependency("material", value.index)
  ...
  async load(kitId, pieceId, subPiecesConfig?, onError?) {
    ...
    try {
      if (this._canonicalUrl && nextPieceId) {
        const { accessibleUrl, files } = await this.editor.api.resolveMedia(
          this._canonicalUrl
        );
        if (this.model) {
          this.editor.renderer.removeBatchedObject(this.model);
      ...
      const kitPieceError = new RethrownError(
        `Error loading kit piece. Kit Url: "${this._canonicalUrl}" Kit Id: "${
          this._kitId
    ...
    if (this.model.animations && this.model.animations.length > 0) {
      for (const animation of this.model.animations) {
        for (const track of animation.tracks) {
          ...
          if (!animatedNode) {
            throw new Error(
              `Model.updateStaticModes: Couldn't find object with uuid: "${uuid}"`
            );
          }
  ...
  serialize() {
    ...
    for (const subPiece of this.subPieces) {
      ...
      for (const materialSlot of subPiece.materialSlots) {
        subPieceConfig[materialSlot.id] = materialSlot.value
          ? materialSlot.value.id
          : null;
      }
