...
export default class SimpleWaterNode extends EditorNodeMixin(SimpleWater) {
  ...
  static async deserialize(editor, json) {
    ...
    const {
      ...
      ripplesSpeed,
      ripplesScale
    } = json.components.find(
      c => c.name === SimpleWaterNode.legacyComponentName
    ).props;
  ...
  constructor(editor) {
    if (!waterNormalMap) {
      console.warn(
        "SimpleWaterNode: water normal map was not loaded before creating a new SimpleWaterNode"
      );
    }
