...
export const GroundPlaneComponent = defineComponent({
  ...
  reactor: function () {
    ...
    const createMaterial = () => {
      ...
      } as MeshStandardMaterialParameters | ShadowMaterialParameters

      return component.visible.value
        ? new MeshStandardMaterial(materialParams)
        : new ShadowMaterial({ opacity: 0.5, ...materialParams })
    }

    const mesh = useMeshComponent(meshEntity, () => new PlaneGeometry(10000, 10000), createMaterial)
    ...
    useLayoutEffect(() => {
      mesh.material.set(createMaterial())
