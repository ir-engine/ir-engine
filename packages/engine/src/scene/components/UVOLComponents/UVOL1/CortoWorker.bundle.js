...
function CortoDecoder(data, byteOffset, byteLength) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    var components = stream.readUChar();
    var type = stream.readUChar();
...
CortoDecoder.prototype = {
  decode: function() {
    var t = this;
    t.last = new Uint32Array(t.nvert * 3);
    t.last_count = 0;
...
Attribute.prototype = {
  ...
  decode: function(nvert, stream) {
    var t = this;
    if (t.strategy & t.Strategy.CORRELATED)
...
ColorAttr.prototype.dequantize = function(nvert) {
  ...
  for (var i = 0; i < nvert; i++) {
    ...
    t.buffer[rgboff + 0] = (e2 + e0) * t.qc[0] & 255;
    t.buffer[rgboff + 1] = e0 * t.qc[1];
    t.buffer[rgboff + 2] = (e1 + e0) * t.qc[2] & 255;
  }
};
...
NormalAttr.prototype.init = function(nvert, nface) {
  var t = this;
  var n = nvert * t.components;
  t.values = new Int32Array(2 * nvert);
  switch (t.type) {
    case t.Type.INT16:
      t.buffer = new Int16Array(n);
      break;
...
NormalAttr.prototype.toOcta = function(i, input, o, output, unit) {
  ...
  var ap1 = p1 > 0 ? p1 : -p1;
  if (input[j + 2] < 0) {
    p0 = input[j] >= 0 ? 1 - ap1 : ap1 - 1;
    p1 = input[j + 1] >= 0 ? 1 - ap0 : ap0 - 1;
  }
  output[k] += p0 * unit;
  output[k + 1] += p1 * unit;
};
  ...
  createDecodingTables: function() {
    ...
    while (n_words < t.dictionary_size) {
      ...
      for (var i = 0; i < n_symbols; i++) {
        ...
        for (var k = 0; k < len; k++)
          t.table[pos + k] = t.table[offset + k];
        pos += len;
        t.table[pos++] = t.probs[i * 2];
        if (i + n_words == t.dictionary_size - 1)
          break;
      }
      if (i == n_symbols)
        t.starts[best] += n_symbols;
      n_words += n_symbols - 1;
    }
    var word = 0;
...
var timer;
var messageQueue = [];
function addMessageToQueue(payload) {
  messageQueue.push(payload);
}
function startHandlerLoop({ meshFilePath, fileHeader }) {
  ;
  globalThis.postMessage({ type: 1 /* INITIALIZED */ });
  timer = setInterval(async () => {
    ...
    try {
      ...
      for (let i = frameStart; i < frameEnd; i++) {
        ...
        const slice = buffer.slice(fileReadStartPosition, fileReadEndPosition);
      ...
      ;
      globalThis.postMessage({ type: 3 /* FRAMEDATA */, payload: outgoingMessages }, transferables);
    } catch (error) {
      ;
      globalThis.postMessage({ type: 3 /* FRAMEDATA */, payload: [] });
