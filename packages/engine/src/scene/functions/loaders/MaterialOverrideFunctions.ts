...
export async function refreshMaterials(target: Entity) {
  ...
  await Promise.all(
    model.materialOverrides
      .filter((override) => override.entity.value != -1)
      .map((override) => {
        removeComponent(override.entity.value!, MaterialOverrideComponent)
      })
  )
  ...
  const result = await Promise.all(
    model.materialOverrides.map(async (override) => await initializeOverride(target, override.value)?.())
