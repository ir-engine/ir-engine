...
function extend<T extends THREE.Material>(source: T, object) {
  ...
  if (isDepthMaterial(source) && defines.DEPTH_PACKING === undefined) defines.DEPTH_PACKING = THREE.RGBADepthPacking
...
function useUniformPairs(instance, uniforms) {
  ...
  if (instance.clearcoatNormalMap) cloneUniforms(UniformsLibExtendable.clearcoatnormalmap, uniforms)
}
...
function useUniforms(name, object, uniforms) {
  ...
  if (lights) {
    if (object.use) {
      ...
      if (use.indexOf('LightProbe') > -1) cloneUniforms(UniformsLibExtendable.lights.lightProbe, uniforms)

      if (use.indexOf('AmbientLight') > -1) cloneUniforms(UniformsLibExtendable.lights.ambientLightColor, uniforms)

      if (use.indexOf('ReactAreaLight') > -1) cloneUniforms(UniformsLibExtendable.lights.rectAreaLights, uniforms)

      if (use.indexOf('HemisphereLight') > -1) cloneUniforms(UniformsLibExtendable.lights.hemisphereLights, uniforms)
    } else {
