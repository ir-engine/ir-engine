...
`;
export class LinkModel extends Object3D {
  static shader = {
  ...
  linkMesh: Mesh<BoxBufferGeometry, ShaderMaterial>;
  _inclination: number;
  _azimuth: number;
  _distance: number;
  ...
  constructor() {
    super();
    LinkModel.material = new ShaderMaterial({
      fragmentShader: LinkModel.shader.fragmentShader,
      vertexShader: LinkModel.shader.vertexShader,
      uniforms: UniformsUtils.clone(LinkModel.shader.uniforms),
      side: BackSide,
      fog: true
    });
    this.cubeCamera = new CubeCamera(1, 100000, new WebGLCubeRenderTarget(512));
    this.linkMesh = new Mesh(LinkModel.geometry, LinkModel.material);
    this.linkMesh.name = "Link";
    this.add(this.linkMesh);
    this._inclination = 0;
    this._azimuth = 0.15;
    this._distance = 8000;
  ...
  get turbidity() {
    return this.linkMesh.material.uniforms.turbidity.value;
  }
  set turbidity(value) {
    this.linkMesh.material.uniforms.turbidity.value = value;
  }
  get rayleigh() {
    return this.linkMesh.material.uniforms.rayleigh.value;
  }
  set rayleigh(value) {
    this.linkMesh.material.uniforms.rayleigh.value = value;
  }
  get luminance() {
    return this.linkMesh.material.uniforms.luminance.value;
  }
  set luminance(value) {
    this.linkMesh.material.uniforms.luminance.value = value;
  }
  get mieCoefficient() {
    return this.linkMesh.material.uniforms.mieCoefficient.value;
  }
  set mieCoefficient(value) {
    this.linkMesh.material.uniforms.mieCoefficient.value = value;
  }
  get mieDirectionalG() {
    return this.linkMesh.material.uniforms.mieDirectionalG.value;
  }
  set mieDirectionalG(value) {
    this.linkMesh.material.uniforms.mieDirectionalG.value = value;
  }
  get inclination() {
  ...
  updateSunPosition() {
    ...
    this.linkMesh.material.uniforms.sunPosition.value.set(x, y, z).normalize();
    this.linkMesh.scale.set(distance, distance, distance);
  }
