...
class TransformControlsEcs extends Object3D<TransformControlsEventMap> {
  [x: string]: any
  isTransformControls: boolean
  domElement: any
  private _gizmo: TransformControlsGizmoEcs
  private _plane: TransformControlsPlaneEcs
  _offset: Vector3
  _startNorm: Vector3
  _endNorm: Vector3
  ...
  constructor(camera, domElement) {
    ...
    const _gizmo = new TransformControlsGizmoEcs()
    this._gizmo = _gizmo
    this.add(_gizmo)

    const _plane = new TransformControlsPlaneEcs()
    this._plane = _plane
    this.add(_plane)
  ...
  updateMatrixWorld() {
    if (this.entity !== UndefinedEntity) {
      ...
      const parentMatrix = getComponent(parentEntity!, TransformComponent).matrix
      parentMatrix.decompose(this._parentPosition, this._parentQuaternion, this._parentScale)
...
class TransformControlsGizmoEcs extends Object3D {
  [x: string]: any
  picker: any
...
class TransformControlsPlaneEcs extends Mesh {
  [x: string]: any
...
export { TransformControlsEcs, TransformControlsGizmoEcs, TransformControlsPlaneEcs }
