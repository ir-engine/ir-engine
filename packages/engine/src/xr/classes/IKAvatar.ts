...
export class Avatar {
  ...
  constructor(object, options: AvatarOptions = { debug: true, top: true, bottom: true, visemes: true, hair: true }) {
    ...
    const Left_thumb2 = _getOptional(
    ...
    const Left_indexFinger3 = _getOptional(
    ...
    const Left_middleFinger3 = _getOptional(
    ...
    const Left_ringFinger3 = _getOptional(
    ...
    const Left_littleFinger3 = _getOptional(
    ...
    const Right_thumb2 = _getOptional(
    ...
    const Right_indexFinger3 = _getOptional(
    ...
    const Right_middleFinger3 = _getOptional(
    ...
    const Right_ringFinger3 = _getOptional(
    ...
    const Right_littleFinger3 = _getOptional(
    ...
    if (options.hair) {
      new Promise((accept, reject) => {
        ...
        springBoneManagerPromise = new VRMSpringBoneImporter().import(object).then((springBoneManager) => {
    ...
    const findFingerBone = (r, left) => {
      const fingerTipBone = tailBones
        .filter(
          (bone) =>
            r.test(bone.name) &&
            findClosestParentBone(bone, (bone) => bone === modelBones.Left_wrist || bone === modelBones.Right_wrist)
        )
        .sort((a, b) => {
    ...
    this.height = eyePosition
      .clone()
      .sub(
        _averagePoint([
          modelBones.Left_ankle.getWorldPosition(new Vector3()),
          modelBones.Right_ankle.getWorldPosition(new Vector3())
        ])
      ).y
    this.shoulderWidth = modelBones.Left_arm.getWorldPosition(new Vector3()).distanceTo(
      modelBones.Right_arm.getWorldPosition(new Vector3())
    )
    ...
    const indexDistance = modelBones.Left_indexFinger1.getWorldPosition(new Vector3()).distanceTo(
      modelBones.Left_wrist.getWorldPosition(new Vector3())
    )
    const handWidth = modelBones.Left_indexFinger1.getWorldPosition(new Vector3()).distanceTo(
      modelBones.Left_littleFinger1.getWorldPosition(new Vector3())
    )
    ...
    if (options.visemes) {
      const vrmExtension =
        ...
        this.object.userData.gltfExtensions &&
        this.object.userData.gltfExtensions.VRM
      const blendShapes =
        vrmExtension && vrmExtension.blendShapeMaster && vrmExtension.blendShapeMaster.blendShapeGroups
      ...
      this.skinnedMeshesVisemeMappings = this.skinnedMeshes.map((o) => {
        ...
        if (morphTargetDictionary && morphTargetInfluences) {
          ...
          const blinkLeftIndex =
            _getVrmBlendShapeIndex(/^(?:blink_l|blinkleft)$/i) || morphTargetDictionary['vrc.blink_left'] || null
          const blinkRightIndex =
            _getVrmBlendShapeIndex(/^(?:blink_r|blinkright)$/i) || morphTargetDictionary['vrc.blink_right'] || null
          return [morphTargetInfluences, aaIndex, blinkLeftIndex, blinkRightIndex]
        } else {
          return null
        }
      })
  ...
  update(timeDiff) {
    ...
    if (this.getTopEnabled()) {
      ...
      this.sdkInputs.leftGamepad.position
        ...
        .add(localVector.copy(this.handOffsetLeft).applyQuaternion(this.inputs.leftGamepad.quaternion))
      ...
      this.sdkInputs.rightGamepad.position
        ...
        .add(localVector.copy(this.handOffsetRight).applyQuaternion(this.inputs.rightGamepad.quaternion))
    ...
    if (!this.getBottomEnabled()) {
      this.outputs.hips.position.copy(this.inputs.hmd.position).add(this.eyeToHipsOffset)
