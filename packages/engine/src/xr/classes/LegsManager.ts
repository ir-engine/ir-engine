...
class Leg {
  ...
  constructor(legsManager, left) {
    ...
    this.upperLeg = new Object3D()
    this.lowerLeg = new Object3D()
    this.foot = new Object3D()
    this.foot['stickTransform'] = new Object3D()
    this.foot['startTransform'] = new Object3D()
    this.foot['endTransform'] = new Object3D()
    this.foot['startHmdFloorTransform'] = new Object3D()
    ...
    this.legLength = 0
    this.eyesToUpperLegOffset = new Vector3()
  ...
  Start() {
    ...
    Helpers.getWorldPosition(this.upperLeg, this.eyesToUpperLegOffset).sub(
      Helpers.getWorldPosition(this.legsManager.rig.shoulderTransforms.eyes, localVector)
    )
...
class LegsManager {
  ...
  constructor(rig) {
    ...
    this.lastHmdPosition = new Vector3()

    this.hmdVelocity = new Vector3()
  ...
  Start() {
    this.legSeparation = Helpers.getWorldPosition(this.leftLeg.upperLeg, localVector).distanceTo(
  ...
  Update() {
    if (this.enabled) {
      ...
      if (/*this.leftLeg.standing && */ !this.rig.shoulderTransforms.prone) {
        ...
        localMatrix3
          .compose(zeroVector, localQuaternion3.setFromEuler(leftFootEuler), oneVector)
      ...
      if (/*this.rightLeg.standing && */ !this.rig.shoulderTransforms.prone) {
        ...
        localMatrix3
          .compose(zeroVector, localQuaternion3.setFromEuler(rightFootEuler), oneVector)
          .premultiply(planeMatrix)
      ...
      if (this.rig.shoulderTransforms.prone) {
        const targetPosition = Helpers.getWorldPosition(this.leftLeg.upperLeg, localVector6).add(
          localVector7.set(0, -this.leftLeg.legLength * 0.95 + floorHeight, 0).applyQuaternion(this.hips.quaternion)
        )
        targetPosition.y = 0
      ...
      if (this.rig.shoulderTransforms.prone) {
        const targetPosition = Helpers.getWorldPosition(this.rightLeg.upperLeg, localVector6).add(
          localVector7.set(0, -this.rightLeg.legLength * 0.95 + floorHeight, 0).applyQuaternion(this.hips.quaternion)
        )
        targetPosition.y = 0
