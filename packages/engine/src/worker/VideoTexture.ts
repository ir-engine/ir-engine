...
import { Mapping, RGBAFormat, LinearFilter, TextureDataType, TextureFilter, CanvasTexture, Wrapping } from 'three'
...
export class VideoTextureProxy extends CanvasTexture {
  ...
  update() {
    if (!this.videoProxy._requestVideoFrameCallback && this.videoProxy.readyState >= 2) {
      ;(this as any).image = this.videoProxy.video
      ;(this as any).needsUpdate = true
    }
  }
