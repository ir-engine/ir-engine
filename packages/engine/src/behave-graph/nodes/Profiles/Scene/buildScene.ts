...
function generateChoicesForProperty(property: ResourceProperties | undefined, resource: Resource) {
  if (!property) return []
...
export const buildScene = ({
  ...
  const addOnSceneChangedListener: IScene['addOnSceneChangedListener'] = (listener) => {
    onSceneChanged.addListener(listener)
  }

  const removeOnSceneChangedListener: IScene['removeOnSceneChangedListener'] = (listener) => {
