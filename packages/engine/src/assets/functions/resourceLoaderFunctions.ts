...
const isCloneable = (resourceType: ResourceType): boolean => {
  ...
  return resourceType === ResourceType.Texture || resourceType === ResourceType.File
}
    ...
    (response: T) => {
      ...
      if (pending[url]) {
        for (const pendingLoad of pending[url]) {
          if (resourceType === ResourceType.File) pendingLoad(response)
          else if (!cloneAsset(response as Cloneable<T>, pendingLoad))
