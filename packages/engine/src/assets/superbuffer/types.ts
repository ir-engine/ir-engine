 ...
 */
export interface BufferView<T extends Serializable = Serializable> {
...
export type SchemaObject<T> = {
  [K in keyof T]: T[K] extends BufferView<infer U>
    ? U
    : T[K] extends Array<BufferView<infer U>>
      ? U[]
      : T[K] extends Schema<infer U>
        ? SchemaObject<U>
        : T[K] extends Array<Schema<infer U>>
          ? Array<SchemaObject<U>>
          : T[K] extends Record<string, unknown>
            ? SchemaObject<T[K]>
            : never;
}
