...
interface AssetLoaderParamType {
  entity?: Entity
  parent?: Object3D
  url: string
  [key: string]: any
...
export class AssetLoader {
  ...
  constructor (
    private readonly params: AssetLoaderParamType,
    private readonly onLoad?: (response: any) => void,
    private readonly onProgress?: (request: ProgressEvent) => void,
    private readonly onError?: (event: ErrorEvent | Error) => void
  ) {
    if (!this.params.url) {
  ...
  handleLODs = (asset: Object3D): Object3D => {
    ...
    const LODs = new Map<string, Array<{ object: Object3D, level: string }>>()
  ...
  processAsset = (asset: any): void => {
    ...
    if (this.params.parent) {
      ...
      if (hasComponent(this.params.entity, Object3DComponent)) {
        if (getComponent<Object3DComponent>(this.params.entity, Object3DComponent).value !== undefined) { getMutableComponent<Object3DComponent>(this.params.entity, Object3DComponent).value.add(asset) } else getMutableComponent<Object3DComponent>(this.params.entity, Object3DComponent).value = asset
      } else {
  ...
  getAssetType (assetFileName: string): AssetType {
    if (/\.(?:gltf|glb)$/.test(assetFileName)) { return AssetType.glTF } else if (/\.(?:fbx)$/.test(assetFileName)) { return AssetType.FBX } else if (/\.(?:vrm)$/.test(assetFileName)) { return AssetType.VRM } else if (/\.(?:png)$/.test(assetFileName)) { return AssetType.PNG } else if (/\.(?:jpg|jpeg|)$/.test(assetFileName)) { return AssetType.JPEG } else { return null }
  }
