...
function generateTransform( transformData ) {
  ...
  const lScalingPivotM = new Matrix4N();
  const lScalingOffsetM = new Matrix4();
  const lRotationOffsetM = new Matrix4();
  const lRotationPivotM = new Matrix4N();
  ...
  const lParentGRM = new Matrix4N();
  lParentGX.extractRotation( lParentGRM );

  // Global Shear*Scaling
  const lParentTM = new Matrix4N();
  ...
  if ( inheritType === 0 ) {
    ...
    const lParentLSM = new Matrix4N().copy( lScalingM );
...
// TODO: this is temporary class with .invert method, remove as soon as we migrate to three.js v123
class Matrix4N extends Matrix4 {
  invert() {
