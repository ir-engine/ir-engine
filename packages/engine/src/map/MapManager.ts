...
import { BackgroundDataSource } from "@here/harp-mapview/lib/BackgroundDataSource";
import { TextElementsRendererOptions } from "@here/harp-mapview/lib/text/TextElementsRendererOptions";
import { IntersectParams } from "@here/harp-mapview/lib/IntersectParams";
...
import { IMapAntialiasSettings, IMapRenderingManager, MapRenderingManager } from '@here/harp-mapview/lib/composing/MapRenderingManager';
...
import { ResourceComputationType, VisibleTileSet } from '@here/harp-mapview/lib/VisibleTileSet';
import { assert, LoggerManager, LogLevel, PerformanceTimer, TaskQueue, UriResolver } from '@here/harp-utils';
import { getOptionValue } from '@here/harp-utils/lib/OptionsUtils';
import { VectorTileDataSource } from "@here/harp-vectortile-datasource";
import { Event as THREEEvent, Box3, Color, Light, MathUtils, Matrix4, Object3D, PCFSoftShadowMap, PerspectiveCamera, Plane, Raycaster, Scene, Sphere, Vector2, Vector3, WebGLRenderer } from "three";
 ...
 */
export interface RenderEvent extends THREEEvent {
...
export class MapView extends EventDispatcher {
    ...
    private m_worldMaxBounds?: Box3 | OrientedBox3;
     ...
     */
    private readonly m_rteCamera = new PerspectiveCamera();
    ...
    private readonly m_targetWorldPos = new Vector3();
    ...
    private m_pointOfView?: PerspectiveCamera;
    ...
    private readonly m_sceneRoot = new Object3D();
    /** Root node of [[m_overlayScene]] that gets cleared every frame. */
    private readonly m_overlaySceneRoot = new Object3D();
    ...
    // gestures
    private readonly m_raycaster = new Raycaster();
    private readonly m_plane = new Plane(new Vector3(0, 0, 1));
    private readonly m_sphere = new Sphere(undefined, EarthConstants.EQUATORIAL_RADIUS);
    ...
    private readonly m_poiManager: PoiManager = new PoiManager(this as any);

    private readonly m_poiTableManager: PoiTableManager = new PoiTableManager(this as any);
    ...
    get delayLabelsUntilMovementFinished() {
        return null; // this.textElementsRenderer.delayLabelsUntilMovementFinished;
    }
     ...
     */
    set pointOfView(pointOfView: PerspectiveCamera | undefined) {
     ...
     */
    get worldMaxBounds(): Box3 | OrientedBox3 | undefined {
    ...
    getWorldPositionAt(x: number, y: number, fallback?: boolean): Vector3 | null;
    ...
    private lookAtImpl(params: Partial<LookAtParams>): void {
        ...
        if (params.bounds !== undefined) {
            ...
            const worldPoints = geoPoints.map(point =>
                this.projection.projectPoint(GeoCoordinates.fromObject(point), new Vector3())
            );
     ...
     */
    public ndcToView(vector: Vector3Like, result: Vector3): Vector3 {
        result
    ...
    private render(frameStartTime: number): void {
        ...
        if (this.m_movementDetector.checkCameraMoved(this as any, frameStartTime)) {
    ...
    private createTextRenderer(): TextElementsRenderer {
        ...
        return new TextElementsRenderer(
            new MapViewState(this as any, this.checkIfTilesChanged.bind(this)),
            updateCallback,
            this.m_screenProjector,
            this.m_poiManager,
            this.m_renderer,
            [this.imageCache, this.userImageCache],
            (this as any).m_options
        );
    }
