...
import { hashResourceString } from "../../../assets/systems/AssetLoadingSystem";
...
export const initializeCharacter: Behavior = (entity): void => {
	...
	if (!assetLoader.loaded) {
		assetLoader.onLoaded = asset => {
			onAssetLoaded(entity, asset);
		};
	} else {
		onAssetLoaded(entity, AssetVault.instance.assets.get(hashResourceString(assetLoader.url)));
	}
};

function onAssetLoaded(entity, asset) {
