...
export const vehicleInterpolationBehavior: Behavior = (entity: Entity, args): void => {
  ...
  // LAZY LOAD CAR MODEL
  if (
    !vehicleComponent.vehicleMesh &&
    model !== undefined &&
    model.value !== undefined &&
    model.value.children[0] !== undefined
  ) {
    console.warn('test')
  ...
  if (!isDriver && args.snapshot.qX != undefined) {
    chassisBody.position.set(args.snapshot.x, args.snapshot.y, args.snapshot.z)

    chassisBody.quaternion.set(args.snapshot.qX, args.snapshot.qY, args.snapshot.qZ, args.snapshot.qW)
