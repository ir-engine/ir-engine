...
import { BoneStructure } from '../VehicleBoneMatching'
import { VehicleComponent } from './VehicleComponent'
import { VehiclePendingComponent } from './VehiclePendingComponent'

export const VehicleAnimationComponent = defineComponent({
  name: 'VehicleAnimationComponent',
...
export const VehicleRigComponent = defineComponent({
  name: 'VehicleRigComponent',
  ...
  onInit: (entity) => {
    return {
      ...
      lowerLegLength: 0,
      /** The height of the foot in a t-pose, from the ankle joint to the bottom of the vehicle's model */
      footHeight: 0,
  ...
  reactor: function () {
    ...
    const anim = useComponent(entity, VehicleRigComponent)
    const pending = useOptionalComponent(entity, VehiclePendingComponent)
    ...
    useEffect(() => {
      ...
      for (const [boneName, bone] of Object.entries(rig)) {
        ...
        // bone.add(axesHelper)
        proxifyVector3(VehicleRigComponent.rig[boneName].position, entity, bone.position)
        proxifyQuaternion(VehicleRigComponent.rig[boneName].rotation, entity, bone.quaternion)
      }
    }, [anim.rig])
