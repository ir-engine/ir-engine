...
class Pose {
  ...
  constructor(entity, clone) {
    this.entity = entity;
    const armature = getMutableComponent(this.entity, IKRigComponent).skinnedMesh;
    this.skeleton = clone ?
      SkeletonUtils.clone(armature.parent).children.find(skin => skin.skeleton != null).skeleton :
      (armature.parent.children.find((skin: SkinnedMesh) => skin.skeleton != null) as SkinnedMesh).skeleton;	// Recreation of Bone Hierarchy
  ...
  apply() {
    ...
    const targetSkeleton: Skeleton = getMutableComponent(this.entity, IKRigComponent).skinnedMesh.skeleton;
