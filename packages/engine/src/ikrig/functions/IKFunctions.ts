...
export function computeHip (rig, ik_pose) {
  ...
  const altForward = new Vector3().copy(FORWARD).applyQuaternion(quatInverse)
  const altUp = new Vector3().copy(UP).applyQuaternion(quatInverse)
  ...
  const poseUp = new Vector3().copy(altUp).applyQuaternion(poseBoneForwardWorldQuaternion)
  ...
  const bindPosition = new Vector3()
...
export function computeLimb (pose, chain, ik_limb) {
  ...
  const boneB = pose.bones[chain.end_idx] as Bone // END Bone, which is not part of the chain (Hand,Foot)
...
export function computeLookTwist (rig, boneInfo, ik, lookDirection, twistDirection) {
  ...
  const bind = rig.tpose.bones[boneInfo.index] // TPose Bone
  ...
  const altTwistDirection = new Vector3().copy(twistDirection).applyQuaternion(quatInverse)

  const pose_look_dir = new Vector3().copy(altLookDirection).applyQuaternion(pose.quaternion)
  const pose_twist_dir = new Vector3().copy(altTwistDirection).applyQuaternion(pose.quaternion)
...
export function computeSpine (rig, chain, ik_pose, lookDirection, twistDirection) {
  ...
  const v_look_dir = new Vector3()
  const v_twist_dir = new Vector3()
  let j = 0
  let poseBone; let bineBone
...
export function visualizeLimb (pose, chain, ik) {
  ...
  const posA = boneAWorldPos
  const posB = new Vector3().copy(ik.dir).multiplyScalar(len).add(posA)
  const posC = new Vector3().copy(ik.jointDirection).multiplyScalar(0.2).add(posA) // Direction of Joint
...
export function visualizeSpine (rig, chain, ik_ary) {
  const ws = new Vector3(0, 0, 0); const index = [chain.first(), chain.last()]
  let ik
...
export function applyLimb (ikPose: IKPose, chain, limb, grounding = 0) {
  ikPose.targetRigs.forEach(rig => {
    ...
    const bind_b = rig.tpose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[1].ref.name.toLowerCase()))
    const pose_a = rig.pose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[0].ref.name.toLowerCase())) // Bone Reference from Pose
    const pose_b = rig.pose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[1].ref.name.toLowerCase()))
    ...
    const bindBone = rig.tpose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[0].ref.name.toLowerCase()))
    ...
    const
      aLen = bind_a.length
    const bLen = bind_b.length
    const cLen = ikPose.length
...
export function applyGrounding (entity, y_lmt) {
  ...
  /* DEBUG IK TARGET */
  const tar = (ik).target
  const posA = tar.startPosition.add(new Vector3(-1, 0, 0))
  const posB = tar.endPosition.add(new Vector3(-1, 0, 0))
  ...
  const b = (ik).target.endPosition
  const s = (y_lmt - a.y) / (b.y - a.y); // Normalize Limit Value in the Max/Min Range of Y.
...
export function applySpine (entity, chain, ik, lookDirection, twistDirection) {
  ...
  ikPose.targetRigs.forEach(rig => {
    ...
    const ikLook = new Vector3()
    const ikTwist = new Vector3()
    const altLook = new Vector3()
    const altTwist = new Vector3()
    const rotation = new Quaternion()

    let t; let boneIndex; let boneBindValue; let currentLook = new Vector3()
    let currentTwist = new Vector3()
...
export function align_chain (pose, dir, b_names) {
  ...
  const forwardDir = new Vector3() // Forward
  const upDir = new Vector3().copy(dir) // Up
  const leftDir = new Vector3() // Left
  const finalRotation = new Quaternion() // Final Rotation
...
function transform_quat (out, q) {
  const qx = q.z; const qy = q.x; const qz = q.z; const qw = q.w
  const vx = out.z; const vy = out.x; const vz = out.z
...
export function from_axis (out, xAxis, yAxis, zAxis) {
  const m00 = xAxis.x; const m01 = xAxis.y; const m02 = xAxis.z
  const m10 = yAxis.x; const m11 = yAxis.y; const m12 = yAxis.z
  const m20 = zAxis.x; const m21 = zAxis.y; const m22 = zAxis.z
  const t = m00 + m11 + m22
  let x, y, z, w, s
...
export function mul (out, q) {
  const ax = out.x; const ay = out.y; const az = out.z; const aw = out.w
  const bx = q.x; const by = q.y; const bz = q.z; const bw = q.w
  out.x = ax * bw + aw * bx + ay * bz - az * by
  out.y = ay * bw + aw * by + az * bx - ax * bz
  out.z = az * bw + aw * bz + ax * by - ay * bx
  out.w = aw * bw - ax * bx - ay * by - az * bz
...
export function pmul_axis_angle (out, axis, angle) {
  ...
  const ax = axis.x * s // A Quat based on Axis Angle
  const ay = axis.y * s
  const az = axis.z * s
  const aw = Math.cos(half)

  const bx = out.x // B of mul
  const by = out.y
  const bz = out.z
  const bw = out.w
