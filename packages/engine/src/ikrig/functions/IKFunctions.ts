...
import { BACK, DOWN, UP, FORWARD, LEFT, RIGHT } from "../constants/Vector3Constants";
...
export function setupIKRig(entity: Entity) {
	//-----------------------------------------
...
export function computeHip(rig, ik_pose) {
  ...
  const poseBone: Bone = rig.pose.bones[boneInfo.index];
  const bindBone: Bone = rig.tpose.bones[boneInfo.index]; // TPose Bone
	...
	// rig.debug.setLine( poseBoneWorldPosition, new Vector3().copy(poseBoneWorldPosition).add(FORWARD), COLOR.white );
	// rig.debug.setLine( poseBoneWorldPosition, new Vector3().copy(poseBoneWorldPosition).add(UP), COLOR.white );
	// rig.debug.setLine( poseBoneWorldPosition, new Vector3().copy( poseForward).multiplyScalar(0.8 ).add( poseBoneWorldPosition ), COLOR.orange );
	// rig.debug.setLine( poseBoneWorldPosition, new Vector3().copy( poseUp).multiplyScalar(0.8 ).add( poseBoneWorldPosition ), COLOR.orange );
	...
	const swing = new Quaternion().setFromUnitVectors(FORWARD, poseForward) // First we create a swing rotation from one dir to the other.
	...
	// share the same forward sxis. The issue is that we do not know if that twist is in the Negative direction
	...
	// // App.rig.debug.ln( pos, Vec3.scale( swing_lft, 1.5 ).add( pos ), "orange" );
	...
	// rig.debug.setLine( position, Vector3.scale( swing_lft, 1.5 ).add( position ), "orange" );
	if (vec3Dot >= 0)
		twist = -twist;
...
export function visualizeHip(debug: IKRigDebugHelper, rig: IKRigComponent, pose: IKPoseComponent) {
	rig.pose.bones[rig.points.hip.index].getWorldPosition(boneAWorldPos);
	debug
    .setPoint(boneAWorldPos, COLOR.orange, 6, 6)
		.setLine(boneAWorldPos, new Vector3().copy(pose.hip.dir).multiplyScalar(0.20).add(boneAWorldPos), COLOR.cyan, null, true);
}

export function visualizeLimb(debug: IKRigDebugHelper, rig: IKRigComponent, chain, ik) {
	const poseBone = rig.pose.bones[chain.first()];
...
export function visualizeLookTwist(debug: IKRigDebugHelper, rig: IKRigComponent, boneInfo, ik) {
...
export function visualizeSpine(debug: IKRigDebugHelper, rig: IKRigComponent, chain, ik_ary) {
...
export function applyHip(pose: IKPoseComponent) {
    ...
    const parentInverse = new Quaternion().copy(parentRotation).invert()

		// pmul_invert(q, parentRotation);
		q.premultiply(parentInverse);
		...
		const position = new Vector3().copy(ikPose.hip.movement).multiplyScalar(h_scl) // Scale the Translation Differnce to Match this Models Scale
...
export function applyLimb(ikPose: IKPoseComponent, chain, limb, grounding = 0) {
	ikPose.targetRigs.forEach(rig => {

		// solve for ik
		// Using law of cos SSS, so need the length of all sides of the triangle
		const bind_a = rig.tpose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[0].ref.name.toLowerCase())),	// Bone Reference from Bind
			bind_b = rig.tpose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[1].ref.name.toLowerCase())),
			pose_a = rig.pose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[0].ref.name.toLowerCase())),		// Bone Reference from Pose
			pose_b = rig.pose.bones.find(bone => bone.name.toLowerCase().includes(chain.chainBones[1].ref.name.toLowerCase()))

		const bindAIndex = rig.tpose.bones.findIndex(bone => bone.name.toLowerCase().includes(chain.chainBones[0].ref.name.toLowerCase()))
		const bindBIndex = rig.tpose.bones.findIndex(bone => bone.name.toLowerCase().includes(chain.chainBones[1].ref.name.toLowerCase()))
    ...
    // this.axis.from_dir( dir, up_dir ); // Target Axis
		...
		ikPose.startPosition.copy(bind_a.position);
		// Set the end position by copying the limb direction, multiplying by length factor and adding bone position
		ikPose.endPosition.copy(limb.dir).multiplyScalar(len)	// Compute End Effector
			.add(bind_a.position);
		...
		const rot = new Quaternion().copy(parentWorldQuaternion).multiply(pose_a.quaternion);
		//Swing
    ...
    outRot.premultiply(new Quaternion().setFromAxisAngle(ikPose.axis.z, twist))
		// END SOLVER
		...
		// pmul_axis_angle(outRot, ikPose.axis.x, -rad)
		// pmul_invert(outRot, parentWorldQuaternion);
    outRot.premultiply(new Quaternion().setFromAxisAngle(ikPose.axis.x, -rad))	// Rotate it by the target's x-axis .pmul( tmp.from_axis_angle( this.axis.x, rad ) )
    outRot.premultiply(parentWorldQuaternion.clone().invert())				// Convert to Local Space in temp to save WS rot for next bone.
    ...
    outRot.premultiply(new Quaternion().setFromAxisAngle(ikPose.axis.x, rad))	// Rotate it by the target's x-axis .pmul( tmp.from_axis_angle( this.axis.x, rad ) )
    outRot.premultiply(poseAWorldQuaternion.clone().invert())				// Convert to Local Space in temp to save WS rot for next bone.
...
// 	rig.debug.setPoint((ik as any).target.endPosition.add(new Vector3(-1, 0, 0)), "orange", 0.05, 6);
...
function transform_quat(out, q) {
	const qx = q.z, qy = q.x, qz = q.z, qw = q.w,
