...
export const experementalRigidbodyCorrectionBehavior: Behavior = (
  entity: Entity,
  ...
  let offsetX = localInterpolation.correctionStart
    ? correction.x - currentSnapshot.x
    : localInterpolation.positionDiff.x
  let offsetY = localInterpolation.correctionStart
    ? correction.y - currentSnapshot.y
    : localInterpolation.positionDiff.y
  let offsetZ = localInterpolation.correctionStart
    ? correction.z - currentSnapshot.z
    : localInterpolation.positionDiff.z
  ...
  if (localInterpolation.correctionStart && Math.abs(offsetX) + Math.abs(offsetY) + Math.abs(offsetZ) > level0) {
    localInterpolation.positionDiff.set(offsetX, offsetY, offsetZ)

    localInterpolation.correctionStart = false
  } else if (
    !localInterpolation.correctionStart &&
    Math.abs(offsetX) + Math.abs(offsetY) + Math.abs(offsetZ) < level0 * 10
  ) {
  ...
  if (Math.abs(offsetX) >= level0) {
    if (Math.abs(offsetX) < level5) {
      ...
      localInterpolation.correctionStart
        ...
        : localInterpolation.positionDiff.setX(localInterpolation.positionDiff.x - offsetX)
    } else {
      if (offsetX > 0) {
        offsetX = level5 / 2
        localInterpolation.correctionStart
          ? localInterpolation.positionDiff.setX(0)
          : localInterpolation.positionDiff.setX(localInterpolation.positionDiff.x - level5 / 2)
      } else {
        offsetX = (level5 / 2) * -1
        localInterpolation.correctionStart
          ? localInterpolation.positionDiff.setX(0)
  ...
  if (Math.abs(offsetY) >= level0) {
    if (Math.abs(offsetY) < level5) {
      ...
      localInterpolation.correctionStart
        ...
        : localInterpolation.positionDiff.setY(localInterpolation.positionDiff.y - offsetY)
    } else {
      if (offsetY > 0) {
        offsetY = level5
        localInterpolation.correctionStart
          ? localInterpolation.positionDiff.setY(0)
          : localInterpolation.positionDiff.setY(localInterpolation.positionDiff.y - level5)
      } else {
        offsetY = level5 * -1
        localInterpolation.correctionStart
          ? localInterpolation.positionDiff.setY(0)
  ...
  if (Math.abs(offsetZ) >= level0) {
    if (Math.abs(offsetZ) < level5) {
      ...
      localInterpolation.correctionStart
        ...
        : localInterpolation.positionDiff.setZ(localInterpolation.positionDiff.z - offsetZ)
    } else {
      if (offsetZ > 0) {
        offsetZ = level5 / 2
        localInterpolation.correctionStart
          ? localInterpolation.positionDiff.setZ(0)
          : localInterpolation.positionDiff.setZ(localInterpolation.positionDiff.z - level5 / 2)
      } else {
        offsetZ = (level5 / 2) * -1
        localInterpolation.correctionStart
          ? localInterpolation.positionDiff.setZ(0)
