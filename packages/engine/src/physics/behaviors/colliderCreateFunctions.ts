...
export function addColliderWithoutEntity (userData: ColliderData, pos = new Vector3(), rot = new Quaternion(), scale = new Vector3(),
  ...
  switch (model.collisionMask) {
    ...
    default: if (/all/i.test(model.collisionMask as string)) { shapeArgs.config.collisionMask = DefaultCollisionMask } else { shapeArgs.config.collisionMask = Number(model.collisionMask) }
      break
  }
