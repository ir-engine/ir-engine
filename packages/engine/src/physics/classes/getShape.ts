...
import { TransformType, SHAPES } from '../types/PhysicsTypes'
...
export const getShape = (physics: Physics, { shape, transform, options, config }): PhysX.PxShape => {
  const geometry = getGeometry(physics, { shape, transform, options })
  if (!geometry) return

  const material = physics.physics.createMaterial(
  ...
  const flags = new PhysX.PxShapeFlags(
    PhysX.PxShapeFlag.eSCENE_QUERY_SHAPE.value |
      (config?.isTrigger ? PhysX.PxShapeFlag.eTRIGGER_SHAPE.value : PhysX.PxShapeFlag.eSIMULATION_SHAPE.value)
  )

  const newShape = physics.physics.createShape(geometry, material, false, flags)
...
const getGeometry = (physics: Physics, { shape, transform, options }): PhysX.PxGeometry => {
  ...
  switch (shape) {
    ...
    case SHAPES.TriangleMesh:
      geometry = createTrimesh(transform, physics.cooking, physics.physics, vertices, indices)
      break
    default:
    case SHAPES.ConvexMesh:
      geometry = createConvexMesh(transform, physics.cooking, physics.physics, vertices)
      break
  }
  return geometry
}
