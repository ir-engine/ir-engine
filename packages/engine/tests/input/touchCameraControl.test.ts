...
//   })
  ...
  describe("movement", () => {
    it ("lifecycle STARTED", () => {
        ...
        expect(data1.value[0]).toBeGreaterThanOrEqual(0.5);
        expect(data1.value[0]).toBeLessThanOrEqual(2);
        expect(data1.value[1]).toBeGreaterThanOrEqual(0.5);
        expect(data1.value[1]).toBeLessThanOrEqual(2);
        expect(data1.lifecycleState).toBe(LifecycleValue.CHANGED);
      //expect(mockedBehaviorOnStarted.mock.calls.length).toBe(1);
    });
