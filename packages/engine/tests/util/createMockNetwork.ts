...
export const createMockNetwork = (networkType = NetworkTypes.world) => {
  if(networkType === NetworkTypes.world)
    Engine.instance.currentWorld._worldHostId = networkType as UserId
  else 
    Engine.instance.currentWorld._mediaHostId = networkType as UserId
  Engine.instance.currentWorld.networks.set(
    networkType,
    new MockNetwork(networkType, networkType)
  )
}