...
describe("check MapUpdateSystem", () => {
    ...
    const nearTileGeoCoords = [
        tile2long(atlantaTileCoord[0] + 2, TILE_ZOOM),
        tile2lat(atlantaTileCoord[1], TILE_ZOOM)
    ]
    ...
    beforeAll(async () => {
        ...
        updateMapMock = jest.spyOn(createMap, "updateMap").mockImplementation((args: MapProps, longtitude, latitude, position) => {
                console.log('MOCKED updateMap')
                return Promise.resolve()
            }
        )
        ...
        addComponent(map, MapComponent, { coordinates: mapCenter })
    ...
    it("not call update if player moves inside of original tile", async () => {
        ...
        actorTransform.position.set(
            sceneCoordinates[0], 0, sceneCoordinates[1]
        )

        execute(1, 1)

        //expect(updateMock.mock.calls.length).toBe(0)
        expect(updateMapMock.mock.calls.length).toBe(0)
    })
    ...
    it("calls update if player moves out", async () => {
        ...
        expect(updateMapMock.mock.calls.length).toBe(1)
    })
