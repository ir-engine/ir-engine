...
const kinematicBodyQuery = defineQuery([RigidBodyComponent, RigidBodyKinematicTagComponent, TransformComponent])
...
const execute = () => {
  ...
  const kinematicEntities = kinematicBodyQuery()
  for (let i = 0; i < physicsSubsteps; i++) {
    // smooth kinematic pose changes
    const substep = (i + 1) / physicsSubsteps
    for (const entity of kinematicEntities) smoothPositionBasedKinematicBody(entity, timestep, substep)
