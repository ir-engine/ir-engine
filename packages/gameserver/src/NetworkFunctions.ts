...
export function getUserIdFromSocketId(socketId) {
  const client = Array.from(useEngine().defaultWorld.clients.values()).find((c) => c.socketId === socketId)
  return client?.userId
}

export async function validateNetworkObjects(): Promise<void> {
  if (!useEngine().isInitialized) return
...
export async function handleHeartbeat(socket): Promise<any> {
  ...
  if (useEngine().defaultWorld.clients.has(userId))
    useEngine().defaultWorld.clients.get(userId)!.lastSeenTs = Date.now()
}
...
export async function handleLeaveWorld(socket, data, callback): Promise<any> {
  ...
  if (useEngine().defaultWorld?.clients.has(userId)) useEngine().defaultWorld.clients.delete(userId)
