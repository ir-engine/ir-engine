...
export class Modifier {
  ...
  propagateUpdate(realtime = false) {
    ...
    const modifiersAffectedBy = this.parent.getModifiersAffectedBy(this, f).map(dependentModifierGroup => {
  ...
  get image() {
    return `data/targets/${this.fullName
      .replace("/", "/images/")
      .replace("|", "-")
      .toLowerCase()}.png`
  }
      ...
      if (tWeights.hasOwnProperty(tpath)) {
        ...
        if (target === undefined) {
          if (!this.parent.human.targets.loading) {
            console.warn(
              "Target not found in",
              _.keys(this.parent.human.targets.children).length,
              " loaded targets. Target=",
              tpath,
              ". Modifier=",
              this.name
            )
...
export class UniversalModifier extends ManagedTargetModifier {
  ...
  constructor(groupName, target, leftExt, rightExt, centerExt) {
    ...
    const left = leftExt ? `${targetName}-${leftExt}` : null
    let right = rightExt ? `${targetName}-${rightExt}` : null
    const center = centerExt ? `${targetName}-${centerExt}` : null
...
export class Modifiers {
  human: any
  children: {}
  blockEthnicUpdates: boolean
  modelingModifiers: any
  modifier_varMapping: {}
  dependencyMapping: {}
  constructor(human) {
    this.human = human
    ...
    // init
    this.loadModifiers(modelingModifiers).map(m => this.addModifier(m))
  }
    ...
    if (modifier.macroVariable && modifier.macroVariable !== "None") {
      if (
        ...
        console.error(
          ...
          modifier.groupName,
          this.modifier_varMapping[modifier.macroVariable]
        )
      } else {
        this.modifier_varMapping[modifier.macroVariable] = modifier.groupName
  ...
  _getRandomValue(minValue, maxValue, middleValue, sigmaFactor = 0.2, rounding) {
    ...
    let randomVal = randomNormal(middleValue, sigma)()
  ...
  randomValues(
    ...
    if (
      (randomValues["macrodetails/Gender"] || 0) > 0.5 ||
