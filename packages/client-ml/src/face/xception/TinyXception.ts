...
function mainBlock(x: tf.Tensor4D, params: MainBlockParams): tf.Tensor4D {
  let out = depthwiseSeparableConv(tf.relu(x), params.separable_conv0)
  out = depthwiseSeparableConv(tf.relu(out), params.separable_conv1)
  out = depthwiseSeparableConv(tf.relu(out), params.separable_conv2)
  out = tf.add(out, x)
  return out
}
...
export class TinyXception extends NeuralNetwork<TinyXceptionParams> {
  ...
  public forwardInput(input: NetInput): tf.Tensor4D {
    ...
    return tf.tidy(() => {
      ...
      const normalized = (normalize(batchTensor, meanRgb) as any).div(tf.scalar(256)) as tf.Tensor4D

      let out = tf.relu(conv(normalized, params.entry_flow.conv_in, [2, 2]))
      out = reductionBlock(out, params.entry_flow.reduction_block_0, false)
      out = reductionBlock(out, params.entry_flow.reduction_block_1)

      range(this._numMainBlocks, 0, 1).forEach((index) => {
        out = mainBlock(out, params.middle_flow[`main_block_${index}`])
      })

      out = reductionBlock(out, params.exit_flow.reduction_block)
      out = tf.relu(depthwiseSeparableConv(out, params.exit_flow.separable_conv))
      return out
    })
  }
