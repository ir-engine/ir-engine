...
export const creatorReducer = (_, action: CreatorActionType) => {
  Promise.resolve().then(() => creatorReceptor(action))
  return state.attach(Downgraded).value
}

const creatorReceptor = (action: CreatorActionType): any => {
  state.batch((s) => {
    let creators
    switch (action.type) {
      ...
      case 'CURRENT_CREATOR_RETRIEVED':
        ...
        return s.lastFetched.set(new Date())
