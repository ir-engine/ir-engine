...
import { NO_PROXY, useHookstate, useImmediateEffect } from '@etherealengine/hyperflux'
...
//#region Materials and geometry for gizmo
...
//#region Gizmo definitions - custom hierarchy definitions for setupGizmo() function
...
const setupGizmo = (obj: Object3D, gizmoMap) => {
...
const useGizmoObject = (translate, rotate, scale) => {
  const translateObj = useHookstate(() => new Object3D())
  const rotateObj = useHookstate(() => new Object3D())
  const scaleObj = useHookstate(() => new Object3D())

  useImmediateEffect(() => {
    setupGizmo(translateObj.get(NO_PROXY) as Object3D, translate)
    setupGizmo(rotateObj.get(NO_PROXY) as Object3D, rotate)
    setupGizmo(scaleObj.get(NO_PROXY) as Object3D, scale)
  }, [])
}
...
const useGizmo = () => {
  ...
  const gizmoObject = useGizmoObject(gizmoTranslate, gizmoRotate, gizmoScale)
  const pickerObject = useGizmoObject(pickerTranslate, pickerRotate, pickerScale)
  const helperObject = useGizmoObject(helperTranslate, helperRotate, helperScale)
...
export const TransformGizmoVisualComponent = defineComponent({
  ...
  reactor: function () {
    ...
    const { gizmo, picker, helper, gizmoObject, pickerObject, helperObject } = useGizmo()

    useImmediateEffect(() => {
      visualComponent.gizmo.set(gizmo)
      visualComponent.picker.set(picker)
      visualComponent.helper.set(helper)
    }, [])
