...
export function controlUpdate(gizmoEntity: Entity) {
  ...
  let parentEntity = SceneState.getRootEntity(getState(EditorState).scenePath!) // we can always ensure scene entity is root parent even if entty tree component doesnt exist
