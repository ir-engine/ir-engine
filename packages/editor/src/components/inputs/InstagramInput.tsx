...
import { InstagramFileTypes } from '@xrengine/engine/src/assets/constants/fileTypes'

const uploadOptions = {
  multiple: false,
  accepts: InstagramFileTypes
}
 ...
 * @returns
 */
export function InstagramInput({ onChange, ...rest }) {
  const onUpload = useUpload(uploadOptions)
  const [{ canDrop, isOver }, dropRef] = useDrop({
    accept: [ItemTypes.Instagram, ItemTypes.File],
    drop(item: any) {
      if (item.type === ItemTypes.Instagram) {
