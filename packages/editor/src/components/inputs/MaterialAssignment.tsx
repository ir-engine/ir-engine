...
import { MaterialOverrideComponentType } from '@xrengine/engine/src/scene/components/MaterialOverrideComponent'
...
function onChangeSize(text, values, onChange) {
  ...
  if (preCount > count) values.splice(count)
    ...
    for (let i = 0; i < count - preCount; i++) {
      values.push({ materialID: '', patternTarget: PatternTarget.OBJ3D, pattern: '' })
    }
  preCount = count
  onChange(values)
}
...
export default function MaterialAssignment({ entity, modelComponent, values, onChange }) {
