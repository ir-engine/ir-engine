...
// 1) Create a local "dynamicViewMode" object in your test file:
const dynamicViewMode = {
  _value: 'icons', // The real stored value
  get value() {
    // Called whenever the component does "viewMode.value"
    return this._value
  },
  set: vi.fn((newVal) => {
    // If the component or test calls "viewMode.set('list')", we store it here
...
describe('FilesToolbar component', () => {
  beforeAll(() => {
    // 1) Mock react-i18next so useTranslation() doesn’t fail
    ...
    vi.mock('@ir-engine/hyperflux', async (importOriginal) => {
      ...
      return {
        // @ts-ignore
        ...actual,
        // If your component also uses useHookstate for some local state, you can mock
        // it or return a minimal object that has “value” or “set(value)” etc.
        useHookstate: vi.fn().mockReturnValue({
          // Just a fake object with the shape your code might expect:
          value: false,
          set: vi.fn()
        }),
        ...
        useMutableState: vi.fn().mockImplementation((stateDef) => {
          if (stateDef === EditorState) {
            // The component calls: useMutableState(EditorState).projectName.value
            return {
              projectName: {
                value: 'mock-project'
              }
            ...
            // The component calls: useMutableState(FilesState).merge(...)
            return {
              selectedDirectory: {
                value: '/mock-directory' // Default selected directory for testing
              },
              projectName: {
                value: 'mock-project'
              },
