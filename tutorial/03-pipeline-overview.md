...
LOD is handled by engine/camera system using defined groups. For example:

![Empty groups associated with the different LODs in Blender](https://user-images.githubusercontent.com/578371/145126904-28c67da8-e0ed-40ba-8815-df34e49f6428.png)
...
To add these, add a cube. Move, rotate and scale the cube in object mode. Do not add a material to the collision object. Do not move vertexes of the cube in edit mode. Set custom properties to `type = box`. Don't use `type = obstacle` unless you have express authorization from the engineers.
...
Group colliders by similarity (i.e.: environment, objects, boxes, trimshes, etc...) and parent them to an appropriately named empty,
The group node must have Custom Properties set to `xrengine.collider.bodyType = 0` and `xrengine.entity` can be named anything.
The children nodes act as shapes of the body, and do not need the `xrengine.collider` prefix, but must have the `type` property set to one of `"box"`, `"obstacle"`, `"trimesh"`, `"ground"`, `"sphere"`, `"capsule"` or `"convex"`.
The most-used `type` values are `"box"`, `"obstacle"` and `"trimesh"`.

For each shape object, there are additional optional properties:

| optional property                 | type / allowed values   | default     |
|-----------------------------------|-------------------------|-------------|
| xrengine.collider.bodyType        | 0 (static), 1 (dynamic) | 0           |
| xrengine.collider.isTrigger       | true, false             | false       |
| xrengine.collider.staticFriction  | number                  | 0           |
| xrengine.collider.dynamicFriction | number                  | 0           |
| xrengine.collider.restitution     | number                  | 0           |
| xrengine.collider.collisionLayer  | number                  | 19          |
| xrengine.collider.collisionMask   | number                  | 1           |
| xrengine.collider.contactOffset   | number                  | 0           |
| xrengine.collider.restOffset      | number                  | 0           |
...
### Setting properties via Python scripts
...
Replace all selected object's custom properties tags and replace it with the active object's custom properties:
...
![image](https://user-images.githubusercontent.com/578371/145127185-5bd3d716-9bf8-484d-b172-e6a6f1b3b3cf.png)
 ...
 - __Baking setup with only 2 UV maps:__
...
![image](https://user-images.githubusercontent.com/578371/145127298-3b04ab11-4bd1-4f1d-a580-0c69003a6e13.png)
 ...
 - Denoise nodes are used to remove artifacts
...
File structure:
 ...
 - /Previews. Images for marketing purposes and work pre-visualization.
 - /Textures. Textures used as input for creating the asset

PBR Roughness - Metallic workflow will be used.
Texture sets will be used at discretion. We need to keep texture sets at a minimum to create performant assets. Each texture set will be represented as a different mesh in blender.
...
The source mesh will be high poly. It doesn't need to be UV unwrapped. It will be split into different materials.
...
__Low poly__: An optimized version of the high poly one. This will be the main file that we are going to work on painter and will be finally delivered on the GLB. It needs to be properly UV unwrapped (use UVPackmaster2). Also, all materials should be erased except 1 that should be named as [NAME]. The mesh needs to be named as: [NAME]\_low
...
UVs must be clean without overlapping. Use mapping nodes in blender to add tiling textures instead of scaling UVs. Use the UVPackmaster for asset creation and baking preparation to optimize the UV space as much as possible.
...
### Alt Lightmap Tools 
 ...
 - [Bake Wrangler Addon (free)](https://www.blendernation.com/2020/02/07/bake-wrangler-addon-free/) (note: This is cannot be used for baking combined lightmaps!)
...
[infosia/avatar-asset-pipeline](https://github.com/infosia/avatar-asset-pipeline): Avatar asset pipeline is a tool to create continuous integration build pipelines for 3D avatar assets using set of common transformation logic as a component, such as "A-pose to T-pose". 
...
[Blender - Import VRM file tutorial](https://styly.cc/tips/blender-modeling-vrm/)
...
[Converting an avatar to VRM format · GitHub](https://gist.github.com/emilianavt/51d8399987d67544fdebfe2ebd9a5149)
...
Save as copy on the Blender rig optimized folder the blend file. Import the avatar from mixamo and use mixamo plugin to fix the armature. Replace the materials of the newly imported armature with meshes from the original avatar. Delete the original model. Check functionality of the blendshapes/materials. Purge unused data and save the blend file again.
