...
import targetCategories from "../../character/json/targets/target-category-data.json"
import targetList from "../../character/json/targets/target-list.json"
...
export class TargetMetaData {
  ...
  constructor() {
    ...
    this.targetIndex = _.map(_.keys(targetList.targets), path => this.pathToGroupAndCategories(path))
    ...
    this.targetGroups = _(this.targetsUrls)
      .map(path => new Target(this.pathToGroupAndCategories(path)))
      .groupBy(gc => gc.group)
      .value()
  }
...
export class Targets extends TargetMetaData {
  ...
  load(dataUrl = "data/targets/targets.bin") {
    ...
    return new Promise((resolve, reject) => this.bufferLoader.load(dataUrl, resolve, undefined, reject))
      .then((data: any) => {
  ...
  lastmorphTargetInfluences(lastmorphTargetInfluences: any, morphTargetInfluences: any) {
