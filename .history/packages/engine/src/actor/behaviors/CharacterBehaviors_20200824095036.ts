...
import { Entity } from "../../ecs/classes/Entity";
    ...
    let characterTransform: TransformComponent
    let characterObject3D: Object3DComponent
	...
	{
        character = getMutableComponent<CharacterComponent>(entity, CharacterComponent as any)
        characterTransform = getMutableComponent<TransformComponent>(entity, TransformComponent as any)
        characterObject3D = getMutableComponent<Object3DComponent>(entity, Object3DComponent)
        // TODO: Something here
		...
		if (character.physicsEnabled) rotateModel(entity: Entity);
    ...
    // Move to system
	export const inputReceiverUpdate(entity, args: { timeStep: number }): void =>
	{
        character = getMutableComponent<CharacterComponent>(entity, CharacterComponent as any)
        characterTransform = getMutableComponent<TransformComponent>(entity, TransformComponent as any)
        characterObject3D = getMutableComponent<Object3DComponent>(entity, Object3DComponent)
	...
	export const setAnimation: Behavior(entity, args: { clipName: string, fadeIn: number }): number =>
	{
        character = getMutableComponent<CharacterComponent>(entity, CharacterComponent as any)
		...
		{
			// gltf
			let clip = AnimationClip.findByName( character.animations, clipName );
	...
	export const getLocalMovementDirection: Behavior(entity: Entity): Vector3 =>
	{
        character = getMutableComponent<CharacterComponent>(entity, CharacterComponent as any)
	...
	export const getCameraRelativeMovementVector: Behavior = (entity: Entity): Vector3 =>
	{
        character = getMutableComponent<CharacterComponent>(entity, CharacterComponent as any)
        characterObject3D = getMutableComponent<Object3DComponent>(entity, Object3DComponent)

		const localDirection = characterObject3D.value.getLocalMovementDirection();
	...
	export const setCameraRelativeOrientationTarget: Behavior = (entity: Entity): void =>
	{
		...
		{
			let moveVector = character.getCameraRelativeMovementVector();
	
			if (moveVector.x === 0 && moveVector.y === 0 && moveVector.z === 0)
			{
				character.setOrientation(character.orientation);
			}
			else
			{
				character.setOrientation(moveVector);
			}
		}
	}
	...
	{
		character.lookAt(character.position.x + character.orientation.x, character.position.y + character.orientation.y, character.position.z + character.orientation.z);
	...
	export const jump(initJumpSpeed: number = -1): void =>
	{
		...
		character.initJumpSpeed = initJumpSpeed;
	}
	...
	export const findVehicleToEnter(wantsToDrive: boolean): void =>
	{
		...
		// Find best vehicle
		let vehicleFinder = new ClosestObjectFinder<Vehicle>(character.position, 10);
		character.world.vehicles.forEach((vehicle) =>
		{
		...
		if (vehicleFinder.closestObject !== undefined)
		{
			...
			// Find best seat
			let seatFinder = new ClosestObjectFinder<VehicleSeat>(character.position);
	...
	export const enterVehicle(seat: VehicleSeat, entryPoint: Object3D): void =>
	{
		...
		{
			character.setState(new OpenVehicleDoor(character, seat, entryPoint));
		}
		else
		{
	...
	export const startControllingVehicle(vehicle: IControllable, seat: VehicleSeat): void =>
	{
		if (character.controlledObject !== vehicle)
		{
			...
			vehicle.controllingCharacter = character;
		}
	}
	...
	export const physicsPostStep(body: Body, character: Character): void =>
	{
		...
		else
		{
			newVelocity = new Vector3(
				Mathlerp(simulatedVelocity.x, arcadeVelocity.x, character.arcadeVelocityInfluence.x),
				Mathlerp(simulatedVelocity.y, arcadeVelocity.y, character.arcadeVelocityInfluence.y),
				Mathlerp(simulatedVelocity.z, arcadeVelocity.z, character.arcadeVelocityInfluence.z),
			);
		}
	...
	{
		if (_.includes(world.characters, character))
		{
		...
		else
		{
			...
			// Register physics
			world.physicsWorld.addBody(character.characterCapsule.body);

			// Add to graphicsWorld
			world.graphicsWorld.add(character);
			world.graphicsWorld.add(character.raycastBox);
	...
	export const removeFromWorld(world: World): void =>
	{
		...
		{
			if (world.inputManager.inputReceiver === character)
			{
			...
			_.pull(world.characters, character);

			// Remove physics
			world.physicsWorld.remove(character.characterCapsule.body);

			// Remove visuals
			world.graphicsWorld.remove(character);
			world.graphicsWorld.remove(character.raycastBox);
		}
	}
}