...
export function execute (delta?: number, time?: number): void {
		...
		// Getting timeStep
		let unscaledTimeStep = (Engine.renderDelta + Engine.logicDelta) ;
		let timeStep = unscaledTimeStep * Engine.params.Time_Scale;
		timeStep = Math.min(timeStep, 1 / 30);    // min 15 fps

    // Logic
    if (Engine.enabled) {
      Engine.systemsToExecute.forEach(system => system.enabled && executeSystem(system, Engine.logicDelta, Engine.clock.elapsedTime));
