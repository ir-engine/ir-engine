...
import { Geometry, Mesh, LoadingManager } from "three";
...
export default class DracoFileCreator {
    constructor(renderer, meshFileSuffix, textureFileSuffix, frameIn, frameOut, outputFileName, progressCallback) {
        this._meshFiles = [];
        this._textureFiles = [];
        this._frameData = [];
        this._maxVertices = 0;
        this._maxFaces = 0;
        this._basisLoader = new BasisTextureLoader();
        ...
        this._manager.onProgress = function (item, loaded, total) {
            ...
            if (progressCallback)
                progressCallback(item, loaded, total);
        };
        // Read path to OBJs and make array
        glob.glob(__dirname + "/*." + meshFileSuffix, {}, (err, files) => {
            if (err)
                console.log(err);
        ...
        glob.glob(__dirname + "/*." + textureFileSuffix, {}, (err, files) => {
            if (err)
                console.log(err);
    ...
    createEncodedFile(fileName, callback) {
        ...
        for (let i = this._frameIn; i < frameOut; i++) {
            // load obj
            this._loader.load(this._meshFiles[i], obj => {
                // Search for mesh object
                obj.traverse(child => {
                    if (child instanceof Mesh) {
                        this.mesh = child;
                        this.geometry = child.geometry;
                        if (child.geometry.vertices > this._maxVertices)
                            this._maxVertices = child.geometry.vertices;
                        if (child.geometry.faces > this._maxFaces)
                            this._maxFaces = child.geometry.faces;
            ...
            const encodedMesh = encodeMeshToDraco(this.mesh.geometry); // convert obj to draco
            const frame = {
                frameNumber: i,
            ...
            // progress callback
            if (callback)
                callback(i - this._frameIn / frameOut - this._frameIn);
        }
        // Close file stream
        writeStream.close;
        ...
        prependFile.prependFile(fileName, combinedBuffer, (err) => {
            if (err)
                console.error(err);
            else
                console.log("Prepended data to " + fileName);
        });
        // Progress callback
        if (callback)
            callback(1);
