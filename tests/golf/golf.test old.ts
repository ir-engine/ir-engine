...
describe('Golf tests', () => {
  ...
  test(
    'ball moved forward, if faild, force server + value, client - value',
    async () => {
      await bot.delay(1000)
      // rotate such that hit direction is in line with the hole
      const teePosition = await bot.runHook(GolfBotHooks.GetTeePosition)
      expect(vector3.copy(await bot.runHook(GolfBotHooks.GetBallPosition)).sub(teePosition).z).toBeLessThan(-0.5)
      await bot.delay(1000)
    },
    maxTimeout
  )
  ...
  test(
    ...
    async () => {
      ...
      expect(
        vector3
          .copy(await bot.runHook(BotHooks.GetPlayerPosition))
          .sub(await bot.runHook(GolfBotHooks.GetBallPosition))
          .length()
      ).toBeLessThan(0.5)
      await bot.awaitHookPromise(GolfBotHooks.GetIsBallStopped)
      await bot.delay(1000)
