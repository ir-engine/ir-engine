...
*The following is largely from the ECSY documentation, but has been reformatted to reflect our functional API*

![ECS architecture](https://ecsy.io/docs/manual/images/ECSY%20Architecture.svg)

Some common terms within ECS engines are:
- Entities: an object with an unique ID that can have multiple components attached to it.
- Components: different facets of an entity. ex: geometry, physics, hit points.   Data is only stored in components.
- Systems: do the actual work with in an application by processing entities and modifying their components.
...
## Components
A `Component` is an object that can store data but should have not behaviour (As that should be handled by systems). There is not a mandatory way to define a component.

To create a component you must extend the `Component` class and define a schema:
```javascript
import { Component } from '@xrengine/engine/src/classes/Component';
...
You can also define your own types with `createType()`.

```javascript
import { createType, copyCopyable, cloneClonable } from '@xrengine/engine/src/types/Types';
...
When releasing that component, by calling `removeComponent(entity, ComponentA)`, it will get returned to the pool, ready to be used by another entity.

The engine should know how to reset a component to its original state, if your component has the proper schema defined, the engine will do this for you.
...
SSC can be defined by extending `SystemStateComponent` instead of `Component`. Once the SSC is defined, it can be used as any other component.
```javascript
...
## Entities
An entity is an object that has a unique ID. Its purpose is to group components together.
...
### Adding components
Once an entity is created, it is possible to add Components to it:
```javascript
class ComponentA {
  constructor() {
    this.number = 10;
...
```javascript
removeComponent(entity, ComponentA);
```
...
- `init()`: This function is called when the system is registered in a world (Calling `registerSystem`) and can be used to initialize anything the system needs.
