...
cli.main(async () => {
    try {
        ...
        if (currentFunctions.indexOf(name) > 0)
            functionResponse = await storageProvider.updateFunction(name, routes)
        else
            functionResponse = await storageProvider.createFunction(name, routes)
