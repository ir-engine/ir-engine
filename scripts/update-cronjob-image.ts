  ...
  try {
    ...
    if (app.k8BatchClient)
      for (const project of autoUpdateProjects.data) {
        await app.k8BatchClient.patchNamespacedCronJob(`${project.name}-auto-update`, 'default', getCronJobBody(project, `${options.ecrUrl}/${options.repoName}-api:${options.tag}__${options.startTime}`), undefined, undefined, undefined, undefined, {
          headers: {
