...
cli.main(async () => {
    try {
        ...
        for (const avatar of Object.keys(currentAvatars)) {
            ...
            const thumbnailReturned = await addGenericAssetToS3AndStaticResources(app, Buffer.from(await thumbnail.arrayBuffer()), {
                key: `avatars/public/${existingAvatar.identifierName}.glb.${/.png/.test((thumbnail.headers.get('content-type') || '')) ? 'png' : 'jpg'}`,
                contentType: thumbnail.headers.get('content-type') || '',
                staticResourceType: 'user-thumbnail'
            })

            const modelReturned = await addGenericAssetToS3AndStaticResources(app, Buffer.from(await model.arrayBuffer()), {
                key: `avatars/public/${existingAvatar.identifierName}.glb`,
                contentType: await model.headers.get('content-type') || '',
