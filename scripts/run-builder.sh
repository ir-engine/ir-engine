...
until [ -f /var/lib/docker/certs/client/ca.pem ]; do
...
cd packages/client && npx cross-env ts-node --swc scripts/create-env-production.ts >buildenv-build-logs.txt 2>buildenv-build-error.txt || (cd "$HOME/app" && npm run record-build-error -- --service=buildenv)
...
elif [ "$SERVE_CLIENT_FROM_API" = "true" ]; then
...
if [ -z "$(kubectl get jobs | grep $RELEASE_NAME-builder-etherealengine-builder)" ]; then
