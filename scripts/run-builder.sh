...
npx cross-env tsx scripts/check-db-exists.ts
...
npx cross-env tsx scripts/create-root-package-json.ts
...
then
  npx cross-env tsx scripts/get-deletable-client-files.ts
...
npx cross-env tsx scripts/update-cronjob-image.ts --repoName=${DESTINATION_REPO_NAME_STEM} --tag=${TAG} --repoUrl=${DESTINATION_REPO_URL} --startTime=${START_TIME}

npx cross-env tsx scripts/clear-projects-rebuild.ts
...
if [ "$SERVE_CLIENT_FROM_STORAGE_PROVIDER" = "true" ] && [ "$STORAGE_PROVIDER" = "s3" ] ; then
  npx cross-env tsx scripts/delete-old-s3-files.ts;
